...
- **Sidebar shortcuts:** a compact bottom icon grid starts with one Settings gear menu containing Workspace settings (General) when a Workspace is validated and Settings for User, Server, and Administration. Remaining icons link to the validated current Workspace's Repositories, Catalog, Workspace changes, Recovery, Archived threads, Vault, and Usage & audit surfaces. Without a validated Workspace, only the Settings gear remains. Every icon has an accessible name and current-page state, and no destination is duplicated elsewhere in the sidebar. Its controls wrap into four columns within the available sidebar content width; the sidebar never scrolls horizontally. The Search popover uses the same content width.

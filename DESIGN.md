...
- **Sidebar shortcuts:** a compact bottom icon grid starts with one Settings gear button that opens Settings directly at Account, without an intermediate menu choosing between Workspace settings and Settings. Remaining icons link to the validated current Workspace's Repositories, Catalog, Workspace changes, Recovery, Archived threads, Vault, and Usage & audit surfaces. Without a validated Workspace, only the Settings gear remains. Every icon has an accessible name and current-page state, and no destination is duplicated elsewhere in the sidebar. Its controls wrap into four columns within the available sidebar content width; the sidebar never scrolls horizontally. The Search popover uses the same content width.
- **Footer:** the Settings gear shares the shortcut grid; there is no separate full-width Settings row. No stacked user-identity row appears because identity lives inside Settings (D-002).
...
The Composer is one friendly rounded container with 16px radius, card surface, faint shadow, a visually spacious upper text-entry region, and one lower action row. The text-entry region is the first content inside the container, with no title or mode-label row above it. The larger chat starter and bottom-docked Thread form use the same component; exact sizing, growth, scrolling, and keyboard behavior are defined only by `docs/specs/20260831-unified_conversation_composer.md`.

The lower action row follows one fixed visual order: attachment `+`, conversation-target selector, flexible spacer, logical-model selector, and circular accent send button. `+` opens the existing Artifact reference and attachment upload chooser, with access to pre-send Advanced settings for an explicit Worker environment choice when starting Task work. Advanced settings default to a new environment; retained-environment provenance, eligibility and submission behavior are owned by the Composer specification. The target selector may show built-in Assistant, a running Goal Orchestrator, the Workspace Knowledge Manager, pinned warm workers, running worker Threads, and `New Shard + Worker`; that last label creates a new Task execution Thread and does not create a durable Shard entity. The model selector shows only logical model IDs and product-safe labels admitted for the selected target; it never exposes a Provider profile, Provider-native model, account slot, private route, Sandbox, Harness, process, or AgentSession identity.

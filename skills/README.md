...
Public Vault secret administration exposes `vault.secret-create`, `vault.secret-rotate`, `vault.secret-revoke`, `vault.grant-create`, and `vault.grant-revoke`. Creation and rotation consume secret stdin JSON. Validation failures for secret-input operations return a fixed error without request-derived schema issues, which may themselves contain secret keys or values. Grants in this slice authorize approved host Git push; repository binding stays in `repository.set-default`. See the public administration reference and operator recipe for the complete flow.

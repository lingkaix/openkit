...
Current builds still require their configured Worker images to be provisioned before startup; the accepted removal of that preload gate is not yet implemented. The current pinned stock OpenShell Gateway may nevertheless access GHCR to obtain its Supervisor image during startup, before Gateway health and NanoCore readiness. This is accepted upstream behavior: empty Worker-image storage does not mean completely offline startup. If bootstrap cannot reach the required registry, inspect the NanoHost/Gateway logs and host DNS, HTTPS and proxy access to GHCR; do not bypass TLS, change the pinned release or erase retained Worker data to repair it. The forthcoming point-of-use Worker-image path does not remove this stock Supervisor dependency.

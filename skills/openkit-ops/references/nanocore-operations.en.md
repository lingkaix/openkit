...
## A2 ws_7 two-Task GitHub dogfood
...
Start Task A on a new shared Thread using `task.start` with `workerStorageChoice: {"kind":"fresh"}`. Ask the Worker to commit and push its branch through OpenKit's `repository.push-request-approval` and `repository.push-execute` operations, then run `gh pr create` inside its sandbox. Record the Task's retained `storageRef` from `worker-environment.list` and its revision, and confirm the PR URL through `gh pr view`. Keep the associated issue open.

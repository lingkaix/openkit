...
For Task B, use `worker-environment.select` with the exact Task A `storageRef`, current `expectedRevision`, `layoutDigest`, `threadId`, `purpose: "work"`, and `taskId: null` for the related new Task; inspect the operation's full schema with `openkit ops describe worker-environment.select`. Selection checks eligibility but does not attach storage. Start Task B on the related Thread using `task.start` with `workerStorageChoice: {"kind":"selected","storageRef":"wst_RETURNED_REF","expectedRevision":REVISION,"purpose":"work"}`. Ask it to inspect the retained checkout, run `gh pr review` and `gh pr merge`, then `gh issue close` for the linked issue. Use distinct GitHub identities for authorship and review if GitHub rejects self-review. Verify the PR merged and issue closed with `gh pr view` and `gh issue view`, and inspect redacted `vault.use-list`, Task history, and `worker-environment.list` for both contributors. If selection reports stale, unknown, incompatible layout, audience, or competing writer, inspect current status and do not weaken the reuse check.

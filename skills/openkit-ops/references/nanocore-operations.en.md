...
If a known validation defect prevents the current image from reopening retained data, do not claim a compatible automatic rollback or restart that image first. While the App still answers, create and verify a deployment backup through its public API, then build and smoke-test the fixed image before the authorized operator replacement. Preserve raw evidence and all configured mounts; retain the previous container as evidence, not a runnable recovery target. Verify the unchanged evidence digest, new boot, retained authentication, NanoHost successor readiness and loaded configuration. This operator recovery is distinct from a successful ordinary App-update receipt.
...
The helper refuses a configured NanoHost with `ready: false` before acquiring or building the candidate and before stopping the App. Inspect the existing execution-host service and its bounded failure evidence under separate host authorization; recover that service before a new update attempt. Do not restart NanoHost inside the update to manufacture a successful readiness predicate. A genuinely unconfigured NanoHost remains optional, while a configured target must reconnect on a greater generation after replacement.
...
If the existing App mounts a repository directory read/write at `/srv/repos`, set the optional protected helper field `repositoryDirectory` to that exact absolute host directory. This declares an existing mount for preservation; it does not create a repository or authorize a new bind. Missing, undeclared or mismatched mounts fail before stopping the App. After rebuilding a server, verify the configured `adminTokenFile` still authenticates against that server; an old credential is not made valid by restoring its file. Refresh it only through the authorized credential procedure, without printing it or changing its scope.
...
Use the public backup/export operations for their documented scopes and re-read their result. A Workspace export is not a complete deployment backup. Portable Workspace archives are `.openkit-workspace.tar.zst` files, not Data Root backups. In server mode, download and import them through Web **Settings → Portability** with the signed-in canonical Better Auth session: **Export workspace**, **Download archive**, choose that exact tar archive in **Portable archive**, **Review import**, then **Import workspace** for the same selected file. The public Skill `workspace.archive-download`, `workspace.archive-import-dry-run`, and `workspace.archive-import` operations are local-mode only; they do not store cookies or use a server-admin bearer as a session substitute. Re-read the import result, then rebind repository and Vault references on the target when needed.

A consistent offline deployment copy requires NanoCore stopped and its Data Root lock released; copy all selected authoritative data plus the external key and required protected configuration under separately secured custody. Do not copy a live SQLite file as if it were a consistent snapshot or place secret files into an ordinary artifact.
...
A grant created by this host-push recipe is restricted to `gateway-only` and `workspace.git.push`. Create a second grant with `injectionPath: "runtime-env"` for the separate Worker credential path; never put the host-push grant in a Worker binding. Generic secrets may use another lowercase `secretKind`, but the public worker-visible grant path accepts only `github-token`.

## A2 ws_7 repository GitHub dogfood

After deploying this build on A2, run `openkit doctor` and inspect `vault.reference-list`, `vault.grant-list`, and the `ws_7` Agent and Workspace configuration. Reuse the active `github-token` reference and keep its existing gateway-only host-push grant in `git.vaultGrantRef`. Mint the distinct Worker grant with `printf '%s' '{"workspaceId":"ws_7","referenceId":"vault_RETURNED_ID","injectionPath":"runtime-env"}' | openkit ops call vault.grant-create --input -`. In the selected dogfood Agent Manifest, declare a required sandbox credential with `requirementId: "github-token"`, `visibility: "runtime-env"`, and `targetEnvVarName: "GITHUB_TOKEN"`; bind that requirement to the new grant ID in `ws_7`'s selected Agent `credentialBindings`. Follow the [Agent and Workspace examples](./nanocore-data-root-config.en.md#workspacesworkspaceidconfigworkspacejsonc), then reload or restart configuration as required and verify Agent readiness. The Worker image must contain `gh`, and its sandbox network rules must allow the intended GitHub API endpoint.

Before worker publication, explicitly select the built-in `openkit-repository` MCP target in the dogfood Codex Agent Manifest's `mcp` list, retaining any existing selections, and reload configuration through the normal administration path. A newly admitted Codex Worker receives the selected tools through its existing authenticated local relay; unsupported adapters fail selection. No App API endpoint, App administrator token or additional network opening is needed for these tools. The public Skill CLI uses `OPENKIT_NANOCORE_URL`, not `OPENKIT_BASE_URL`, and defaults to `127.0.0.1:3000`; inside a sandbox that default addresses the sandbox itself. The public CLI's `repository.push-*` calls are not the worker publication path, and a GitHub token does not authenticate them to NanoCore.

Task Mode prompts that ask to review, approve, or merge a concrete PR (PR number or GitHub pull URL) are delegated to a Worker. Bare “review the previous work” remains human review-mode and is not treated as a false-negative solely because of the word review.

Start editing Task A on a new shared Thread using `task.start` with `workerStorageChoice: {"kind":"fresh"}`. Let it finish and hand off its retained changes; output collection occurs after the Worker terminal barrier. Complete the existing authorized review/apply flow and verify the resulting commits in the host-linked checkout. Record Task A's current `storageRef` and `revision` from `worker-environment.list` and `worker-environment.status`. List `environment.snapshot-list`, match Task A's exact Thread and Turn, and copy `snapshot.extensions.openkit.workerStorage.workSlotRef` for Task B. A sandbox-only commit cannot be pushed through the host publication capability; `git_push_source_unavailable` means the host source must be admitted and verified before a new request.

Start a separate publication Task using the selected Codex Agent. Supply the verified host source, exact commit IDs, repository resource and intended target branch. Discover `repository_push_request_approval` and `repository_push_execute` on `openkit-repository`, request approval and inspect the returned approval identifiers. Human mode creates a real Action Center Gate and stops the Worker; it becomes actionable only after proved worker cleanup and handoff. After an authorized human grants it, start a new Worker request to execute that exact approval. Inspect-only `recovery_required` requires inspection, not an approval response or blind retry. Deployment-configured `repo.push` `auto_allow` keeps the publication Worker running so it can call the execute tool separately. Neither mode relaxes repository targets, review linkage, Vault grants or interrupted-push recovery. After a `pushed` record, ask the publication Worker to run `gh pr create` with explicit repository, base and `--head` equal to the published target branch. Confirm the PR URL with `gh pr view` and keep the associated issue open.

For the #84 regression smoke, first verify that an Agent without this explicit MCP selection cannot discover or call the repository tools. With the selection, verify a real pending Approval and Action Center Gate in human mode, or a still-running Worker followed by a `pushed` record in an already authorized auto-allow Workspace. Use a source already admitted to the host-linked checkout and inspect `repository.push-list` plus redacted Task and Vault evidence. Transport success alone is insufficient. Do not change persistent deployment policy just to pass the smoke, print token values, or report live worker-to-host commit synchronization as covered by this routing check.

For Task B, describe `worker-environment.select`, `environment.snapshot-list`, and `task.start` first. Use `worker-environment.select` with Task A's `storageRef`, the **current** `expectedRevision` and `layoutDigest`, the related Task's `threadId`, `purpose: "work"`, and `taskId: null`. Selection checks eligibility and does not attach storage. Same `storageRef` without `reuseWorkSlotRef` admits a distinct Thread slot on that volume; it does not recover Task A's checkout. Start Task B on the related Thread using `task.start` with `workerStorageChoice: {"kind":"selected","storageRef":"<storageRef>","expectedRevision":<current revision>,"purpose":"work","reuseWorkSlotRef":"<workSlotRef>"}`. The Web conversation composer cannot send that choice; use the public Skill or App API for this operation. Re-read `worker-environment.status` for the selected association, then ask a read-only Worker inspection of that checkout before claiming reuse. Then run `gh pr review` and `gh pr merge`, then `gh issue close` for the linked issue. Use distinct GitHub identities for authorship and review if GitHub rejects self-review. Verify the PR merged and issue closed with `gh pr view` and `gh issue view`, and inspect redacted `vault.use-list`, Task history, and `worker-environment.list` for both contributors. If selection reports stale, unknown, incompatible layout, audience, or competing writer, inspect current status and do not weaken the reuse check.

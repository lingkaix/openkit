...
Before approval, use `goal.revise` with the exact current Plan and the user's requested changes, then `goal.plan` to produce a revised draft. The existing Goal Orchestrator reads that recorded instruction and prior immutable Plan with only `goal.plan.propose`; it cannot approve, launch Workers, or access repositories, MCP or secrets. The proposal Tool checks unresolved questions and changes against the prior Plan before acknowledging a candidate, allowing correction within the bounded model loop. A successful Tool response is not evidence of durable persistence or human approval. Read the new draft with `goal.plan-read` and obtain human approval again. Missing model configuration, an invalid or unchanged proposal, cancellation or model failure leaves the Goal recoverable in planning; do not present the old deterministic draft as a successful revision. Concurrent requests for the same Goal are rejected or coalesced rather than creating competing drafts. Read `usage.read` for durably recorded successful revision model calls, including calls whose proposal was later rejected; process-local Gateway diagnostics alone are not the Workspace ledger.

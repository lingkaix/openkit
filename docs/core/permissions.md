...
An active member is eligible for the full operation set of their Workspace, including every Light App in it. This initial product baseline uses the existing Policy Kernel and does not require owner/editor/viewer grant ceilings, per-app roles, or collection/field ACLs. Membership is evaluated from current authoritative facts; invitations, removed membership and guessed identifiers do not grant Workspace content access. A currently usable presented `server-admin` bearer is the explicit cross-Workspace exception described below.
...
- A presented usable `server-admin` bearer supplies explicit cross-Workspace eligibility, including Task execution, while preserving current credential validation, private audience checks, effect policy and audit attribution. Owning a Token without presenting it grants no Workspace bypass.
- Missing actor, responsible-user, required membership or presented administrator credential, resource, token-binding, or required policy facts MUST fail closed.

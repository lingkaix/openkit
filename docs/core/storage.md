...
| --- | --- | --- |
| Raw audio, microphone frames, partial transcripts, interrupted audio, speculative tokens, incomplete Tool calls, runtime-private chain-of-thought, and hidden provider reasoning | Not persisted by default. Bounded raw capture requires an accepted owning capture specification plus a current permission-owner allow and remains `restricted-raw`; a bounded redacted health, retry, or feature-negotiation summary is `ephemeral-diagnostic`. Raw-audio capture is forbidden until an accepted owner declares a capture path. | Neither class turns raw material into product history, completion proof, authority, or required recovery state. A future retained audio product requires its own accepted consent, access, visibility, export, deletion, and retention contract. |

...
| Change execution and rollout evidence | `docs/changes/` |
...
Goal summaries can include the current task, task counts, pending human attention, terminal stop reason, terminal summary, stored task verification evidence, artifact ids, risks, and suggested next work. They do not synthesize queued or applied steering counters: unresolved delivery is projected from the exact pending owner, accepted delivery from the verified S39 trace, and historical command replay from those owners. Task Evaluator loops and an independent final-verifier completion gate remain deferred.
...
The Goal steering route accepts one exact message or current non-restricted Material revision only while one checkpoint-backed worker Turn owns the active Goal. Accepted submission returns HTTP `202` with exactly `{ state: "queued", pendingTurnId, requestId, contentItemId, goalId, activeTurnId }` after the completed input Item, Thread-unique pending owner, and body-free command receipt are durable. The owning Goal step claims the input for its reserved `ctxpkg_<turnId>`, includes it in the exact verified S39 Context Package, and cleans the pending row only after trace verification and queue compare-and-set complete before worker launch. Missing capability, conflicting ownership, partial proof, or changed replay fails with the typed S16 result and never mutates a live worker filesystem; human-gate responses remain separate commands.

The terminal steering targets are `POST /api/app/workspaces/:workspaceId/threads/:threadId/goal/steering/:pendingTurnId/follow-up` as `convertGoalSteeringToFollowUp` and `POST /api/app/workspaces/:workspaceId/threads/:threadId/goal/steering/:pendingTurnId/cancel` as `cancelGoalSteering`. Each request body is exactly `{ requestId }`, where that identity belongs to the terminal command and remains distinct from the original send request stored by the pending owner. Both successes return HTTP `200`. The follow-up response is exactly `{ state: "follow-up", pendingTurnId, requestId, sourceRequestId, contentItemId, goalId, activeTurnId, followUpTurnId, followUpItemId }`; the cancel response omits only the two follow-up identities and has `state="cancelled"`. S16 owns the completed Core-local history Turn, immutable `SteeringTerminalOutcome`, body-free receipt, cleanup, replay, and failure predicates. Both terminal routes require the original Goal to be terminal, preserve its historical lineage, and reject terminal-request reuse before creating a follow-up effect.
...
The generic unversioned Artifact Review route is absent. S16's version-owned Artifact Review route owns ordinary Artifact decisions, while the Workspace Sync Review route owns only its exact durable staged review and passes only `accepted`, `needs_refinement`, `rejected`, or `blocked` without translation. The exact durable Workspace Sync Review `artifactId` excludes that presentation Artifact from generic Review decisions regardless of either owner state; contradictory dual ownership returns `recovery_required` from both decision routes and remains one inspect-only risk projection rather than becoming a fallback or alias.
...
### Artifact And Material Interaction

The accepted S16 contract defines the following App API operations. Artifact import, idle-Thread introduction, the eleven Material operations, and the two version-owned Artifact Review operations are live in NanoCore, generated OpenAPI, and Core Client. These are the only owner-specific operations in this family; the deleted generic Artifact mutation and unversioned Review aliases remain absent.

| Method and path | `operationId` | Success status | Success identity |
| --- | --- | --- | --- |
| `POST /api/app/workspaces/:workspaceId/artifacts/imports` | `importWorkspaceArtifact` | `201` | `{ artifactId, artifactVersion }` |
| `POST /api/app/workspaces/:workspaceId/threads/:threadId/artifacts/:artifactId/introductions` | `introduceWorkspaceArtifact` | `201` | `{ artifactId, artifactVersion, turnId, itemId }` |
| `GET /api/app/workspaces/:workspaceId/artifacts/:artifactId/reviews` | `listArtifactReviews` | `200` | `{ reviews }`, ordered by `artifactVersion` ascending |
| `POST /api/app/workspaces/:workspaceId/artifacts/:artifactId/versions/:artifactVersion/review/decision` | `submitArtifactReviewDecision` | `200` | `{ reviewId, artifactId, artifactVersion, decision, followUpTurnId }` |
| `GET /api/app/workspaces/:workspaceId/materials` | `listWorkspaceMaterials` | `200` | `{ materials }`, ordered by `createdAt`, then `materialId` |
| `POST /api/app/workspaces/:workspaceId/materials` | `createWorkspaceMaterial` | `201` | `{ materialId }` |
| `GET /api/app/workspaces/:workspaceId/materials/:materialId` | `getWorkspaceMaterial` | `200` | `{ material }` |
| `GET /api/app/workspaces/:workspaceId/materials/:materialId/revisions` | `listWorkspaceMaterialRevisions` | `200` | `{ revisions }`, ordered by `createdAt`, then `revisionId` |
| `POST /api/app/workspaces/:workspaceId/materials/:materialId/revisions` | `saveWorkspaceMaterialRevision` | `201` | `{ materialId, revisionId }` |
| `GET /api/app/workspaces/:workspaceId/materials/:materialId/revisions/:revisionId` | `getWorkspaceMaterialRevision` | `200` | `{ revision }` |
| `GET /api/app/workspaces/:workspaceId/threads/:threadId/material` | `getThreadMaterial` | `200` | `{ material }`, where `material` is the singular Thread projection or null |
| `POST /api/app/workspaces/:workspaceId/threads/:threadId/materials/:materialId/bind` | `bindThreadMaterial` | `200` | `{ materialId, threadId, outcome: "bound" }` |
| `POST /api/app/workspaces/:workspaceId/threads/:threadId/materials/:materialId/unbind` | `unbindThreadMaterial` | `200` | `{ materialId, threadId, outcome: "unbound" }` |
| `POST /api/app/workspaces/:workspaceId/threads/:threadId/materials/:materialId/exclude` | `excludeThreadMaterial` | `200` | `{ materialId, threadId, outcome: "excluded" }` |
| `POST /api/app/workspaces/:workspaceId/threads/:threadId/materials/:materialId/restore` | `restoreThreadMaterial` | `200` | `{ materialId, threadId, outcome: "included" }` |

Mutation request bodies are exactly the canonical inputs in S16 plus the required `requestId`; path identifiers are scope, not duplicated caller input. Artifact import returns version 1, Artifact introduction returns the deterministic completed Turn and Item, Material responses return stable identities only, and an Artifact Review decision returns the version-owned Review identity plus its nullable applied Material revision and deterministic follow-up Turn. Exact replay returns the same success identity, changed input returns `idempotency_key_conflict`, and typed conflict, stale, busy, and recovery results follow S16 without success-shaped error payloads.

Every target route authorizes its path Workspace before target lookup and then reads only that Workspace's owners. An opaque target id absent there returns `stale`, including a string that exists in another Workspace; NanoCore does not scan other Workspace databases to distinguish foreign from unknown ids. `forbidden` means access to the addressed path Workspace or another pre-disclosure authorization predicate failed.

The Artifact Review list exposes only S16's closed `ArtifactReviewView`, never the full owner row or its `decisionRequestId`. An Action Center Artifact Review source includes `reviewId`, `artifactId`, and `artifactVersion`, derives its row identity from that exact owner, and in Stage 4 exposes only a read-only Artifact inspection action; Stage 5 may connect executable decisions only to the existing versioned endpoint. An Artifact-id-only source or generic fallback is invalid. Material list, detail, revision-summary, exact-revision-content, and Thread material responses use only the other closed public view shapes in S16; owner-only request proof is not added to a response. The Thread material read does not retain expected-base conflict state. An authorized human-facing App API client may read and edit a restricted exact revision. The transport-neutral Agent Skill follows S16's narrower operation set: metadata reads and binding or inclusion transitions remain available, while restricted create, exact-content get, and save fail `sensitive_content` before a content-bearing route and no generic call bypass exists. Workspace Sync Review remains a different owner and route family under S49; its exact `artifactId` relationship excludes that presentation Artifact from generic Artifact Review projection, and neither route may translate or fall back to the other.

All fifteen operations have landed across schemas, NanoCore routes, generated OpenAPI, and Core Client, including the Stage 4 worker-output producer and version-owned Artifact Review authority. The removed user-facing MCP facade must not be restored; Agent-facing use is projected through the transport-neutral operation catalog, bundled CLI, and unified Skill only after each App API contract is executable.

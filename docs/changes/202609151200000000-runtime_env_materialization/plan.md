...
The branch was pushed to `origin/fix/81-runtime-env-materialization`, and [PR #82](https://github.com/lingkaix/openkit/pull/82) was opened against `main` with `Closes #81`. Implementation head is `442ac91`; the PR is unmerged and CI was running at handoff. No production deployment, A2 Task recipe, real GitHub authentication, review/merge/issue-close dogfood action or live secret read was performed. These fixture results do not claim that A2 has been updated. Private `turn.start` changes require the matching worker-shim image and NanoCore build; older shim images refuse the new field with no success receipt. No compatibility path is introduced.

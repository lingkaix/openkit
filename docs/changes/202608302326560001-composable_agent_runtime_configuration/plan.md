...
- **Current facts:** Current schemas and implementation preserve one concrete LLM route per AEP, one fixed Codex Harness despite an adapter registry, one active Turn across that Harness, Workspace-record execution defaults, direct Core/Gateway Provider defaults, no User config loader, a model-to-Agent inversion, no production NanoHost credential injection, an unused AEP `staleWhenPackageChanges` value, and a Gateway model catalog that can advertise an undispatchable Provider model while exposing the Provider profile ID. Codex already launches one child per Turn and resumes the exact native handle from AgentSession-private state.
- **Accepted direction:** Intent Epochs 5 and 6 reconcile the engineer direction with independent evidence. Server provides resources and defaults without acting as a generic Workspace ceiling; authored Workspace composition precedes non-authoring resolution; User → Workspace → Server is persistent preference order; logical model routing is Gateway-private and pinned to catalog-derived effective capabilities and `modelFamilyId`; Sandbox is the containment boundary; and the unified Composer selects a role or Agent plus logical model while `New Shard + Worker` enters Task Mode by creating a new execution Thread rather than a Shard entity.
- **Delivery boundary:** Intent Epoch 3 extends the work through a runnable system. The first durable gate is independently accepted and committed documentation; no production implementation may begin before that commit.
- **Resolved unknowns:** Use one `internal-role-profiles.jsonc`; add no shared environment file; use `users/<userId>/config/user.jsonc`; keep the current `session.concurrent-turns` feature ID; derive logical-model capabilities and family from the vendored catalog; expand manifest `all` only into a new immutable AEP; represent account rotation as ordered Provider-profile members; and keep credential materialization, Pi readiness correction, concurrency activation, Workspace rename rejection, natural next-Turn activation, and Composer delivery inside this plan.
- **Documentation state:** The consultant completed with thirteen material findings. Forty-four tracked documents are modified, and the plan, findings report, and unified Composer specification are new; no production source or test file is edited. Six of the original thirteen consultant findings and seven later documentation-review findings are closed by owning-document corrections, while eight implementation findings remain open for Stage E.
- **Independent review state:** The first and second FalseFire-style documentation reviews and repository-governance audits returned `FAIL` and drove the recorded corrections. The third FalseFire review returned `PASS`, the third repository-governance audit returned `PASS`, and both reported zero documentation-commit blockers after independently inspecting current bytes and implementation facts.
- **Verification state:** `git diff --check`, specification lifecycle, documentation model and link validation, generated index, AgentSession terminology, Agent Skill reachability, story schema, test governance, the five root-contract tests, and all 146 `@openkit/config-schema` tests pass on the reviewed bytes. The only observed environment note is the existing Node engine warning because the active Node is 22.22.3 while the repository requests 24.18.0.
- **Documentation baseline:** Commit `d3fe95d3261a12153dc37ee4dbc8e857d14d4ceb` (`docs: define composable agent runtime configuration`) contains the forty-seven-file authority-bearing documentation baseline and no production source or test edit. Its staged Biome check and conventional-commit hook passed.
- **Frontier:** The documentation design, review, validation, and commit gates are accepted. This checkpoint is a documentation-only record of that boundary; Stage E may begin only after this record is committed with no production diff.
- **Predicted Next Action:** Begin Stage E from the committed baseline at the existing configuration-schema and Workspace-record seams, keep each implementation slice coherent and tested, and update the eight open findings only from direct implementation evidence. Expected observable: the first contract-derived test fails against current code before its smallest owning implementation lands.

...
Primary owns all Markdown. Cursor `live-builder` owns model metadata and runtime-configuration corrections; `ops-packager` owns release packaging, documentation classification and their focused tests; `app-update-builder` owns the bounded host helper, App API/client/Skill operations and Web update surface. Their exact writable paths are assigned independently; App composition belongs to `app-update-builder`, and runtime configuration belongs to `live-builder`. Cursor `live-deployment` owns authorized host work, while `live-tester` owns the dedicated A2 engineering fixture and its probes. Independent Claude `live-resume-direction` examines the scheme and direction; Cursor `live-review` inspects implementation. A fresh operations Skill Actor uses the extracted archive and normal read-only A2 host tools outside the checkout. No second instance, NanoHost maintenance, new daemon or evaluation database is implied by this checkpoint.

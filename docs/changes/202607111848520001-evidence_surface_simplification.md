...
- 2026-07-17: A deeper preserved-invariant audit found that portable worker manifests, change sets, and automatic review bundles still retained source Turn ids in `worker` evidence refs. Test commit `8266407` exposed all three paths; fix `1f3c59d` routes them through the existing import lineage rewriter and reduces production code by 32 net lines without adding a schema, state, or compatibility path. Current-HEAD release and story gates pass after the grouped audit also corrected two unrelated stale acceptance projections in commits `afa0d5a` and `71f6608`.
...
- Changed-package schema, unit, typecheck, lint, and build gates passed for `@openkit/app-api-schemas`, `@openkit/core-client`, `@openkit/nanocore`, `@openkit/mcp`, and `@openkit/web`; the current NanoCore unit and coverage runs each pass 1,965 tests with 7 environment-gated OpenShell E2E tests skipped.
...
- `CI=true pnpm -w verify:release` passed repository L0-L2 checks, package tests and coverage, builds, 21 NanoCore E2E tests with 1 environment-gated skip, and NanoCore and Web built-artifact smoke tests on the final current HEAD.
- `CI=true pnpm -w test:stories` passed 65 story-runner tests, all five deterministic MCP story runners, and the Web Playwright story; the Goal Mode smoke exposed 98 tools and omitted `openkit.create_evidence_bundle`.

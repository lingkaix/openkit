...
Stop and switch to the isolated OpenKit sidecar relay fallback if OpenShell 0.0.80 cannot resolve the per-package placeholder on the exact REST rules while preserving streaming, cancellation, compression, and runtime hints; do not weaken identity or direct-egress invariants to keep the stock route.
...
- 2026-07-13: The feasibility audit rejected stock OpenShell `inference.local` for complete attribution because the route is gateway-global and strips the per-AEP identity and native headers this contract requires. The accepted landing path is now a per-package OpenShell credential placeholder plus an exact REST policy into OpenKit's internal worker-inference routes, with an isolated OpenKit sidecar only as the stop-rule fallback. The audit also found version drift between the Codex 0.130.0 worker image and 0.144.1 schema snapshot, no current transcript-collection RuntimeEvidence producer, restricted raw-ref leakage through ordinary bundle reads and default exports, non-strict evidence replay idempotency, and under-specified normalized-index import reminting; the execution plan now makes each a testable owner-specific requirement.
- 2026-07-13: The Phase 1 foundation now pins the worker image to Codex 0.144.1 and validates it against the vendored schema, rejects OpenShell versions outside the 0.0.80 compatibility range before capability claim, snapshots exact REST rules plus generic-provider placeholder and delete surfaces, validates relay-required AEP shape, preserves immutable provider/model selection, and projects relay-required AEPs to the NanoCore worker-inference base URL with placeholder credential visibility, exact Codex POST rules, and no direct sandbox network, credential, provider-backed MCP, vault, or provider attachments. The backend per-package provider lifecycle, token-only WorkerControlGateway binding and restart hydration, internal worker-inference routes and transport, worker-shim token isolation, and executable OpenShell 0.0.80 probe remain pending; no provenance completeness capability or product correlation field is enabled.
- 2026-07-13: The per-package relay provider lifecycle and worker token isolation landed through `848d68e`, `1af05dd`, and `f2e98ad`. Relay-required materialization now creates one deterministic transient generic provider with the short-lived worker token, renders only the two exact Codex POST paths for the two pinned Codex binaries, suppresses host Codex auth/config uploads and extra direct egress, rejects backend-private credential injection, validates the connected gateway version, prevents concurrent duplicate materialization, revokes rotated and torn-down WorkerControl sessions, and performs bounded idempotent provider and sandbox cleanup across failure and retry paths. Canonical AEP validation now permits exactly the control-local, capability-local, and worker-inference rules, while NanoCore revalidates the package at the backend boundary. The relay capability remains disabled by default until a same-target OpenShell 0.0.80 executable probe proves placeholder injection, transport, streaming, cancellation, compression, and runtime-hint preservation.
...
- 2026-07-13: The remaining same-target gate is blocked by external prerequisites rather than reported as skipped success. The exact OpenShell 0.0.80 CLI is available through `OPENKIT_OPENSHELL_BINARY`, and the worker image is now built, but the connected local gateway is 0.0.63, the configured `a1-openkit` 0.0.80 target tunnel is disconnected, and no authorized real Codex quota run or authoritative pinned multi-agent Responses fixture is available. The required proof must run the real Codex root-and-two-child path through the package-scoped provider placeholder and exact process/network policy, preserve transport and cache telemetry, and complete teardown on the same 0.0.80 target. Production relay and provenance capability advertisement remains off until that proof passes.
- 2026-07-13: The external prerequisites are now available. A disposable A1 target runs OpenShell 0.0.80, the arm64 `openkit/worker-codex:dev` image was built and smoked on A1 with Node 24.18.0 and Codex 0.144.1, the local OpenShell 0.0.80 CLI reaches that exact target through the dedicated tunnel, and one quota-authorized real Goal completed through the package-scoped relay with the exact proof file, nine linked usage rows, three capability calls, three audit events, two evidence bundles, one runtime-evidence row, an accepted workspace review, an unchanged repository HEAD, and no correlated transient provider or sandbox remaining. The run exposed three false identity assumptions in workspace cleanup and recovery: sandbox, agent-session, and package ids had been overloaded as `workerSessionId`. Commits `d53b0e4`, `0b964f2`, `485277e`, `f32c942`, `16b66e0`, `e9df98f`, `ff9824a`, and `ea31f8e` made package lineage mandatory on materializations and backend handles, removed inferred input/materialization records, reminted package lineage on import, correlated terminal, teardown, maintenance, and restart recovery by package, and preserved real sandbox ids only as backend session identity. The exact same-target transport probe and the real root-plus-two-child Task acceptance remain open, so this plan is active rather than blocked or complete.

## Current Status

- The design is accepted in `docs/specs/20260711-worker_runtime_subagent_provenance.md`.
- Related active specs describe the deterministic implementation as complete and keep the same-target executable gate and production capability activation explicitly pending.
- Phase 1 deterministic relay behavior is complete: version and schema alignment, exact relay-only policy, package-scoped provider lifecycle, token rotation and revocation, direct credential and egress suppression, worker-shim isolation, live lease authentication, the two internal routes, bounded decoding, compression, cancellation, Codex continuity, runtime hints, and terminal capability recording are implemented. The real same-target transport proof and production activation remain pending.
...
- Phase 6 deterministic conformance, active-spec projection, worker-image packaging, exact process-policy alignment, and the skip-aware real acceptance asset are complete. One real Goal has passed on the pinned A1 target; the exact transport probe and real root-plus-two-child Task remain pending.
...
- Current OpenShell backend materialization creates and deletes the transient generic provider, installs the package token placeholder, removes host credentials and extra direct-provider egress for trusted inference packages, and fails closed on incompatible or unknown gateway versions. The production factory enables trusted inference and runtime provenance capability declarations; this is configured support rather than proof, and the same-target L6 run remains the acceptance gate.

## Final Implementation Summary

The deterministic implementation now preserves a bounded Codex runtime forest as restricted evidence, promotes only product-safe origin indexes and correlation refs, keeps runtime children out of canonical Core entities, authenticates worker inference through the AEP package and live lease, isolates worker cache routing from outer OpenKit lineage, reconciles every package-scoped worker-inference call at turn end, and retains portable evidence through workspace export/import without copying raw runtime files. Protocol, App API, OpenAPI, Core Client, MCP, NanoCore storage, worker shim, OpenShell policy, the Codex worker image, active specs, local guides, deterministic conformance, and a skip-aware real acceptance story are aligned. Production relay and provenance capabilities remain intentionally disabled.
...
- Open acceptance: the quota-gated `pnpm -w test:stories:real-task-mode` has not yet executed the real Codex 0.144.1 root-and-two-child workload against the connected OpenShell 0.0.80 A1 target. The exact transport probe, cache telemetry, complete provenance reconciliation, and correlated provider, sandbox, handle, lease, and capacity teardown must still pass before this record becomes verified.

...
- Phase 6 deterministic conformance, active-spec projection, worker-image packaging, exact process-policy alignment, and the skip-aware real acceptance asset are complete. The real same-target story has not run.
...
- Blocked acceptance: the quota-gated `pnpm -w test:stories:real-task-mode` has not executed against one connected OpenShell 0.0.80 target with the real Codex 0.144.1 root-and-two-child workload, provider placeholder, exact policy, transport, cache telemetry, and teardown. This record must remain `blocked`, and production capabilities must remain disabled, until that evidence exists.

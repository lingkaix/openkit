...
- **Intent epoch:** 1.
- **Current facts:** The first registered Reviewer rejected six local defects, then closed the FD cleanup, session-only projection, tar-end alignment, and workflow-policy findings but rejected the second candidate because a valid export could still contain an unrepresentable USTAR basename, CLI cleanup retained a check-then-unlink race, and the archive-order test sorted away its oracle. Those implementation findings are independently accepted after strict USTAR representability became a shared owner contract, CLI failure stopped unlinking pathnames, and the order oracle inspected emitted order directly. The first Auditor pass then blocked stale Current Implementation and storage-owner projections plus a missing standalone offline-verifier negative regression; the corrected documents and regression are now independently accepted. The registered Verifier returned `Continue`, the registered Reviewer returned `ACCEPT`, and the registered Auditor returned `ACCEPT`. Focused NanoCore and OpenAPI tests pass 53/53, Skill interface tests pass 16/16, Core Client tests remain 76/76, release-workflow tests pass 6/6, NanoCore typecheck and build pass, Skill validation passes, documentation lifecycle and index checks pass, isolated portability e2e passes 3/3, and the complete local source/target CLI runner passes with shared archive SHA-256 `6ca8587132902c2fb5d060c96303841169d12e0b449954299e9a00df93bc2826`. Every check was process-local or used temporary files and test-owned random-port child processes; the current App Server, host services, Docker, Herdr, and machine were untouched.
- **Current method:** Commit and push the independently accepted coherent R008 slice, then manually dispatch the existing `nano-core-e2e` workflow gate to obtain the required two-fresh-runner artifact evidence. Local validation and Git hooks run directly on the host without `OPENKIT_TEST_USE_IMAGE`; they must not start, stop, restart, or replace the current App Server, a host service, or a container.
- **Frontier:** Local behavior, evidence, owner documentation, direction, implementation review, and governance audit agree; only the required fresh-runner artifact proof remains before R008 closure.
- **Unknowns:** The first remote workflow run may expose a job-container path or artifact-owner difference that cannot be reproduced by the same-host runner; R009 and R010 remain untouched implementation slices.
- **Next action:** Commit the accepted R008 slice, push the branch, manually dispatch `CI` with gate `nano-core-e2e`, and inspect the source and target job outputs plus shared artifact SHA.
- **Predicted observation:** Both fresh-runner jobs should report the same original archive SHA and the target should pass semantic, complete-history, knowledge, repository, Vault, mutation, and re-export checks without comparing the re-export digest.

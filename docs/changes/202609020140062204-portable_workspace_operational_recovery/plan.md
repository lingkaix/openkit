...
- **Intent epoch:** 1.
- **Current facts:** Plan baseline `e9aed2d` passed independent acceptance and repository hooks. The same registered independent Reviewer inspected the actual four-spec candidate, required corrections for legal-hold ownership, recovery-attempt identity, concurrent deletion requests, foreign server-handle authorization, explicit confirmation, recovery expiry, and archive dry-run wording, then accepted the corrected bytes after the deletion admission and central idempotency rules were made exact. Focused document validation passes for spec lifecycle, the 210-document model, documentation index, root contract 5/5, and `git diff --check`; no product code or regression has changed.
- **Current method:** Commit the accepted owner baseline with normal repository hooks, then begin R008 with the smallest focused RED regression for the missing archive route and unrelated server-managed-handle rejection.
- **Frontier:** The four owner specifications are accepted and ready for their durable baseline commit. Production code remains unchanged and must follow these accepted bytes.
- **Unknowns:** The smallest complete App/runtime mutation-publisher coverage remains an implementation probe that may change file ownership but may not weaken the accepted fence. The direct package placement needed for the accepted archive stream must follow the existing dependency owner rather than rely on an undeclared transitive import.
- **Next action:** Commit the accepted owner documentation, update this checkpoint with the commit identity, then derive the R008 focused RED test without starting or lifecycle-managing an existing process.
- **Predicted observation:** Normal hooks should accept the documentation commit; the first R008 regression should fail only because the accepted archive route and strict unrelated-handle denial are absent.

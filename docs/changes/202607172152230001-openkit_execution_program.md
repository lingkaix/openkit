...
Current position: WP-0 and WP-1 are complete. WP-2 is active; its bounded G03 preamble, both deletion-first security blockers, and focused adapter-contract precision are complete, and test-first manifest/AEP plus generic-shim work is next.
...
- `DOC-DRIFT` — S16's current implementation boundary still says the deleted generic Artifact metadata `PATCH` remains — WP-4.

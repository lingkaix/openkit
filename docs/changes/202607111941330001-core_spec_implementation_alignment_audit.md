...
- Use `docs/core/foundation.md` as the highest-level OpenKit doctrine owner for human authority, system responsibilities, and cross-aspect invariants.
...
- Audit all current Core aspect documents, including Foundation and Metering as separate owners.
- Audit all active root specifications against Core authority, implementation, tests, current use, and related change plans.
- Use archived specifications only as historical evidence, replacement evidence, or prior-art context; do not treat them as current implementation contracts.
...
The baseline was refreshed from the current worktree on 2026-07-16 after the Worker Runtime Provenance handoff and Worker Adapter specification additions.
...
- The active specification root contains 66 tracked specifications; 62 are Accepted and 4 are Draft.
- Current implementation alignment is 24 Implemented, 32 Partial, 1 Diverged, and 9 Not Started. The Diverged storage-layout contract and every Partial or Not Started contract remain explicit audit work.
...
- The active Not Started set includes the accepted Web rebuild, Agent Skill Interface, and three Worker Adapter designs plus the four Draft specifications for self-improvement, evaluation harness, recurring scheduler triggers, and Skill catalog versioning.
- The archive contains 58 lifecycle-classified specifications. They are evidence, not active authority.
...
- Worker Runtime Sub-Agent Provenance is implemented and handed off; S33 remains pending independent audit.
...
| C02 | [Work Model](../core/work-model.md) | Protocol work records, NanoCore stores and route flows, Action Center, Chat/Task/Goal/Quick Chat, Agent Skill CLI and Web projections | Promote the four Human Attention categories and Quick Chat workspace semantics; preserve human acceptance; move exact item and Action Center shapes downward; test whether real workflows remain expressible without hidden product state. Resolve C01-F02 by reconciling Artifact creation with the accepted item-backed lineage invariant. | Completed 2026-07-16: authority and Artifact lineage aligned; unsupported steering fails closed; real delivery routed to S05, S13, S16, and S39 |
...
| C06 | [Agent Workflow](../core/agent-workflow.md) | Internal agents, Chat/Task/Goal mode modules, goal runtime modules, Action Center, workspace review and evidence | Own coordination, acceptance and workspace-change governance; retain roadmap-backed graph and recipe direction; prove accepted Goal delegation boundaries reach the real worker turn; test bounded steps, handoffs, gates, refinement, stop and evidence behavior. | Pending |
...
| S09 | [Web Product Surface Projection](../specs/20260628-web_product_surface_projection.md), Accepted / Partial | Current Web app, Core Client, App API schemas, NanoCore APIs, Web tests and README | Resolve C00-F07 by replacing browser-local Goal Reject and Revise decisions with the existing Core Client mutation; verify only durable projection rules remain, move valid guidance to Web README, and supersede only after the rebuild contract lands and no independent design authority remains. | Pending |
...
| S14 | [Quick Chat Workspace](../specs/20260709-quick_chat_workspace.md), Accepted / Partial | Quick Chat workspace and route modules, internal agent, app schemas, storage, MCP/Web projections and tests | Verify it remains a real durable workspace while prohibiting workers, repositories and external side effects; repair the missing change link and test guard bypasses. | Pending |
...
| S64 | [Codex Worker Adapter](../specs/20260716-codex_worker_adapter.md), Accepted / Not Started | Shared worker harness contract, Codex adapter and image profile, native-output/provenance fixtures and image smoke | Verify that all Codex command, output and native-provenance knowledge is removable with one adapter and image; reuse shared supervision and control, advertise only proven capabilities, and add no NanoCore runtime branch. | Pending |
| S65 | [OpenCode Worker Adapter](../specs/20260716-opencode_worker_adapter.md), Accepted / Not Started | Shared worker harness contract, OpenCode adapter and image profile, pinned native JSON fixtures and image smoke | Verify one-shot native translation, bounded output parsing, safe resource and credential supply, and clean adapter deletion without an OpenCode server becoming a second control plane. | Pending |
| S66 | [Pi Worker Adapter](../specs/20260716-pi_worker_adapter.md), Accepted / Not Started | Shared worker harness contract, Pi adapter and image profile, pinned native JSON fixtures and image smoke | Use Pi only to falsify accidental Codex/OpenCode coupling; verify one adapter/profile/image, bounded resource discovery, no RPC control-plane expansion, and no native schema leakage into NanoCore or worker protocol. | Pending |

### E. Storage, Workspace Data, Synchronization, Git, And Portability

| ID | Specification and current alignment | Primary implementation and test surfaces | Required review focus | State |
| --- | --- | --- | --- | --- |
| S46 | [Storage Layout And Record Ownership](../specs/20260703-storage_layout_record_ownership.md), Accepted / Diverged | NanoCore DB/migrations/schema, filesystem layout, store, backups, workspace export/import and tests | Reconcile the accepted owner-independent Workspace root with the current user-nested implementation, then verify one writer and source of truth per record family, filesystem/SQLite atomicity, rebuildable indexes, lock behavior, deletion and current file-map accuracy. | Pending |
...
| S60 | [Knowledge Store Governance Rules](../specs/20260702-knowledge_store_governance_rules.md), Accepted / Partial | OKF validator/store, Knowledge Manager, observations/claims/conflicts/proposals, source refs and tests | Resolve the C00-F09 governance boundary for recoverable accepted-proposal application and exact reversal lineage; verify accepted versus proposed knowledge, human override, conflicts, stale detection, sensitivity, path validation, source repair and no automatic promotion of generated claims. | Pending |
| S61 | [Knowledge Store Implementation](../specs/20260703-knowledge_store_implementation.md), Accepted / Partial | OKF files, search/index rebuild, retrieval traces, context packages, import/export, App API and future worker capability routes | Resolve the C00-F09 persistence and operation mechanics for proposal-to-entry revision lineage; verify file-first truth, deterministic indexes, corrupt and concurrent edits, retrieval quality, traceability, context materialization, portability and operational performance on realistic stores. | Pending |
| S63 | [Single-Deployment Multi-User Workspace System](../specs/20260715-multi_user_workspace_system.md), Accepted / Partial | Better Auth identity, Core workspace registry and membership, owner-independent storage, policy mapping, auth resolver, audit, concurrency, App API, CLI/Skill and Web | Verify one deployment as one trust domain, Workspace as neither tenant nor organization, exactly one owner, fixed owner/editor/viewer access, fail-closed request resolution, actor attribution, narrow shared-write CAS, one-way storage migration and no second RBAC engine. | Pending |
...
- Reconcile the active-spec index with all current root specifications without overwriting concurrent spec authoring.
...
- Treat the completed NanoCore Maintainability Recovery and verified Evidence Surface Simplification slices as settled handoffs; reopen their code only for an evidence-backed finding in the owning ledger row.
...
Exit criteria: all active specifications have evidence-backed authority and implementation alignment, every current mismatch is resolved or explicitly owned by a deferred plan, and no specification claims implementation based only on scaffolding or deterministic fixtures.
...
- Real-worker tracing proved the current Goal launch passes only its objective and has no generic immutable worker Context Package trace. C02 therefore removed the speculative acceptance, Goal lineage, recovery, projection, and success-test paths instead of adding a receipt, settlement, or recovery workflow; real delivery remains S05, S13, S16, and S39 work.
...
Active. The roadmap preflight, durable audit ledger, Phase 0 baseline refresh, C00 Foundation authority audit, C01 Core Concepts authority and exact Turn assignment audit, C02 Work Model authority repair, Metering current-state alignment, worker-capability truth reset, scheduler hardening, Codex provider hardening, workspace-review base correction, real Goal L6 slice, and S26 disposable OpenShell Cell audit are implemented or in progress as stated in their ledger rows. The remaining Core and specification rows stay open for sequential design, code, persistence, failure-path, security, and L0-L6 review.

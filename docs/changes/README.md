...
An optional `proposal.md` retains the reasons, alternatives, assumptions, evidence, and material Consultant objections behind the chosen scheme. Explore under `temp/` first, curate worthwhile rationale when entering implementation, and link it from `plan.md`. Update only material scheme changes with their reasons, using Git history. It has no design authority or separate lifecycle and becomes historical at close; lasting design and rationale belong to Core or specifications. A short plan or complete Draft specification avoids duplication.

...
|---|---|---|---|---|
| 0 | Preserve the completed Workspace-recovery cluster, make the host requirement honest, let an Agent stand up a deployment, and prove truthful interrupted outcomes | R008, R009, R010, R005, R110, R109 | None — B1 design accepted; R008, R009, and R010 are complete; R005 is unblocked | R008, R009, and R010 are complete and remain recorded in this pathway. R110 and R109 remove the single-host bottleneck that stalled this Phase. R005 is correctness rather than assurance and shares R110's one reboot campaign. |
| 1 | Recurring work runs on a schedule with current authority checked before every run | R092 | None — B2 design accepted | Small, and its value accrues with elapsed time rather than at delivery: the earlier it lands, the more unattended real-use hours it generates. |
| 2 | Workers can use supported third-party MCP servers through the Gateway | R058 | None | Best value per unit cost on the list: one governed path imports an existing tool ecosystem, and its owner is already Accepted. Scope is the Workspace-declared catalog its owner defines; marketplace or registry discovery stays deferred. Its implementation may proceed independently of deployment and recurring work. |
...
| 7 | Capture real-use Knowledge with source lineage from first use | R071, R072 | None; B6 resolved by notebook design | Retroactive capture is impossible, so the recording half cannot wait. The improvement loop can. |
...
- **B1 — Deployment host requirements accepted.** [Deployment Host Requirements](specs/20260909-deployment_host_requirements.md) is `Accepted` / `Not Started`. It separates NanoCore source/image and NanoHost roles, supports Linux NanoHost on x86_64 and arm64, keeps 2 cores/8 GiB/30 GiB as empirical guidance, and defines a bounded archive-local static requirement check with exact profile/product/archive/machine binding. NanoHost, release, deployment and verification owners retain installation and real-runtime qualification, including the distinction between supported architectures and current arm64-only evidence. The design blocker is cleared; R109/R110 still own implementation, amd64 packaging and fresh qualification. Historical R002 evidence is not proof of the new profile, another architecture, or runtime readiness.
- **B2 — Recurring trigger contract accepted.** [Scheduler Recurring Triggers](specs/20260711-scheduler_recurring_event_triggers.md) is `Accepted` / `Not Started`: one five-second soft scan, three committed admission attempts, ten-minute pre-admission expiry, durable exact history and restart reconciliation, one waiting occurrence per schedule, and current authority before admission and dispatch. Storage, audit, Policy, identity, portability and Web projections are aligned. The design blocker is cleared; R092 remains unimplemented, and external event/calendar scheduling remains deferred.
- **B3 — Separate Generative Kernel and Generative UI contracts.** [Generative Kernel Data And Operations](specs/20260908-generative_kernel_data_operations.md) and [Generative UI Interaction](specs/20260908-generative_ui_interaction.md) are `Accepted` / `Partial` for the initial MVP: PocketBase-inspired schema/general data operations in per-app SQLite, selected Worker access, and native A2UI presentation with immutable Item-linked history, explicit refresh and conditional form updates. Storage, backup/portability, audit, Policy and Worker capability owners now define this slice's required extensions; the initial design blocker is cleared and the [MVP change plan](changes/202609090046400001-generative_apps_mvp/plan.md) owns execution. This does not complete R096–R098 or enable the full design: richer operations, app packages, fixed Plugin operations, independent saved views and the HTML delegate retain their feature-specific contract and implementation prerequisites. R098 remains the composed Kernel-backed acceptance path; UI over existing sources requires no Kernel app. Web retains rendering-stack and publication authority, and neither contract acceptance nor a fixture makes a catalog operation release-ready. Begin on A2UI v0.9 and target eventual v1.0 compatibility without waiting for an upstream release. This clarifies dependencies without reordering the execution pathway.
- **B4 — Personal Assistant management.** Design is owned by the Assistant specification's finite private management entry: three fixed Core-backed tools, explicit automatic/proposal/approval/prohibited classes, current-user scope checks, exact confirmed configuration commands, truthful reload/recovery and private dialogue with eligible effect audit. `20260909-thread_visibility_and_sharing.md` owns its audience. No independent administration Agent is required. R035 implementation remains pending.
- **B5 — `docs/specs/20260711-skill_catalog_versioning_pinning.md` is Accepted.** Remaining work is the App projection and delivery proof owned by R108. The specification settled immutable version identity, current-version selection, Workspace pinning, reviewed promotion, delivery verification, rollback, and per-Worker assignment. R108 additionally requires the App API operations and the Catalog screen disposition in `docs/specs/20260628-web_product_surface_projection.md`. Blocks R076 and R108 until those projections are live.
- **B6 — Resolved: generated Knowledge maintenance.** S60/S61 and `20260909-knowledge_notebook_editing.md` own fixed-base multi-page edits, merge/split/reference repair, supersede/archive, current-base restoration, Git history, explicit maintenance delegation and exact required human decisions. Scoped learning owns capture and privacy. R071/R072 may proceed to implementation; their unchecked roadmap state remains implementation work, not an unresolved design decision.

Steps 2 and 5 carry no blocker, and so does R071 in step 7. Their owners are Accepted and cover those items: `docs/specs/20260704-worker_mcp_tool_supply.md`; `docs/specs/20260715-multi_user_workspace_system.md` with the two Web specifications; and `docs/core/knowledge.md` with the Knowledge Store specifications, which require explicit current publication authority rather than model self-authorization.
...
- **Workspace membership baseline.** The target in `docs/core/permissions.md` and `docs/specs/20260715-multi_user_workspace_system.md` gives every active member the full Workspace and Light App operation set through the existing Policy Kernel. The current owner/editor/viewer implementation needs a coherent cutover; this is not a claim of changed runtime. Credentials, Vault, confidentiality, data integrity, exact human-gate principals, and lifecycle/recovery predicates remain strict. No gate-takeover workflow or deployment-admin content bypass is authorized.

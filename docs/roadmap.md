...
### Single-deployment multi-user Workspaces

The vision (§2) targets three-to-five-person expert teams sharing one canonical Workspace inside one NanoCore deployment. The accepted design is `docs/specs/20260715-multi_user_workspace_system.md`: one deployment is one trust domain rather than a tenant host; Workspaces use owner/editor/viewer policy projection, owner-independent storage, registered-user invitations, actor attribution, and narrow shared-write concurrency. The identity and policy foundations exist, but the top-level Workspace migration, centralized access resolver, sharing lifecycle, concurrency controls, and first-party product projections remain planned implementation work under `docs/changes/202607160021540001-contract_stability_multi_user_workspaces.md`. Multi-tenancy, organizations, Federation, P2P, real-time coediting, and per-member hidden Workspace record families remain outside this milestone.

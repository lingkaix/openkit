...
- **B6 — Generated Knowledge maintenance authorization.** Exact single-page create/replace, monotonic revisions, prior-byte retention, application ownership and explicit forgetting are now owned by S60/S61 and `20260909-personal_memory_and_knowledge_learning.md`. That bounded learning slice can proceed to implementation. Multi-page merge/split, generated supersede/archive/delete and their reference-repair semantics remain deferred, so the full R072 scope is not yet design-clear. R071 remains unblocked.

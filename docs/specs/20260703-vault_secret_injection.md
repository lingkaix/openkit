...
- The current Agent Manifest path names concrete grants and has no reusable `requirementId` plus `workspace.jsonc` binding composition. The production worker-turn executor resolves credential arrays, but the current NanoHost materialization call drops them before the backend effect. Both are implementation divergences; a receipt created before actual sink completion is not proof that the worker received a secret.

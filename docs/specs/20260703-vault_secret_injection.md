...
A Server Agent Manifest that directly names a `VaultGrant` may name only a Server-scope grant whose reference is also Server-scoped. A reusable Manifest declares a stable credential `requirementId` without a grant identity, and `workspace.jsonc` binds that requirement to one Workspace-scope target-issued grant during authored composition. The same Manifest may therefore resolve to different secret material, accounts, and permissions in different Workspaces, while a missing required binding fails before worker launch. A grant from another Workspace, an unbound requirement, or a matching string in a different scope never becomes usable by reference alone.

User preference does not rewrite a Workspace credential binding. A user-specific credential needs its own User-scope reference and an explicit cross-scope grant accepted by the owning credential declaration contract; personal preference alone creates no secret authority.
...
- Agent Manifests may declare reusable `requirementId` credentials, and each Workspace Agent binding maps them to that Workspace's concrete VaultGrant. The production worker-turn executor carries resolved runtime-file and runtime-env credential arrays through NanoHost to backend-private sinks; required missing bindings and sink failures fail closed, and the worker resolver writes a receipt only after successful sink completion.
...
- OpenShell worker materialization no longer uploads host runtime configuration directly. Runtime-file and runtime-env declarations resolve through the shared audited worker resolver, materialize through NanoHost backend-private sinks, and create the receipt after successful sink completion. Sandbox-provider declarations remain fail-closed before provider or sandbox effects because the AEP does not yet carry the exact Providers v2 endpoint and binary policy. Codex auth JSON has a durable runtime-file path through `grant_codex_auth_json` and `vault_codex_auth_json`, and host-side Git push has a gateway-only path through `git.vaultGrantRef`; the host Git push path still creates its receipt before the external Git process completes and remains a separate implementation divergence. The current reference-revocation cascade also marks every active receipt `stale-session` without checking whether its visibility path could retain material. Neither residual projection is authority, completed downstream-effect evidence, or proof of retained material. Secret values remain absent from AEP snapshots, public materialization records, exports, logs, and product records.
...
- Replacing material behind an already attached OpenShell sandbox-provider may take effect immediately only through the backend's proved dynamic replacement operation. Adding a new credential or changing runtime-file or runtime-env material never mutates the active process: it enters a later AEP, and the current per-Turn Codex path reads it when the next child starts and resumes the same Thread and AgentSession through the existing adapter handle. An adapter that retains a native process between Turns must refuse reuse until its own accepted replacement behavior completes. No special restart command, replacement Thread, or mutable AEP is created.
- A Sandbox is the containment boundary. Different AgentSessions in one Sandbox may receive different resolved credential declarations and process-local projections, but this contract creates no stronger mandatory intra-Sandbox isolation; users who require separate trust or accounts that must not share one compromise domain select separate Sandboxes through the ordinary compatibility and placement owners.
...
- target path, environment variable name, token handle, or credential-injection detail
- policy decision id
- approval id
- revocation status

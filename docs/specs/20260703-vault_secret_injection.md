...
- OpenShell worker materialization no longer uploads host runtime configuration directly. GitHub sandbox-provider resolution creates the durable `VaultGrant`, logical AEP attachment, `InjectionPlan`, `InjectionReceipt`, and linked non-secret `VaultUse` lineage, but the current OpenShell backend rejects the backend-private provider credential before provider or sandbox effects because the AEP does not yet carry the exact Providers v2 endpoint and binary policy. Codex auth JSON has a durable runtime-file path: `grant_codex_auth_json` and `vault_codex_auth_json` create a runtime-file `InjectionPlan`, `InjectionReceipt`, linked non-secret `VaultUse`, and backend-private upload to `/sandbox/.codex/auth.json` without putting the credential value in the AEP snapshot or public materialization record. `POST /api/app/vault/bootstrap/codex-auth-json` can create that server-owned reference and grant from base64 request content without echoing the submitted auth JSON. Workspace vault references imported from portable exports are publicly discoverable through redacted App API, Core Client, Web, and the unified Skill/CLI `vault.reference-list` operation before being rebound through base64 request-only material. Host-side Git push has a gateway-only path: linked repository Git config stores only `git.vaultGrantRef`, the approved push route resolves that grant after policy/preflight checks, records an `InjectionPlan`, `InjectionReceipt`, workspace-scoped `VaultUse`, and linked audit event, then passes GitHub token material only as child-process env to the fixed Git push command. If that gateway-only material cannot be resolved, the Git push executor records a redacted `auth-failed` terminal `GitPushRecord` and does not invoke the child process.

Durable grant policy engine expansion, cross-scope grant review beyond metadata identity validation, a policy-equivalent non-transient OpenShell provider attachment path, and audited backend resolution wiring for future credential-use classes remain incomplete.

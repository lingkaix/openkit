...
After restart, the same owner classifier may finish only deterministic missing closeout writes fixed by the exact stored Gate response and validated outer Chat tuple. It preserves every incomplete or contradictory checkpoint as `recovery_required`, never replays worker execution, and deletes the checkpoint only after the required receipts and cleanup owners are durable. Direct Task Gate responses remain bound to their exact `task.start` receipt, Goal Gate responses remain bound to their exact `goal.step` and Goal Task tuple, and Chat clarification responses retain their existing same-Turn Chat continuation contract; none may qualify through this Chat-subordinate worker branch.
...
Chat-subordinate user-input Gate closeout and the required receiving-Thread handoff lineage are not implemented yet. The current shared closeout accepts only direct `task.start` or Goal `goal.step` ownership and the current Chat handoff still derives the worker Turn from the originating Thread, so this specification remains Partial until the outer `chat.start` validation and new-Thread branch above are implemented and verified without a nested Task receipt or copied worker history.
...
- L3: a Chat-subordinate non-secret user-input Gate closes only under the exact actor, Workspace, Thread, request hash, deterministic `chat.start.task` Turn, and outer `task-handoff` receipt tuple; exact response replay duplicates no Item, cleanup, release, receipt, or checkpoint deletion, changed answers conflict, owner mismatch fails closed, restart never resumes the worker, and no nested `task.start` receipt exists.

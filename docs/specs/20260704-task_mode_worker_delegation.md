...
A direct Task Gate response closes the existing envelope and never calls the adapter to continue worker execution. Gate creation immediately changes the Product Turn to `awaiting_human`, but an AEP-backed exact Gate becomes acknowledged, actionable, and `waiting_for_user` only after its accepted `ask_user` worker-control record, existing backend cleanup, Workspace handoff, and source AgentSession `suspended` projection agree; the delivered Codex MCP path obtains that worker outcome through the typed session-continuity `human-gate` stop and never through a bounded-turn command or raw fallback. An enqueue-unproved or contradictory Gate is inspect-only `recovery_required`, keeps its checkpoint at its prior non-waiting stage, and rejects the response command while existing final-status, scheduler, backend cleanup, reconnect, and fencing owners retain the attempt. The exact accepted response command must own one matching `user-input-response` or `approval-decision` Item, clear the Turn Gate, change the same Product Turn to `completed` and source AgentSession to terminal `closed` for an answer or granted approval, or change the Product Turn and source AgentSession to `interrupted` for a denied approval, move the checkpoint to `completed/completed` or `aborted/aborted` respectively, retain the original `ask_user` request plus response or decision pair in checkpoint evidence, and close the exact scheduler lease and capacity before publishing its receipt. Replay projects a completed envelope with that Gate evidence as Task state `blocked`, with no completion payload, and an aborted denied envelope as `cancelled`; neither claims that the worker itself later returned `completed` or `aborted`. The response receipt is durable before checkpoint deletion, and exact replay may finish only deterministic missing writes under that tuple. The in-process adapter compromise permits this closeout only when the exact AgentSession has no AEP snapshot, no backend session, and no worker-control record. Until the AEP-backed owners can prove complete closeout, the command remains `recovery_required`. NanoCore does not synthesize those records, resume the worker AgentSession, or add a Gate settlement owner; later execution requires a new `task.start`, Turn, lease, and successor AgentSession.

Only that exact Core Gate tuple may project a Task as `awaiting-human`. An AEP accepted raw `blocked/ask_user` final status without an exact Core Gate uses S05's bounded unavailable-transport fallback instead: NanoCore preserves the accepted row, finishes backend cleanup, interrupts the Product Turn with `worker_human_gate_unavailable` and its AgentSession with the same bounded message, leaves the `preparing` checkpoint with null stop reason and Worker Backend Session id, releases only the exact matching lease as `released + needs-evidence`, and returns `409 recovery_required`. Task Mode MUST NOT synthesize a Gate, publish a Gate receipt, project `awaiting-human`, offer an answer or approval action, retry, or resume the worker from that tuple.

For Chat-to-Task handoff, the initiating Chat or clarification-response command remains the only ledger owner. Task Mode receives that immutable outer scope and causation, creates one deterministic receiving Thread in the same Workspace with the originating Chat Thread as `parentThreadId`, derives the downstream Task Turn and tuple inside that receiving Thread, and does not publish a nested `task.start` receipt. The originating Chat Thread retains one handoff reference Item and no worker transcript. Missing, stale, duplicated, or contradictory originating Thread, receiving Thread, parent lineage, reference Item, Turn, or receipt state returns `recovery_required`; retry reconstructs no worker history and creates no replacement Thread or Turn. For Task-to-Goal escalation, the initiating Task command likewise owns the receipt, Goal Mode creates one deterministic receiving Goal Main Thread in the same Workspace with the Task Thread as `parentThreadId`, creates the Goal objective Turn and Item in that receiving Thread, leaves one escalation reference Item in the Task Thread, and publishes no nested `goal.start` receipt. The Goal Main Thread references selected Task history and evidence without copying or rewriting the original attempt. Direct public Task and Goal requests continue to own their respective command records.

#### Chat-subordinate worker user-input Gate closeout

A Chat-subordinate Task worker may pause only on an exact Core user-input Gate whose questions all have `isSecret=false`. This is non-secret elicitation, not authorization: the branch MUST NOT accept secret answers, create or require an Approval, policy action, or `PermissionDecision`, or add another Gate kind, response shape, receipt kind, state, or settlement owner.

The existing `turn.input.submit` command is the sole response command. The initiating outer `conversation.submit` command remains the sole delegation-ledger owner, while the ordinary `turn.input.submit` receipt owns only the response command; neither live closeout nor recovery may publish or require a nested `task.start` receipt. Before any response Item or closeout mutation, NanoCore MUST validate the authenticated response actor and Workspace against the checkpoint-bound outer conversation command scope; validate the originating Thread, deterministic receiving Task Thread, and their `parentThreadId` lineage; validate the checkpoint request id and canonical caller-input hash against that exact `conversation.submit` receipt; recompute the downstream Turn from that immutable outer command for the actor, Workspace, receiving Thread, and request id; and require receipt metadata `resultKind=task-handoff`, `status=202`, and downstream `{ kind: "task", threadId, turnId }` naming that exact Thread and Turn.

After that validation, closeout reuses the direct Task user-input path: one matching `user-input-response` Item on the waiting Turn, Gate removal, terminalization of the same Turn and AgentSession, `completed/completed` checkpoint terminalization with the original request and exact structured answer map retained in evidence, accepted AEP final status and backend workspace-handoff cleanup when the AgentSession is AEP-backed, exact scheduler lease and capacity release, `turn.input.submit` receipt publication, and checkpoint deletion only after both the outer `conversation.submit` receipt and response receipt are durable. The closed envelope projects as blocked Task work with no completion payload. It MUST NOT resume the worker, invoke an adapter continuation, create another Turn, or mutate the outer conversation receipt.

`turn.input.submit` receipt lookup precedes owner mutation. The same response command identity and exact answer map replays the closed Turn without another response Item, terminalization, cleanup, capacity release, or checkpoint deletion; reusing that identity with a changed answer map returns `409 idempotency_key_conflict`. Secret input returns `400 secret_input_not_supported` before a response Item or command write. A missing outer receipt; wrong actor, Workspace, Thread, request id, input hash, derived Turn, result kind, status, downstream kind, or downstream Turn; a missing or contradictory Gate, Item, checkpoint, AgentSession, AEP, backend, workspace-handoff, lease, capacity, or evidence owner; or a missing response receipt after otherwise non-deterministic or contradictory closeout writes returns `409 recovery_required` without worker resume, nested receipt publication, replacement work, or inferred repair.

After restart, the same owner classifier may finish only deterministic missing closeout writes fixed by the exact stored Gate response and validated outer conversation tuple. It preserves every incomplete or contradictory checkpoint as `recovery_required`, never replays worker execution, and deletes the checkpoint only after the required receipts and cleanup owners are durable. Direct Task Gate responses remain bound to their exact `task.start` receipt, Goal Gate responses remain bound to their exact `goal.step` and Goal Task tuple, and Assistant clarification responses retain their existing same-Turn continuation contract; none may qualify through this conversation-subordinate worker branch.
...
| `awaiting-human` | The Turn is `awaiting_human` on its exact user-input or approval gate and the checkpoint is `waiting_for_user`; this is an acknowledged command result. The gate response Item attaches to this same Turn, which resumes only to close the old envelope under S05; further worker execution requires an ordinary new `task.start` command rather than AgentSession resume. |
...
| `cancelled` | An accepted cancellation request causes the Turn to terminate `interrupted` with canonical `stopReason=aborted`, or an accepted worker-control final status canonicalizes to `aborted`, including raw `status=interrupted` with raw `stopReason=aborted`. Cancellation is the Task outcome projected from that interruption; it is not a Turn terminal state. |
...
Escalation must preserve selected Task Mode history as references from the new Goal Mode objective and Plan draft. The new Goal Main Thread never copies or rewrites the original Task attempt's Thread history.
...
`task.start` uses ordinary current-resource replay and stores no payload snapshot. Its ledger row contains only the normal command metadata and the original response Turn identifier. On replay, NanoCore validates that Turn against the receipt Workspace and either the addressed direct-Task Thread or the deterministic receiving-Thread lineage of the accepted handoff. A Goal whose `createdByItemId` resolves to that response Turn in the receiving Goal Main Thread, together with the deterministic Task-to-Goal reference Item in the originating Task Thread, owns an `escalated-to-goal` projection; otherwise the existing bounded worker Turn owns the Task state, completion, and current evidence projection. Missing or contradictory originating Thread, receiving Thread, parent, reference Item, Turn, or Goal lineage returns `409 recovery_required`; replay never reruns Coordinator, launches another worker, creates another Thread, or creates another Goal. Because this is a current-owner projection, later legitimate Turn progress may advance `state`, `completion`, and evidence while retaining the original lineage.
...
Conversation-subordinate user-input Gate closeout and the required receiving-Thread handoff lineage are not implemented yet. The current shared closeout accepts only direct `task.start` or Goal `goal.step` ownership and the current legacy Chat handoff still derives the worker Turn from the originating Thread, so this specification remains Partial until the outer `conversation.submit` validation and new-Thread branch above are implemented and verified without a nested Task receipt or copied worker history.
...
- L3: a conversation-subordinate non-secret user-input Gate closes only under the exact actor, Workspace, originating and receiving Threads, request hash, deterministic Task Turn, and outer `conversation.submit` task-handoff receipt tuple; exact response replay duplicates no Item, cleanup, release, receipt, or checkpoint deletion, changed answers conflict, owner mismatch fails closed, restart never resumes the worker, and no nested `task.start` receipt exists.
...
Previously open questions are resolved by accepted V1 defaults: Task Mode performs no automatic worker retry. A user-requested retry is an ordinary new Task invocation with a new request id and Turn, and the prior attempt remains unchanged in Thread history; there is no hidden replay, AgentSession resume, Task-specific causation record, or retry lifecycle. Every Chat-to-Task handoff creates a new receiving Thread in the executing Workspace, links it to the originating Chat Thread through `parentThreadId`, and leaves only a handoff reference Item in the originating Thread.

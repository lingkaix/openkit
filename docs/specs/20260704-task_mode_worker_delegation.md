...
The command ledger does not repair a crash between effect creation and receipt publication. The later launch-fence correction must use deterministic request-derived identities and the existing Turn, checkpoint, scheduler admission, Goal, and Item owners. It MUST NOT add a Task row, response-body snapshot, settlement workflow, or Task-specific recovery state.
...
The opt-in real OpenShell/Codex L6 story `tests/stories/task-mode-real-worker-release.story.md` now validates the real worker path against an existing NanoCore deployment. Its runner requires explicit real-worker and provider-quota opt-in, links a disposable repository through MCP, calls `openkit.start_task`, requires owner-derived `completed` Task state, treats any returned reviews only as independent cleanup work, requires visible thread items, requires at least one completed assistant-message item from the worker path, and writes only redacted evidence. The a1 acceptance run passed against NanoCore on `http://127.0.0.1:54001` with `openkit/worker-codex:dev`, Codex auth/config injection, model `gpt-5.5`, and a real OpenShell sandbox. Worker-shim failure transcripts now preserve redacted stdout/stderr diagnostics for failed Codex launches, and NanoCore retries transient OpenShell provider detach conflicts so cleanup races do not mask worker outcomes.

The current launch path delivers the complete Coordinator request rather than the caller prompt, including ordered Knowledge references, constraints, verification, review policy, and explicit null Review context; the existing Turn-owned Item is the immutable byte-level delivery proof. It still does not materialize the referenced Knowledge content as the accepted Context Package snapshot or persist that package's turn-owned trace. `StartTaskModeRequestSchema` requires `requestId`, `@openkit/core-client` generates one when its caller omits it, and `startModeWorkerTurn` no longer mints a hidden UUID. Chat-to-Task therefore preserves the exact outer Chat request and replays a completed outer receipt without another worker launch. Direct `task.start` consults the shared command ledger before Coordinator or downstream effects, stores no payload snapshot, and replays current Task state, completion, evidence, or Goal escalation from the receipt Turn and existing Item, Artifact, Review, and Goal owners. The public response and generated OpenAPI no longer expose the otherwise unowned full Coordinator decision. Contradictory worker or Goal owner tuples fail closed as `recovery_required`, and replay does not launch another worker or create another Goal. `TaskModeAttemptStateSchema` removes the unowned `needs-review` value and preserves an exact cancelled Turn as `cancelled`; independent reviews remain evidence and Action Center owners rather than Task lifecycle. Neither direct Task nor Chat-originated Task yet has the deterministic checkpoint, Turn, Item, and admission identities needed to reconstruct a crash before receipt publication. The checkpoint also lacks request id and input-hash fields and is not established as the request-bound launch fence before Turn and scheduler effects. Current non-escalation state projection still reads the Turn without verifying its required checkpoint and terminal StopReason tuple. These Context Package materialization, launch-fence, crash-recovery, and terminal-owner mismatches keep this specification Partial; correction must reuse existing records and command idempotency without adding a Task-specific runtime, state row, settlement, or recovery workflow.

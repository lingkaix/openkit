...
- Generative Apps MVP operations (`kernel.*`, `generative-ui.*`, and built-in `openkit-generative` tools) use the existing operation-to-right mapping above. Runtime `WORKSPACE_ROLE_OPERATION_CEILINGS` still gate `workspace.configure` to owner and `workspace.write` to owner/editor; equal active-member eligibility remains unmet for those role rows and is not claimed by this slice.

...
| MCP boot backfill of missing Items (`reconcileWorkerMcpItems` at `apps/nanocore/src/worker-mcp-routes.ts:838-934`; `publishWorkerMcpItem` at `:1992-2034`) | Admit only if full decided values match, including `createdAt` | Fill the missing Item of the already-decided publication | Same-id different `createdAt` or body is rejected. The row domain is the calls that publish a tool-call Item, which carry `capability_id` exactly `mcp.call_tool`; a generative call row carries `mcp.call_tool.<name>` and stores its parent in `item_id`, publishes no Item, and is not reconstructed. `parentItemId` is reconstructed from the AEP snapshot the row's `package_snapshot_id` names and compared like every other decided value. A row that names no snapshot has nothing to be faithful to and is filled without a parent. A row whose named snapshot cannot be read is left unfilled for a later boot, because an Item without the parent the publication carried is a different record and this row admits filling the decided publication, not an approximation. A snapshot that exists and does not validate fails | Item/Artifact created and completed events that accompany the fill are the same repair, not a second exception |
...
The canonical Workspace record is `workspace-record.json` and carries no editable `name` or execution defaults. `name` and `defaultAgentId` live only in `config/workspace.jsonc`; `defaultModelId` and `defaultSkillIds` are absent. Direct readers, index rebuild, backup, export, import, protocol, API, Core Client, fixtures, and Web consume the split owners without a compatibility path.
...
The optional `assistant.resources` selection owned by `20260909-internal_agent_resource_integration.md` also lives in this file and uses the same revision-aware configuration API. Its first slice retains only existing Skill IDs; catalog version/pin remains with the Skill owner. Any later MCP capability binding is admitted with its concrete adapter consumer and refers to the existing MCP catalog, which retains enablement, policy and Vault authority. No per-user Tool selection, additional resource file, internal-run record, persisted prompt or Tool-array record is added. Existing Workspace configuration backup/import carries these references and resolves or leaves unavailable their catalog dependencies without automatic activation.
...
This storage owner assigns User Memory to `users/<userId>/memory/` and Server Knowledge to `server/knowledge/`; existing Workspace Knowledge remains `workspaces/<workspaceId>/knowledge/`. All use the same notebook subtree (`notebook.git`, optional materialized `pages`, `schema`, `proposals`, `reviews`, and existing maintenance families) owned by the Knowledge implementation. Scope schema files are `schema/scope-schema.yaml`; the former Workspace-only schema location is migrated by that owner. Sources remain sibling families at `users/<userId>/sources/`, `server/sources/`, and `workspaces/<workspaceId>/sources/`. Derived scoped indexes remain non-authoritative. No new database, source engine or per-user Workspace tree is created.

`20260909-personal_memory_and_knowledge_learning.md` owns scoped learning; `20260909-knowledge_notebook_editing.md` owns fixed-base editing and Git publication/history under profile v4. `notebook.git` retains the sole published content tree and revision history; `pages/` is only a rebuildable materialization/export, never a second writer. Original Sources remain immutable evidence, not copies made solely to retain page revisions. Root/scope verification and the existing deployment writer exclusion prevent cross-scope or unsupported concurrent access. All notebook writers share the same short publication fence.

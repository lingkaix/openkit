...
The bounded G03 ownership review found no continuing contract unique to this routing overview. C13 owns agent supply meaning and the single authored setup authority; S22 owns `AgentManifest` resolution into `ResolvedAgentSetup`; S20 owns the immutable AEP and governance-backend handoff; S23 owns workspace materialization and review-gated writeback; and S25 owns the generic worker shim, adapter boundary, candidate records, and direct control relationship. Keeping this document active would duplicate those owners and preserve stale launch assumptions.

## Retention Reason

This document preserves the 2026-06-28 consolidation rationale and the historical transition from earlier setup/profile specs. It remains useful for review history, but current design and implementation decisions must follow the owners named above.

...
The NanoCore Config And Identity Contract absorbed runtime-config authorization, file ownership, validation, dry-run, and reload behavior that the UI must project. This UI-management slice lost authority because product presentation cannot define a separate configuration contract from the server-owned source of truth.

## Retention Reason

This document preserves the source-editor workflow, operator affordances, and security constraints considered for the original Settings surface so future UI work can reuse evidence without inheriting obsolete contract ownership.
...
- `docs/specs/superseded/nanocore-config-identity/20260525-runtime_config_reload.md`

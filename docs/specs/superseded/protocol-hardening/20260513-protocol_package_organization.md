...
Protocol Contract Consolidation absorbed package ownership, schema boundaries, generated artifacts, and conformance responsibilities into one active protocol contract. The organization-only document lost authority because package layout must follow the consolidated semantic and evolution rules rather than evolve independently.

## Retention Reason

This document preserves the original package decomposition, generation workflow, and ownership alternatives so maintainers can trace why the active protocol contract chose its present structure without reviving a separate layout authority.

...
Before Plan approval, an explicit human revision request admits one planning-only Goal Orchestrator Turn whose complete Tool set is `goal.plan.propose`. Its input contains the authoritative Goal, the exact previous immutable Plan and the recorded human revision instruction. This exception does not admit initial semantic drafting, Task dispatch, Plan approval, repository, MCP or Vault access, or Goal completion. The initial draft remains deterministic. The same `goal-orchestrator` role and internal-role execution-profile resolution own model selection; there is no second planner role or per-Goal model configuration introduced by this exception.

The planning-only Tool validates its proposed replacement through the existing immutable Plan schema, graph checks and Goal owner. Existing Goal command identity and single-flight admission own retries and concurrent requests; stale prior Plan state, missing or invalid revision context, an unavailable model, or an invalid or absent proposal produces an explicit typed failure, preserving the prior Plan and recoverable Goal state. It must never silently return the unchanged deterministic initial draft as a successful revision. Restart reconstructs revision input from the durable Goal, Plan and human instruction, and every new Plan requires exact human approval before execution. Observable acceptance requires that an explicit revision consumes the stored instruction and previous Plan, yields an inspectable new immutable draft, and cannot dispatch or approve work.

After Plan approval, the complete Goal Orchestrator Tool set is exactly:
...
The unified `conversation.submit` Assistant branch consumes this slice for explicit handoffs without changing Coordinator ownership. Bounded worker requests create a Task Mode handoff through Workflow Coordinator, and longer-running requests create a Goal Mode handoff from the Coordinator's `goal` decision through the shared Goal Mode objective path. When Coordinator returns a non-quick-chat decision that does not select a worker turn, such as `clarify`, `blocked`, `retry`, `review`, `refinement`, `handoff`, or `unsupported`, the Assistant branch records a refused status item with the Coordinator explanation instead of falling through to Knowledge Manager or provider-backed quick chat. Historical deterministic L6 evidence covered both paths: Task handoff created the visible status item, started bounded worker progress, and exposed the worker approval gate, while Goal handoff created a durable planning goal. The retired MCP-only and `chat.start` stories are not active release gates.

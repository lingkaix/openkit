...
The `image.build` effect has an additional conditional requirement: Buildx `0.35.0` or later, BuildKit `0.31.0` or later, and the `exec.proxy` capability must pass the existing pre-effect check. A host may start the base runtime without proving this conditional capability, but it must reject `image.build` before a build effect when the check fails. Worker images are acquired and verified at point of use under the NanoHost owner and are not readiness prerequisites. Stock OpenShell Supervisor bootstrap may still retrieve its pinned image from GHCR before Gateway health; deployment must provide that supported upstream access when needed.
...
For the initial combined small-deployment profile, begin with at least 2 available logical CPU cores, 8 GiB of available memory, and 30 GiB of available persistent storage before loading deployment images and Workspace data. This is an empirical operating recommendation, not a hard minimum, admission rule, capacity promise, per-process allocation, or storage reservation. Actual deployment images, build cache, retained Workspace data, and operating-system use may require more. The NanoHost Image Store's administrator-configurable content limit defaults to 200 GiB; it is an admission ceiling, not reserved capacity or automatic eviction. Full-store admission preserves retained images and refuses growth, and the limit does not replace measuring available storage.
...
Runtime acceptance begins after configuration, enrollment and service start. Worker-image availability is proved separately for each admitted workload; missing Worker images do not block substrate readiness, while stock OpenShell bootstrap dependencies remain applicable. NanoCore readiness belongs to its bootstrap-readiness owner; NanoHost fresh-empty, containment, connection, capacity, and real-stock acceptance belong to the NanoHost owner. A passed requirement check and complete installation satisfy neither acceptance boundary.

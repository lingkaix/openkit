...
The Dockerfile must declare persistent `/workspace` and `/sandbox` targets and create `/openkit` as an independent ephemeral directory; those changes remain part of the pending implementation. Image-level declarations and Docker preservation checks do not prove the complete NanoHost attachment/restart path; that acceptance remains with the volume owner. Fixed immutable executables, non-root containment, current AEP authority, declared runtimes, image verification and build egress rules continue unchanged. Initial volume content is seeded only from the exact selected image; later images must not overwrite existing home data to refresh software.
...
The image must create writable `/sandbox`, `/workspace`, an empty `/openkit/sessions`, `/openkit/session`, `/openkit/artifacts`, the Python virtual environment, and normal user cache/config directories before switching to `sandbox`. No attempt package is baked into `/openkit/sessions`. System runtime directories, the shared shim deployment, the immutable Python installation, and native Agent runtime installations remain owned by root and are not writable by the worker.
...
Every current leaf inherits the common environment and adds only its pinned native Agent runtime, runtime-specific immutable files, labels, and smoke command. Each image installs the fixed absolute `/usr/local/bin/openkit-worker-shim` bootstrap target and shared sanitized launcher. The OpenShell driver replaces image `ENTRYPOINT` and `CMD` with its inert sandbox command, so NanoHost starts the governed worker only through the runtime owner's fixed timeout-zero unary `ExecSandbox` request for `/usr/local/bin/openkit-worker-shim` with no arguments and ignored stdin in `/workspace`; image metadata, package content, configuration, and caller input cannot select that command or carry a Turn credential.

The fixed image helper preserves the ten existing workspace import identities and adds one separate AgentSession-private input: only `reference.import` with literal identity `package-config` and the closed relative path `<agent-session-id>/config/package.json` may create `/openkit/sessions/<agent-session-id>/config/package.json`. It uses the existing no-follow, no-overwrite, mode-`0600`, fsync, atomic-placement, clean-zero-exit, and fixed value-free failure boundary. Adjacent identity or path, export, another config destination, caller or image-metadata selection, a baked package, and a generic configuration-file surface are prohibited.

The fixed zero-argument bootstrap Start environment contains only the existing safe image environment and no AgentSession, Turn, package, or raw route credential; its unary stdin is empty and reaches EOF before execution. The launcher starts only the shared Sandbox Integration supervisor. Private `turn.start` later carries three mutually distinct route tokens through the existing AgentSession-local Harness operation: worker control remains inside the Worker Shim control client, inference enters only the authorized sanitized `OPENKIT_WORKER_INFERENCE_TOKEN` native-Agent binding, and capability enters only the sanitized `OPENKIT_WORKER_CAPABILITY_TOKEN` binding when exact selected MCP supply enables it. The launcher otherwise preserves only the existing bounded OpenKit route bindings, proxy variables supplied by OpenShell, locale and terminal variables, the common development `PATH`, Python virtual-environment variables, TLS variables, and user identity variables. The Pi image may carry `/usr/local/lib/openkit/allow-anthropic-api-key` as carrier capability only. `docs/specs/20260703-agent_manifest_aep_resolution.md` `credentials.declarations` is the authority for whether `ANTHROPIC_API_KEY` is declared for the selected session.
...
| --- | --- | --- |
| Worker image | Installed binaries, immutable versions, fixed shim path, local filesystem layout, non-root identity, fixed zero-argument Harness launcher, smoke command | Endpoint permission, credential generation or persistence, provider selection, Start-field selection, product state |
...
The current strict version 4 AEP selects exactly one reference or bounded build image form. A reference may select one of the current published deployment leaves through the authored reference and pull policy owned by `docs/specs/20260703-agent_manifest_aep_resolution.md`, including the mutable development tags used by built-in templates; selecting the published empty-set base instead fails the launch precondition closed, because it declares no runtime. A build produces one non-published attempt image: NanoHost validates and executes the bounded build, admits the verified OCI result to its Image Store, imports the exact digest into the current Runtime Epoch, and returns digest evidence without creating a published artifact. The exclusions above remain current for that attempt image.

The repository now builds `worker-common`, `worker-codex`, `worker-opencode`, and `worker-pi` from `containers/workers/Dockerfile`, passes the required manifest target through the local build helper and both release-CI build steps, and retains only runtime-specific smoke ownership under each leaf directory. The published common stage installs the accepted tool baseline including pinned mise `2026.8.14` at root-owned `/usr/local/bin/mise`, deploys one shared worker shim and sanitized zero-argument Harness launcher, creates the writable non-root layout and empty `/openkit/sessions`, seeds the Python virtual environment, labels and smokes itself as `worker-common`, finishes as `USER sandbox`, and leaves no root-owned build state beneath `/sandbox` or `/workspace`. Each current leaf regains `USER root` only to install its declared runtime and smoke command, then returns to `sandbox`. The helper preserves the ten workspace import identities and accepts only the exact import-only `package-config/<agent-session-id>/config/package.json` specialization at `/openkit/sessions/<agent-session-id>/config/package.json`. The NanoHost fixed unary bootstrap/response monitor, authenticated Integration-ready latch, private `turn.start` route-token delivery, and mandatory process-group-absence barrier are implemented but remain runtime-owned rather than image-owned.
...
On 2026-09-06, final Worker source `6bf9bfbc01eb4d8903dc71a45fb51e63646f5fb6` passed all eight catalog build and smoke combinations: common, Codex `0.153.4`, OpenCode, and Pi `0.85.1` on `linux/amd64` and `linux/arm64`, including the common-derived image proof. The exact three arm64 leaf images then passed stock OpenShell `0.0.99` create, AEP upload, generic shim dry-run, and delete on A2. This is current image-content acceptance; the adapter owners retain their separate runtime and selectability requirements.
...
- the shared launcher accepts no arguments or stdin credentials and starts only the Sandbox Integration supervisor; private `turn.start` later supplies the three distinct worker-control, inference, and capability route tokens to the AgentSession-local Harness;
...
- Every image exposes the same fixed absolute zero-argument Harness target without using `ENTRYPOINT`, `CMD`, an environment variable, image label, or package field as a bootstrap selector.
- Every image begins with an empty `/openkit/sessions`, accepts exactly one canonical package import per admitted Turn after exact AgentSession admission and before Context imports and native `turn.start`, and exposes no generic config root, package selector, alternate destination, or export path.

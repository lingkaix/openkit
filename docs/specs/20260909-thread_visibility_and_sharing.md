...
Loss of current Workspace access eligibility immediately removes project Thread and derived content eligibility, including its private owner's access. Membership is the ordinary eligibility source; a currently usable presented server-admin bearer is the explicit exception owned by the Permissions Model and never bypasses private ownership. Archive prevents new work under the existing Thread owner but is not deletion or permission revocation. Deletion and cache invalidation use existing retention owners. Previously shared copies are separately owned published material and are not silently rewritten when the private source is deleted; their reuse still respects retained source restrictions.
...
The Core route guard now applies the same audience predicate to addressed Thread paths, conversation-target queries, Turn submission bodies, and opaque feedback, approval and Git-push lineage. Core Thread lists and Workspace Thread counts project eligible records; Thread creation binds request replay to the authenticated actor. Core Turn event publication rechecks current Workspace authority, presented Token usability and audience. Attention, scheduler-admission and interrupted-worker projections filter eligible sources, while generated presentations check both target and retained Item-source audience and refuse private Item publication into shared Threads. These corrections do not complete explicit sharing, private-to-shared work handoff, exported history or all runtime context retrieval.
...
- Full-history snapshot sharing excludes subsequent private messages; exact replay creates no duplicate Thread or output. Stale input, lost current Workspace eligibility or forbidden source disclosure creates no shared result.

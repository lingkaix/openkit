...
- Workspace-scoped tokens MUST be checked against workspace membership on every request. In the single-user v1 posture the owner is the implicit member of every workspace, but the check MUST be structured as a membership lookup so multi-user tenancy later only adds `WorkspaceMember` facts, not new check sites.
...
Existing dogfooding deployments re-authenticate by consuming a bootstrap token (fresh data roots) or by having an operator with an existing Better Auth session issue the first `server-admin` token through the new routes (existing data roots with users).

...
- Consuming the bootstrap token MUST atomically create the owner `User` with the supplied display name, email, and email/password credential, create the first `server-admin` token, then invalidate the bootstrap token in the same transaction. Partial consumption MUST NOT leave a consumed-but-ownerless or non-login-capable-owner state.
...
### Named local credential destinations

- The existing endpoint administration slot is the persistent credential selected by normal CLI authentication. Generic token create/rotate MUST NOT write it. Bootstrap and explicit `credential.store` retain their endpoint-scoped write path; `credential.delete` deletes only that endpoint slot. Named credentials are local secret delivery destinations, not additional Token authority, automatic authentication selectors, server-side NanoHost sinks, or arbitrary filesystem paths.
- The local store exposes `preflightNamedWrite`, `writeNamedToken`, `readNamedToken`, and `deleteNamedToken` with exact `{ baseUrl, destination }` identity, plus `token` for writes. Destination names are non-secret, case-sensitive strings matching `[a-z][a-z0-9_-]{0,63}`. Empty, whitespace-bearing, path-like and malformed names fail closed without normalization; `admin`, `endpoint`, `default`, `current`, and names starting with the token prefix are reserved. Callers cannot address the endpoint administration slot through a named method.
- Named identity uses the SHA-256 digest of the JSON tuple `[baseUrl, destination]`. OS keychains use a distinct `openkit.nanocore.named-token` service (Linux uses that value for the `application` attribute), never the endpoint service or Linux `openkit` application. Encrypted fallback uses the separate owner-only `credentials/nanocore-named` directory below the OpenKit config root, with the existing hashed filenames, AES-GCM encryption, machine-scoped key derivation and identity-bound authenticated data. Keychain preference, secret-safe stdin writers and the degraded-storage warning retain the client credential storage rules above. Named storage rejects linked or non-directory credential parents and linked or non-regular destination files; writes atomically replace only the named file with owner-only bytes instead of following a destination link. The same named slot file uniquely selects the backend: `{ "version": 1, "backend": "os-keychain" }` is a non-secret keychain selection marker; an encrypted envelope carries `backend: "encrypted-file"`. Successful delivery requires publishing that selection after the backend write. An absent, malformed or unreadable selection resolves to no credential, never a search for keychain orphans or older fallback material. Selected-keychain unavailability also resolves to no credential. Thus keychain preference applies to writes; reads and restart honor the last published selection rather than backend availability.
- A write creates or replaces the explicitly selected named slot; callers must deliberately select an existing name when replacing its credential. Different endpoints and names remain isolated. Deletion removes only that named slot selection and attempts keychain cleanup, never the endpoint slot or another name. Its boolean reports whether local selection was removed; unavailable keychain cleanup may leave an orphaned secret in the OS store, but no named read may rediscover it. Deletion does not revoke the NanoCore Token. Reads return the stored secret only to the local credential-store caller and are not exposed as raw-token CLI output. Missing or unreadable encrypted entries resolve to no credential; unsafe linked destinations fail closed. Restart uses the same identity and persisted backend selection. Slot existence does not prove token validity: expiry, revocation, rotation lineage and authorization remain NanoCore-owned records and are inspected through token inventory.
- Preflight checks local fallback prerequisites without storing a probe token and without claiming that later delivery cannot fail. NanoCore issuance and local storage are separate effect domains. Backend write and selection publication are also separate effects: publication failure reports delivery failure and an unknown local outcome, not successful replacement. A failed or interrupted delivery does not roll back issuance or rotation; the one-time secret is not recoverable from NanoCore, and no automatic retry, backend repair, revocation, endpoint replacement or authentication switch is authorized. Inspect inventory before making a new explicit issuance or recovery request.
- Acceptance requires endpoint credential bytes to remain unchanged after named create/replace/delete, separation across names and endpoints on both keychain and fallback paths, rejection of invalid/reserved names before issuance, secret-free command arguments and CLI output, and typed failures distinguishing preflight failure from delivery failure after issuance. Named destinations add no new server lifecycle or access scope.
...
The NanoCore token, bootstrap, authorization, audit, `@openkit/core-client`, bundled CLI credential substrate, and stopped-server administrator recovery are implemented. Generic token creation and rotation remain intentionally excluded from the Agent Skill Interface until a safe named destination exists, so this spec remains Partial.
...
- The local compiled `openkit-operator` discovers active canonical Users and issues one file-first stopped-server recovery credential while holding the existing data-root lock through final output. It opens only an existing Core database, derives one request and Token identity from the exact output path under separate domains, publishes and re-syncs the strict `0600` envelope before any inactive-state commit, and atomically creates the matching `server-admin` Token and server-owned AuditEvent through their existing owners. Exact retries resume or confirm; every contradiction returns `recovery_required` without repair. The app runtime image exposes the compiled executable, while `credential.store` accepts either `{ "token": <secret> }` or the complete envelope and persists only its Token field.

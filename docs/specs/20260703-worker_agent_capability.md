...
### Built-In Repository Target

An Agent Manifest may explicitly select the reserved built-in `openkit-repository` MCP target. Its two tools request exact repository push approval and execute an already granted push under [Git Write Workflow](20260704-git_write_workflow.md#worker-initiated-repository-push). The immutable AEP contains the existing selected descriptor, digest and schema-policy references; strict tool schemas are supplied through MCP discovery. It contains no App API endpoint, App bearer credential, host repository path or host-push secret. An unselected target exposes no route or tool authority. Workspace catalogs and Plugins cannot replace the reserved target with a command or URL. The initial projection supports the existing Codex MCP and human-Gate runtime; unsupported adapters fail selection before a call or Gate can be created.

The existing authenticated package session supplies actor, Workspace, Thread, Turn and AgentSession. Claimed scope cannot override that identity. Each invocation rechecks live session admission, current Workspace authority and selected supply before dispatching the repository owner's command in process. Host Git execution, Vault resolution, target-bound `repo.push` policy and publication records remain with the repository owner. The MCP call records invocation evidence; it is not push authority or evidence that publication succeeded.

Selection is resolved for a new immutable package; removal or drift rejects stale supply at the existing gateway boundary. Human approval uses the coordinated worker Gate-stop path; automatic approval leaves the active worker running. Missing host commits, expired session authority, dependency failures, command replay and interrupted publication follow the Git owner below. No new capability family, token class, durable lifecycle, synchronization mechanism or generic App API proxy is introduced. Acceptance requires selected worker discovery and invocation, a real target-bound human Gate or policy-authorized host push, preserved worker lifecycle, and refusal before effects for unselected or conflicting scope.

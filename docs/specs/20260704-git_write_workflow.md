...
- L2: contract tests: apply-with-commit atomicity (induced commit failure reverts the apply); `commitIds` recorded and linked; deterministic `repo.push`-only Gate replay, changed-input conflict, owner-without-receipt `recovery_required`, and exact granted/completed or denied/cancelled closeout without runtime continuation; push refusals for each typed outcome; review-branch lifecycle (create, terminal-state deletion, foreign-branch drift diagnostic); canary credential values never appear in records, logs, or diagnostics.

...
The workspace synchronization spec deliberately stopped at review-gated patch apply: `WorkspaceApplyResult.commitIds` is always empty, and commit, branch, push, and protected-branch interaction were deferred to a future spec. Real work loops end in commits and pushes, so V1 needs that path — without ever giving workers write credentials or letting publishing become a side effect of accepting a change.

This spec adds three layers, each with its own blast radius: commit-on-apply (an accepted apply may create one local commit with product lineage trailers), ephemeral review branches (an optional staging strategy inside the linked repository), and push as a distinct approval-gated action executed from NanoCore host with vault-resolved credentials and a durable `GitPushRecord`. Workers keep zero write access in every path; protected branches are refused by default.
...
`docs/specs/20260703-workspace_synchronization.md` fixed the safety posture: patch-first review, workers never push, direct protected-branch mutation excluded from the apply contract, and commit/push/tag/deploy deferred as "separate explicit human-approved actions". Its resolved decisions record that the first implementation applies patches without creating commits and records an empty `commitIds` list. The OpenKit self-improvement loop already runs on Git repositories, so the missing write path is felt on every loop iteration: accepted patches must currently be committed and pushed by hand outside OpenKit, which loses lineage exactly at the moment work becomes permanent.

## Decision

- Commit-on-apply: a linked repository MAY enable `commitOnApply`; accepted applies then create exactly one commit containing exactly the applied change set, with lineage trailers, atomically with the apply.
...
- Push is a distinct action: `workspace.git.push` requires its own Action Center approval and a `repo.push` policy decision, executes from NanoCore host with vault-resolved credentials, and produces a durable `GitPushRecord`.
...
- When `commitOnApply` is enabled, an accepted workspace review's apply MUST create exactly one commit in the linked repository containing exactly the applied change set — no more, no less. Apply and commit are one unit: if the commit cannot be created, the apply MUST fail as a whole per the apply plan's conflict handling; a silent apply-without-commit is prohibited.
...
- When the review reaches a terminal state (`approved` and applied, `rejected`, `superseded`), the review branch MUST be deleted, or archived under `openkit/archive/` when workspace configuration requests retention.
...
- Fast-forward and normal merge pushes only: NanoCore MUST NOT force-push, MUST NOT use `--force-with-lease`, and MUST NOT delete remote branches in this contract.
...
Every push attempt — successful or failed — produces a durable `GitPushRecord` carrying:

- push record id and full product lineage (workspace, repository record, approval row id, policy decision id, actor context)
...
- Push rejection (non-fast-forward, remote protection, auth failure, unreachable remote) records the typed outcome on the `GitPushRecord` and raises an Action Center row with recovery options: refresh and re-approve (after fetching new remote state and re-validating linkage), or abandon. NanoCore never resolves push conflicts autonomously and never force-pushes.
...
Commit-on-apply extends the existing Git apply path in the workspace materializer: after `git apply` succeeds against the linked repository, the same operation stages exactly the applied paths and commits with the constructed message, all inside one apply transaction boundary (filesystem-level: apply and commit succeed together or the worktree is restored). Review branches are a second staging adapter beside the opaque staging root, sharing the staging interface from the sync spec. Push is a small NanoCore service invoked only from the approval-resolution path: resolve credentials, select the GitHub provider adapter in V1, execute fixed-argument `git` or `gh` operations with no shell interpolation, observe before/after heads, write the record, emit audit. Remote interaction uses fixed argument lists and a scrubbed environment so credential material never reaches logs.
...
The first fixed-command push helper also exists. `apps/nanocore/src/runtime/git-push-command.ts` builds a `git push --porcelain -- <remote> <source>:refs/heads/<target>` argument vector, rejects option-like, force, delete, and compound-refspec shapes, and returns a scrubbed environment that keeps only caller-supplied basic process variables and caller-supplied V1 GitHub credential variables `GITHUB_TOKEN` and `GH_TOKEN` while disabling interactive Git prompts. It does not read GitHub credentials implicitly from NanoCore's global `process.env`.

The first runner-injected push executor also exists. `apps/nanocore/src/runtime/git-push-executor.ts` composes preflight, immutable `repo.push` permission-decision validation, the V1 GitHub-only provider gate, fixed command construction, a host-side `spawn` runner, terminal outcome classification, `GitPushRecord` persistence, and shared capability usage recording. Once the fixed Git runner is invoked, the executor writes one `workspace.git.push` capability call plus one `category: "network"`, `unit: "requests"` usage row, marking the call succeeded for pushed outcomes and failed for remote rejection or runner failure. It records `pushed`, `rejected-non-fast-forward`, `rejected-protected`, `auth-failed`, `remote-unreachable`, `refused-policy`, and `unsupported-provider` outcomes without exposing credentials beyond the Git child-process environment. A local bare-remote test proves the runner can advance a remote branch through the fixed command vector only when a workspace-scoped `repo.push` `allow` decision matches the repository and target branch.

The public `workspace.git.push` approval path also exists. `packages/app-api-schemas/src/repository.ts` defines redacted Git push approval request and response schemas; NanoCore exposes `POST /api/app/workspaces/:workspaceId/repositories/:resourceId/git-push/approval`; `@openkit/core-client` exposes the route through `client.repositories.requestGitPushApproval`; and MCP exposes it as the mutating `openkit.request_git_push_approval` tool. The NanoCore route binds the request to an existing thread and turn, validates the linked repository resource, creates a policy-originated Action Center approval row through the existing approval gate helper, and records the matching workspace-scoped `repo.push` `require_approval` permission decision. The existing approval response route now resolves these policy-originated Git push approvals locally even when the active worker runtime does not support runtime approvals, writes an immutable `repo.push` `allow` or `deny` decision, creates the `approval-decision` item that removes the Action Center row, and resumes the gated turn.

The public push mutation route also exists. `packages/app-api-schemas/src/repository.ts` defines the approved Git push execution request and response schemas; NanoCore exposes `POST /api/app/workspaces/:workspaceId/repositories/:resourceId/git-push`; `@openkit/core-client` exposes it through `client.repositories.executeGitPush`; and MCP exposes it as the mutating `openkit.execute_git_push` tool. The route requires a granted push approval, resolves the approval item row, reuses the repository's stored Git policy, resolves the repository's optional `git.vaultGrantRef` through the unlocked vault backend only after preflight and policy checks pass, passes the resulting gateway-only GitHub token env into the existing executor, and records the terminal `GitPushRecord`. Route-level coverage proves an approved push can advance a local bare remote through the public API path, that a canary process-level `GITHUB_TOKEN` value does not appear in the public response, and that the workspace-scoped `VaultUse` row links the push to the configured vault grant.

Worker read-token provisioning also exists through the OpenShell provider attachment path. Workspace data-source roots that carry `vaultGrantRef: "grant_github_read"` now cause Agent Environment Package resolution to attach the same durable GitHub read provider even when the worker did not request the GitHub MCP server. Durable resolution requires an active turn-scoped `VaultGrant` targeted to the current agent session, a non-empty policy decision id, backend-handle injection permission, an active vault reference, an `InjectionPlan`, an `InjectionReceipt`, and a redacted `VaultUse` row before backend-private `GITHUB_TOKEN` material reaches OpenShell provider setup. The token value is not serialized into the AEP, vault projection, workspace source projection, logs, or product records. Focused tests cover the source-driven provider attachment path, the policy decision requirement, session binding, expiry projection, and the no-secret serialization boundary.

No remaining current-scope work from this spec is known. Pull requests, GitLab, Gitea, generic Git servers, tags, releases, deploys, and broader code-host APIs remain deferred non-goals.
...
- Push failures become typed, recorded, recoverable events instead of terminal shell errors.
...
- L2: contract tests: apply-with-commit atomicity (induced commit failure reverts the apply); `commitIds` recorded and linked; push refusals for each typed outcome; review-branch lifecycle (create, terminal-state deletion, foreign-branch drift diagnostic); canary credential values never appear in records, logs, or diagnostics.
- L3: NanoCore black-box tests against local bare remotes plus a GitHub-adapter harness: full flow — accepted review → commit → approved push → remote head advanced and `GitPushRecord` `pushed`; non-fast-forward rejection with recovery re-approval after refresh; protected-target refusal without explicit config+approval; unsupported non-GitHub provider refused typed; push with unlinked commits refused; worker clone of a private GitHub repo with a read-only token that cannot push.
...
Acceptance: no force-push path exists; apply-and-commit is atomic; every push attempt leaves a `GitPushRecord`; workers can never write to any remote; protected defaults refuse until configured and approved.

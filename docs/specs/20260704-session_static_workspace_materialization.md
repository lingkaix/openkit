...
OpenShell upload, download, exec, policy, provider, and log outputs are backend evidence and transport effects. For the NanoHost V1 projection, each Context Package inventory entry is imported after bridge readiness, exact AgentSession admission, and canonical AEP import, but before `turn.start`, each declared output is exported only after the terminal barrier, and verified bytes enter an existing canonical collection owner before bridge close and sandbox delete. None is product truth until NanoCore performs that canonical handoff.

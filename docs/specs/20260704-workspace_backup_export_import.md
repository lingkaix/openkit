...
- The fixed source and target runner and its two-job GitHub workflow are implemented through the real bundled local-mode CLI. A same-host execution passes with one transferred archive SHA-256, complete seeded Turn history and semantic graph comparison, authoritative knowledge, repository and Vault re-binding, target mutation, and re-export dry-run. Fresh-runner GitHub evidence has not yet been obtained, and this fixed proof does not claim cross-machine L6 continuation.

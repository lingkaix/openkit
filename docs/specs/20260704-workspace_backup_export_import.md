...
- Backend-native handles, host paths, active sandbox details, runtime compatibility keys, agent-session policy-snapshot bindings, and similar source-host state MUST be excluded or reduced to an explicitly redacted portable summary.
...
The completed G01 implementation slice adds the three Material JSONL families, replaces the Artifact-id-only Review portable schema with the version-keyed row, extends the Workspace SQLite coverage guard, and validates both the source graph and reminted target graph in the same change. Context Package and workspace-handoff references are rewritten in that slice; no later repair pass or compatibility reader exists.

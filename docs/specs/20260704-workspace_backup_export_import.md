...
- When the workspace id does not collide in the target deployment, the importer MUST preserve it. When it collides, the importer MUST mint a new workspace id. Every successful import MUST record `importedFrom` with source deployment id, source workspace id, export timestamp, and manifest digest. A durable Core-backed import MUST also emit a workspace import audit event.

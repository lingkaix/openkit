...
The current implementation still exports and imports the old `workspace.json` record with editable `name` and execution defaults. It is divergent from the accepted record/config split and must be replaced in the same implementation change as the canonical storage rename; prior internal exports are rejected and regenerated rather than upgraded.

...
- For an interactive human request authenticated by a usable presented server-admin bearer, admission retains only its non-secret Token ID, bound to the immutable human triggerActor and exact admission. This credential reference is not a grant, ActorRef extension, AEP field or Token secret. Null provenance uses the ordinary member/responsible-user path; internal, automated and recurring triggers cannot inherit an unpresented Token. Exact replay must preserve the same provenance.
- Dispatch and subsequent Worker launch, inference, tools, credential use and content publication resolve that same admission through existing exact Turn/lease/package lineage and revalidate the Token's current owner, server-admin scope, usability and active canonical User. Invalid recorded provenance, revoked or expired Token, changed owner or scope, or contradictory lineage denies without membership fallback. Restart reloads the reference and applies the same live checks; it must never reconstruct an administrator grant from actor identity alone. Token reference, actor and request lineage remain available for audit; private audience and effect-specific checks remain independent. No Token lifecycle or recovery owner is added.
- Acceptance requires a nonmember administrator Task to complete through the real Worker authority chain, and persisted/reloaded admission with a revoked, expired, rebound or otherwise unusable Token to deny its next governed effect. Foreign private conversation access remains denied.

...
The current lease record has no durable release timestamp or deadline, the lease-watch loop does not expire stuck `releasing` leases, and restart recovery does not close them. The accepted release-grace design therefore remains unimplemented and could leak capacity after an interrupted closeout; the roadmap now treats durable release closeout as a prerequisite rather than an active default.

The V1 durable configured single-Cell scheduler is implemented for local-or-remote admission and target selection, placement, live lease timing, same-snapshot renewal, terminal closeout, and initial restart recovery. Durable release-grace recovery, the scaled profile, weighted per-workspace fairness, full per-scope cap enforcement, concrete endpoint-level health probes, richer scheduler recovery actions beyond admission retry/cancel, and budget-class accounting integration remain deferred future work.
...
- L1: unit tests for the lease state machine (every legal and illegal transition, timing math from durable timestamps), same-snapshot renewal without refresh declarations, continuation under draining pools and quarantined targets with fresh heartbeats, queue ordering (class order, FIFO within class, aging promotion, no intra-class reorder), fairness round-robin selection, quarantine counters and backoff, and epoch fencing predicates.

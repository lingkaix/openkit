...
A durable backend session with no nonterminal scheduler lease owner is an orphan, not an adoptable worker. The pre-listen scan compare-and-sets its existing session row to `cleanup-pending`, preserves the current capacity fence, and permits listener bind. The ordinary post-listen maintenance owner validates a retained terminal lease against its own executing placement plan, admission, backend session, and capacity row before physically cleaning that exact session; a missing lease requires one exact executing plan. Definite cleanup retires its session row as `cleaned` and records a server audit event. Capacity and pool occupancy may release only the verified orphan boundary's proved surplus unit, after no dirty orphan or separate recovery fence remains on that boundary; unrelated rows and versions remain untouched. An unproved surplus, failed or unknown cleanup, or contradictory placement leaves the session and capacity fenced for inspection or retry by the same owner. A live lease never enters this orphan path. No lease, receipt, or product outcome is synthesized from the orphan session.

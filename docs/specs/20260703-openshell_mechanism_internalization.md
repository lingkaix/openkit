...
- `VaultReference` and `VaultGrant` exist as first-slice AEP schema fields and durable non-secret NanoCore metadata; `InjectionPlan`, `InjectionReceipt`, and `VaultUse` exist as first-slice durable non-secret NanoCore metadata. GitHub MCP package resolution derives the AEP provider attachment from the durable `VaultGrant` when Core DB and an unlocked vault backend are available, generates and validates the NanoCore-owned OpenShell profile, projects grant expiry into the AEP and receipt, writes the plan/receipt/vault-use chain, passes resolved credential material only through backend-private materialization context, upserts the OpenShell provider, and attaches it during sandbox creation. NanoCore may detach a provider from a live sandbox when a matching grant is revoked, and it collects redacted provider and refresh-status evidence without storing secret material in product records. Final teardown never depends on detach or resource deletion: it revokes process-local authority and recycles the complete Cell into a verified fresh empty epoch.
- The production TypeScript CLI adapter uses only the official platform path (`/usr/bin/openshell` on Linux and `/opt/homebrew/bin/openshell` on macOS) and exact version `0.0.80`. Across that adapter and the fixed Cell helper, the retained stock CLI surface is version and Gateway inspection; Providers v2 get/set; provider profile export/import; provider get/create/update/refresh-status; sandbox create/list/exec/download; and runtime provider detach for grant revocation. The helper owns Providers v2 enablement and stable-empty sandbox inspection; the TypeScript adapter owns the remaining runtime commands. Sandbox delete, provider delete, host doctor, custom binary selection, insecure Gateway flags, and version ranges are absent.
...
- AEP provider attachment declarations do not attach every runtime file or environment value. The OpenShell launcher passes `--provider` only for credentials resolved through the backend provider materialization path and for explicit transient provider inputs supplied by the launch request; unrelated runtime-file and runtime-environment attachments remain file or environment materialization.
- The V1 mechanism-internalization boundary is partially implemented. Exact profile export/import, generated artifact validation, provider upsert and attachment, currently observable provider and refresh-status evidence, and whole-Cell teardown are implemented. Refresh-log import remains unavailable because stock `0.0.80` exposes no stable refresh-log export surface. The separate full real Codex `0.144.1` root-plus-two-child provenance acceptance independently passed on A1 against stock OpenShell `0.0.80`; it does not close the remaining refresh-log or richer policy-compilation gaps owned here.

...
- The rule that OpenShell-side injection, refresh, attach, and detach events are imported back into NanoCore `InjectionReceipt`, `VaultUse`, and `AuditEvent` records with lineage to the backend session.
...
- Remote OpenShell gateway placement, transport, workspace synchronization, session-static workspace layout, and review gating, which belong to `docs/specs/20260627-remote_openshell_gateway.md`, `docs/specs/20260704-session_static_workspace_materialization.md`, and `docs/specs/20260703-workspace_synchronization.md`.
...
- Reflect every product-relevant OpenShell injection, refresh, attach, and detach event as NanoCore `InjectionReceipt`, `VaultUse`, and `AuditEvent` records with backend session lineage.
...
- Do not make OpenShell provider profiles, provider instances, policy YAML, gateway names, or sandbox ids part of public App API, MCP, or Web UI contracts.
...
- When a grant is revoked or expires, NanoCore MUST detach the corresponding provider from affected sandboxes and delete or disable the provider instance. Because OpenShell does not mutate a live process environment, NanoCore MUST mark affected running sessions stale or degraded per the vault spec's revocation semantics and record the limitation.
...
- Provider attach and detach on a sandbox → `AuditEvent` linked to the grant, injection plan, permission decision, and agent session.
...
- The mapping layer MUST detect upstream drift signals at runtime: gateway rejection of previously valid artifacts, gateway version outside the pinned compatibility range, unknown fields or enum values in imported evidence, or missing expected `_provider_*` layers. Drift signals MUST be surfaced as diagnostics and, when enforcement-relevant, MUST fail closed rather than degrade silently.
...
1. A renderer that turns NanoCore provider access shapes, grants, plans, and decisions into profile YAML, instance create/update/refresh-configure operations, and attach/detach operations, tagged with mapping version and snapshot id.
...
The snapshot package is `packages/openshell-schema-snapshot`, exported as `@openkit/openshell-schema-snapshot`, following `docs/specs/20260522-vendor_snapshot_packages.md` (dated snapshot directories, metadata, checksums, package-local validation). Its snapshot metadata pins the compatible gateway version range, so drift detection reads the range from the snapshot rather than deployment config.

## Current Implementation Projection

- `packages/openshell-schema-snapshot` now pins OpenShell 0.0.80 release provenance, provider profile surface, sandbox policy surface, CLI surface, reserved namespaces, compatibility range, checksums, and package-local conformance tests. The snapshot separates the complete tagged upstream categories, built-in profile ids, protocols, access modes, and enforcement modes from the narrower OpenKit-emitted mapping.
...
- OpenShell worker materialization no longer uploads host Codex auth files directly. Codex auth JSON reaches `/sandbox/.codex/auth.json` only through the vault-backed runtime-file path created by `grant_codex_auth_json`; the backend still allows a non-secret Codex `config.toml` upload for model/config parity with the host CLI.
...
- `VaultReference` and `VaultGrant` exist as first-slice AEP schema fields and durable non-secret NanoCore metadata; `InjectionPlan`, `InjectionReceipt`, and `VaultUse` exist as first-slice durable non-secret NanoCore metadata. GitHub MCP package resolution now derives the AEP provider attachment from the durable `VaultGrant` when Core DB and an unlocked vault backend are available, generates the NanoCore-owned OpenShell profile id `okp-local-github-mcp-v1`, validates the generated profile artifact through `@openkit/openshell-schema-snapshot`, projects grant expiry into the AEP and receipt, writes the plan/receipt/vault-use chain, passes the resolved credential only through backend-private materialization context, upserts the OpenShell provider through `openshell provider create/update`, attaches the provider by id during sandbox creation, tags sandbox creation with the OpenShell snapshot id and mapping version, detaches attached providers through `openshell sandbox provider detach` during backend teardown, can detach active sandbox providers by matching revoked vault grant ids against AEP provider attachments, collects redacted provider evidence through `openshell provider get`, collects redacted refresh status evidence through `openshell provider refresh status` without storing secret material in product records, and runs a NanoCore process service that calls the refresh-status polling entrypoint on the scheduler live-health cadence for currently materialized provider-backed sessions without waiting for full session closeout evidence collection. Provider evidence is normalized into attached, refreshed, detached, or unavailable records when the redacted inspection output exposes that state.
- The accepted V1 mechanism-internalization boundary is implemented. OpenShell 0.0.80 exposes profile import, but OpenKit custom profile import remains deferred because the current generated-profile path does not require it; refresh-log import remains deferred because the pinned CLI still has no stable refresh-log export surface. The V1 generated-profile slice validates generated profile artifacts, uses the generated profile id, imports currently observable provider and refresh-status evidence, and records explicit product-safe diagnostics for unavailable upstream evidence instead of claiming gateway-side capabilities that do not exist.
...
4. Implement the importer: receipts on create/update/refresh-configure/attach/detach, refresh status and log normalization, effective policy digest verification with fail-closed provider-layer checks.
...
- L3: NanoCore black-box tests with a deterministic OpenShell stub covering: denied `vault.use` produces no provider attachment; allowed grants produce attach plus receipt plus audit rows with backend session lineage; expired credential produces fail-closed launch with redacted diagnostic; suppressed `_provider_*` layers (global override or v2 disabled) block launch; revocation detaches providers and marks running sessions stale.
...
- Risk: delegated refresh material makes the gateway a shadow vault. Mitigation: delegation requires its own plan, receipt, and policy decision, and revocation must delete gateway refresh state.
...
- Adopting upstream roadmap features (profile-driven placement, endpoint-scoped static injection, OCSF refresh events, policy prover integration) via snapshot refresh and mapping version bumps when they become current behavior.

...
- The V1 mechanism-internalization boundary is partially implemented. Exact profile export/import, generated artifact validation, provider upsert and attachment, currently observable provider and refresh-status evidence, and whole-Cell teardown are implemented. Refresh-log import remains unavailable because stock `0.0.80` exposes no stable refresh-log export surface. The separate full real Codex root-plus-two-child provenance acceptance is still pending and is not implied by deterministic relay or remote materialization coverage.

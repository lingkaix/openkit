...
- `packages/openshell-schema-snapshot` now pins the OpenShell 0.0.63 provider profile surface, sandbox policy surface, CLI surface, reserved namespaces, compatibility range, checksums, and package-local conformance tests as the first snapshot package slice.
...
- The accepted V1 mechanism-internalization boundary is implemented. Custom profile import and refresh-log import remain deferred because the pinned OpenShell 0.0.63 CLI surface has no stable profile-import command or stable refresh-log export surface; the V1 generated-profile slice validates generated profile artifacts, uses the generated profile id, imports currently observable provider and refresh-status evidence, and records explicit product-safe diagnostics for unavailable upstream evidence instead of claiming gateway-side capabilities that do not exist.

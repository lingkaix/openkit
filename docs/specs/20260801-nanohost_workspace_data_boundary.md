...
- The sole non-workspace `package-config/package.json` import identity, its fixed image-private destination, and its order before Context Package materialization and worker bootstrap.
...
The first `reference.import` after `sandbox.create` is the sole non-workspace source: NanoCore-owned canonical AEP bytes under exact identity `package-config`, exact relative path `package.json`, and exact image-private destination `/openkit/config/package.json`. The body, lowercase SHA-256, byte length, and deterministic request identity follow the AEP owner's canonical byte algorithm. This identity is import-only and is not a declared workspace slot, Context Package entry, output, Artifact, snapshot, configuration selector, or general file destination. An adjacent identity or path, export, another destination, or another package-config file is rejected before the effect.

The remaining `reference.import` source is the prepared immutable Context Package root named `context_<turnId>`. `WorkerContextPackageFiles` supplies the exact bytes and a sorted `fileInventory` of package-relative path, byte length, and content digest; the package-root digest binds that inventory and root. The selected backend matches the generated AEP input to NanoCore-private `workspaceRoots`, accepts only regular files, recomputes the sorted inventory and package-root digest, and enqueues exactly one import per inventory entry only after package-config admission and before `bridge.open`. A source path, host path, archive, adjacent or mutable locator, or another source kind never crosses the wire.
...
For the build form, the fixed `image.build` metadata, exact `image.build/input` verification, local build, and unchanged JSON result settle before `sandbox.create`; the input sub-carriage creates no independent lifecycle step or effect. The fixed sandbox sequence is then `sandbox.create`, the exact `package-config/package.json` `reference.import`, every generated Context Package `reference.import`, `bridge.open`, the accepted worker terminal and process-group barrier, every `file.export`, canonical collection by existing owners, `bridge.close`, and `sandbox.delete`. No later step may be reported complete while an earlier barrier remains unproved. Missing or failed package-config admission blocks every Context import and bootstrap.
...
- The canonical AEP body is the first import after sandbox creation under exact `package-config/package.json`, reaches only `/openkit/config/package.json`, and succeeds before every generated Context Package import and `bridge.open`; adjacent identity, path, destination, export, mutation, or failed admission blocks launch and follows existing no-replay, sandbox-delete, and uncertain-delete epoch-fence semantics.
...
10. The canonical worker-consumed AEP bytes are the first imported file after sandbox creation under exact identity `package-config`, exact relative path `package.json`, and sole destination `/openkit/config/package.json`; they precede every Context Package file and `bridge.open`, while adjacent identity, path, destination, export, missing or changed bytes, and uncertain admission block launch under the existing no-replay and delete-to-epoch-fence lifecycle.

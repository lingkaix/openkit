...
After successful identity discovery, the reader issues exactly one bodyless `GET https://cli-chat-proxy.grok.com/v1/billing?format=credits` request with the same four explicit provider headers plus `x-userid: <userId>`. One timeout of exactly 15,000 milliseconds covers that request and raw-body read, and the same 65,536-byte and fatal-UTF-8 boundaries apply. The consumed response requires `config` to be an object and `config.creditUsagePercent` to be a finite non-negative number. `config.currentPeriod` may be absent; when present, it is an object whose `end` may be absent or a valid RFC 3339 timestamp convertible to canonical UTC `Date.toISOString()` form. Unknown fields are ignored. Deprecated `monthlyLimit`, `used`, `billingPeriodStart`, and `billingPeriodEnd` fields do not supply a compatibility path; a response without the current credits fields is unavailable.
...
Codex quota now uses the strict direct reader, while the current xAI route and schema still return network-free `unsupported`; they do not yet implement the accepted xAI adapter and provider-neutral quota union above. The prior Codex account and Gateway artifacts have been physically removed. This spec remains `Partial` until that implementation gap and the accepted real-use evidence close; the fixed run shape is an L3 opt-in test concern and its execution history belongs to the executing release work rather than this specification.

...
With one resolved bearer snapshot, the reader issues exactly one bodyless `GET https://cli-chat-proxy.grok.com/v1/user?include=subscription` request. Its explicit provider headers are `Authorization: Bearer <apiKey>`, `X-XAI-Token-Auth: xai-grok-cli`, `x-grok-client-version: 1.0.12`, and `x-grok-client-mode: headless`; it must not send `x-userid` on this discovery request. Transport-generated headers such as `User-Agent` use the ordinary NanoCore HTTP client identity and are not provider-specific adapter inputs; NanoCore does not synthesize the Grok Build process user-agent string. One timeout of exactly 10,000 milliseconds covers the request and raw-body read. The reader accepts at most 65,536 raw response bytes inclusive, decodes them as fatal UTF-8, and parses JSON. The consumed response requires a non-empty string `userId`; `subscriptionTier` may be absent or a non-empty string, and unknown fields are ignored. The canonical `userId` is held only for the immediately following request and never enters durable state, logs, diagnostics, audit details, or a public response.

After successful identity discovery, the reader issues exactly one bodyless `GET https://cli-chat-proxy.grok.com/v1/billing?format=credits` request with the same four explicit provider headers plus `x-userid: <userId>`. One timeout of exactly 15,000 milliseconds covers that request and raw-body read, and the same 65,536-byte and fatal-UTF-8 boundaries apply. The consumed response requires `config` to be an object. `config.creditUsagePercent` may be absent; when present, it must be a finite non-negative number. A valid observation requires at least a supplied percentage or a valid `currentPeriod.end`; an empty config does not establish quota availability. `config.currentPeriod` may be absent; when present, it is an object whose `end` may be absent or a valid RFC 3339 timestamp convertible to canonical UTC `Date.toISOString()` form. Unknown fields are ignored. Deprecated `monthlyLimit`, `used`, `billingPeriodStart`, and `billingPeriodEnd` fields do not supply a compatibility path; a response with neither a valid percentage nor a valid `currentPeriod.end` is unavailable.

The available xAI projection emits `planType` only when discovery supplied `subscriptionTier` and emits exactly one window with fixed id `included`, `usedPercent: creditUsagePercent` clamped to `0..100` and `remainingPercent: 100 - usedPercent` only when the percentage was supplied, and `resetsAt` only when a valid `currentPeriod.end` exists. An absent percentage remains unknown: both percentage fields are omitted, Web labels the usage as not reported and renders no percentage meter, and Skill preserves the same omission. A known reset time and optional plan type remain available without a percentage; neither the vendor CLI display default nor absent fields justify reporting zero usage. This clamp preserves an available exhausted-quota result when the provider reports overage above 100 rather than converting it into dependency failure. The projection does not include current-period type or start, prepaid balance, on-demand state or amounts, auto-top-up rules, history, product breakdown, provider bodies, user ids, headers, credentials, or raw errors. Discovery or billing network, timeout, non-2xx HTTP, body-limit, UTF-8, JSON, or consumed-schema failure returns only `temporarily_unavailable`; there is no retry, third provider request, durable quota state, or stale-value fallback.
...
Codex and xAI quota now use their strict direct readers and the provider-neutral `available` / `temporarily_unavailable` projection above. The xAI reader preserves a valid reset time when the upstream omits `creditUsagePercent`; missing percentages remain unknown rather than becoming zero usage. Successful HTTP responses without either recognized quota value still do not prove an available observation. The prior Codex account and Gateway artifacts have been physically removed. This spec remains `Partial` until the accepted real-use evidence closes; the fixed run shape is an L3 opt-in test concern and its execution history belongs to the executing release work rather than this specification.

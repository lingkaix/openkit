...
The accepted initial subscription providers are `openai-codex` and `xai`. `openai-codex` supports interactive login and Codex subscription inference; `xai` supports the reviewed pi-ai xAI login and Grok inference path. Both providers expose subscription quota through small, strict, release-coupled adapters traced to versioned source in their vendor-maintained clients. A provider interface does not need to be publicly documented when that source establishes the exact read-only contract and the adapter preserves the bounded failure and redaction rules below. Quota failure never blocks inference.
...
| `xai` | `device_code` through the reviewed pi-ai version | Grok through pi-ai | `available` through the strict xAI credits adapter, otherwise `temporarily_unavailable` |

The public inventory is fixed and ordered as `openai-codex` followed by `xai`. Its exact descriptors are `{ subscriptionProviderId: "openai-codex", displayName: "OpenAI Codex", loginModes: ["device_code"], quotaCapability: "available" }` and `{ subscriptionProviderId: "xai", displayName: "xAI", loginModes: ["device_code"], quotaCapability: "available" }`. Catalog order, authored profile order, account state, credential state, and dispatch readiness cannot alter or remove either descriptor. Inventory handling must return these descriptors without enumerating or reading account directories, `account.json`, Vault references, Vault availability, or credential state.
...
GET    /api/app/provider-subscriptions/{subscriptionProviderId}/accounts/{accountSlotId}/auto-topup
```

The canonical operation ids in route order are `listSubscriptionProviders`, `listProviderSubscriptionAccounts`, `createProviderSubscriptionAccount`, `updateProviderSubscriptionAccount`, `deleteProviderSubscriptionAccount`, `getProviderSubscriptionAccountStatus`, `startProviderSubscriptionAccountLogin`, `cancelProviderSubscriptionAccountLogin`, `logoutProviderSubscriptionAccount`, `getProviderSubscriptionAccountQuota`, and `getProviderSubscriptionAccountAutoTopup`.

All eleven routes require deployment-admin authority and register through the canonical App API operation catalog. Inventory, account list, status, quota, and auto-top-up are read operations; create, update, delete, login, cancel, and logout are mutations. DELETE succeeds with `204`, an empty body, and no response `Content-Type`; every other lifecycle read or mutation succeeds with `200`; quota normally returns its typed union with `200`. Create, update, login, and cancel require the strict bodies defined below. Logout accepts no request body. Provider ids and slot ids are validated before storage or credential access. For auto-top-up, the xAI-only capability check is part of request validation and precedes account reconciliation; a valid non-xAI pair yields `invalid_request` regardless of its metadata state.

Provider inventory is the only account-state-free operation. The other ten ordinary operations—account list, create, update, delete, status, login start, login cancellation, logout, quota, and auto-top-up—must apply the failed-closed metadata, whole-backend-inventory, and exact Core-row checks defined by this spec before account existence, conflict, lifecycle, or quota handling. A pair-addressed operation checks its requested pair; an account list checks every account-directory pair for that provider and every live or revoked backend inventory row whose non-secret owner metadata names that provider. If any checked current state or historical row violates the ordinary validity invariant above, the entire operation fails, and an account list must not return a partial array that omits the pair. Completed historical tombstones are validated but never projected as accounts. Delete and logout have only the narrow exception that the exact backend-recognized revoked tombstone for the current binding, together with its exact same-version Core row, may continue the already-started removal sequence; no other operation may treat that prefix as an ordinarily valid account.
...
```ts
type SubscriptionAccountObservation = {
  planType?: string;
  subscriptionActive?: boolean;
  accountObservedAt?: string;
};

type ProviderSubscriptionQuota = SubscriptionAccountObservation & (
  | {
      subscriptionProviderId: SubscriptionProviderId;
      accountSlotId: string;
...
### Release-Coupled Private Provider Interfaces

A private quota interface is accepted only when its consumed behavior is present in versioned source maintained by the provider for its own client, the operation is read-only and scoped by the same account credential, the request and consumed response fields are frozen in this specification and focused tests, and every drift or dependency failure becomes the redacted `temporarily_unavailable` branch. Public documentation is preferred but is not required. Browser or console scraping, guessed endpoints, billing mutations, raw response projection, retries, stale-value fallback, and inference gating remain forbidden. Each provider adapter is reviewed independently on an upstream-source change; common public projection does not turn the private transports into a general billing framework.
...
Codex never emits `subscriptionActive`, `accountObservedAt`, or `billing`. The available projection always emits `planType` as the exact `plan_type` string and includes only non-null windows, ordered primary then secondary. Those windows use the fixed ids `primary` and `secondary`, preserve the exact integer as `usedPercent`, derive `remainingPercent` as `100 - usedPercent` clamped to `0..100`, and emit the converted timestamp as `resetsAt`.
...
### xAI Release-Coupled Quota Reader

For `xai`, the quota route first completes the same reconciliation and pair-handle resolution as Codex. It then calls the handle's stock pi-ai `models.getAuth("xai")` without an API-key override, so pi-ai retains ownership of expiry detection, serialized refresh, and persistence through the slot-scoped `CredentialStore`. The resolved auth is usable only when `auth.apiKey` is a non-empty string. An absent or invalid result or any auth-resolution or refresh failure returns `temporarily_unavailable` without calling either quota-interface endpoint. NanoCore does not read or extend the xAI OAuth credential shape to obtain provider identity.

With one resolved bearer snapshot, the reader issues exactly one bodyless `GET https://cli-chat-proxy.grok.com/v1/user?include=subscription` request. Its explicit provider headers are `Authorization: Bearer <apiKey>`, `X-XAI-Token-Auth: xai-grok-cli`, `x-grok-client-version: 1.0.12`, and `x-grok-client-mode: headless`; it must not send `x-userid` on this discovery request. Transport-generated headers such as `User-Agent` use the ordinary NanoCore HTTP client identity and are not provider-specific adapter inputs; NanoCore does not synthesize the Grok Build process user-agent string. One timeout of exactly 10,000 milliseconds covers the request and raw-body read. The reader accepts at most 65,536 raw response bytes inclusive, decodes them as fatal UTF-8, and parses JSON. The consumed response requires a non-empty string `userId`; `subscriptionTier` may be absent, null, or a string, and unknown fields are ignored. Discovery emits `accountObservedAt` when successfully observed, `planType` only for a non-empty tier, and `subscriptionActive` using the official Build subscription check: true only for a non-empty tier other than exact case-sensitive `Free`, without trimming or case normalization, false otherwise. This is subscription eligibility, not an effective coding-access grant, a successful inference promise, or the separate remote-settings access gate. No retention preference or payment method implies access. The canonical `userId` is held only for the immediately following request and never enters durable state, logs, diagnostics, audit details, or a public response.

After successful identity discovery, the reader issues exactly one bodyless `GET https://cli-chat-proxy.grok.com/v1/billing?format=credits` request with the same four explicit provider headers plus `x-userid: <userId>`. One timeout of exactly 15,000 milliseconds covers that request and raw-body read, and the same 65,536-byte and fatal-UTF-8 boundaries apply. The consumed response requires `config` to be an object. Optional consumed provider fields accept absence or null as unknown; a supplied non-null field of the wrong type fails the billing observation. `creditUsagePercent` must be finite and non-negative, clamped to `0..100` for the public used percentage; remaining is derived only from that supplied value. `currentPeriod` accepts optional `type`, `start`, and `end`: type is a string, with only `USAGE_PERIOD_TYPE_WEEKLY` and `USAGE_PERIOD_TYPE_MONTHLY` projected as `weekly` and `monthly`; an unrecognized string is omitted without inventing a period. Supplied start/end must be valid RFC 3339 instants and are canonicalized to UTC. Deprecated quota fields, history, product breakdowns, and billing-response subscription/on-demand flags are ignored.

`prepaidBalance`, `onDemandUsed`, and `onDemandCap` are independently optional USD-cent objects. For a present object, absent `val` means the protocol's explicit zero, while supplied `val` must be a safe integer; null `val` or a non-object value is invalid. Signed amounts are preserved without absolute-value conversion or floating-point money arithmetic. An absent or null entire amount object remains omitted, never zero. Optional `isUnifiedBillingUser` is a boolean projected as `billing.sharedAllowance`; absence is not false. A valid billing observation requires at least a supplied percentage, recognized period type, period timestamp, amount, or shared-allowance value; empty or wholly unrecognized config remains unavailable.

The available xAI projection emits one `included` window only when usage or a recognized period field exists; a money-only observation emits no synthetic window. It emits `billing` with `currency: USD` and only supplied money/shared-allowance fields when any exist. Missing percentages stay omitted and never produce a meter or a zero-used default. A zero prepaid balance says nothing about included quota exhaustion. Successful discovery metadata comes only from that same invocation, never a previous observation, and survives a subsequent billing failure with its own `accountObservedAt`; quota `observedAt` marks the completed billing attempt, and failure contains no windows or billing values. Discovery failure supplies neither account metadata nor a fictitious account observation. Discovery or billing network, timeout, non-2xx HTTP, body-limit, UTF-8, JSON, or consumed-schema failure yields `temporarily_unavailable`; quota never requests auto-top-up or a third provider endpoint, retries automatically, or stores a durable observation. The upstream authority is the reviewed `xai-org/grok-build` revision `482711333c7195dc16a272777f86086d615e2afb`, specifically the billing, login model, and subscription-check code; changes require a new source review.

### Lazy xAI Auto-Top-Up Read

`getProviderSubscriptionAccountAutoTopup`, exposed to Skill as `provider-subscription.account-auto-topup`, is a read-only capability for the xAI pair; a supported but non-xAI provider returns `invalid_request` before account or credential access. It requires the same deployment-admin authorization, ordinary account reconciliation, pair handle, and typed error precedence as quota. With one stock pi-ai auth resolution and fresh bounded user discovery, it sends exactly one bodyless `GET https://cli-chat-proxy.grok.com/v1/auto-topup-rule` with the billing header set, including the ephemeral `x-userid`. The rule request has a 10,000-millisecond request-and-body timeout and the same byte, UTF-8, JSON, and redirect boundaries. It never fetches billing, mutates a rule, or retries automatically.

The consumed response is an object with optional nullable `rule`. Missing/null rule yields an available observation with unknown enabled state and omitted amounts. A present rule is an object: absent `enabled` is protocol-default false, a supplied value must be boolean, and `minBeforeHittingSl`, `topupAmount`, and `maxAmountPerMonth` use the same optional signed safe-integer USD-cent parsing above, projected as `thresholdCents`, `amountCents`, and `monthlyCapCents`. Saved-payment information is never consumed or used to infer enabled state. Any auth, discovery, transport, decoding, or consumed-rule-schema failure gives the redacted temporary-unavailability observation. Failure affects only this observation, not the quota card, login state, or inference.

### Web And Skill Observation Semantics

The deployment-scoped AI interface card has three levels: account name and connection status with plan and only the actual supplied quota period; usage/reset/last-check with an account-scoped Refresh action; and native disclosures for Balance and costs and Account settings. Account metadata and billing refresh on page load and manual refresh of that account. Auto-top-up is fetched only when that account's costs disclosure is expanded, and again on explicit retry or refresh while expanded; no collapsed-card request, background poll, cross-account refresh, or durable cache is added. Failed refresh removes prior observation values instead of presenting them as current. Unknown individual monetary fields and auto-top-up state are labeled not reported. A successful billing response with no percentage is labeled Provider did not report usage; a failed query is labeled query failed and offers retry. Explicit zero displays zero used and 100 percent remaining. Only supplied weekly/monthly periods are shown, without inferred alternate windows. Prepaid balance, extra spend, spend cap, and auto-top-up status stay distinct in the expanded details. Times use the browser's local timezone with the full instant and timezone inspectable. Web, Core Client, and Skill expose identical omission, zero, currency, period, and observation-time semantics; none interprets missing quota as sufficient or exhausted.
...
- A corrupt `account.json` remains failed closed even when no live Vault material exists. Missing metadata remains failed closed when account-directory residue exists or a live Vault reference has owner metadata for that pair; an absent account with only completed historical tombstones is not missing metadata because those histories create no account. All ten ordinary account operations use the exact persistence failure above for corrupt, live-orphan, incomplete-removal, or invalid-history states, while completed history alone does not block fresh pair use; provider inventory remains unaffected because it does not access account state.
...
The seven server `AuditEvent` actions are active through the composed manager, and the existing audited `vault.resolve` path remains the credential-read producer. The unified Gateway resolves the explicit pair before dispatch and uses stock pi-ai for native Codex Responses and reviewed xAI inference. The bundled Skill catalog maps the same eleven operations through its existing generic `ops call` path without a provider-specific command branch.

The Web AI interface now projects the fixed Codex and xAI inventory and the complete provider-scoped account lifecycle through `client.providerSubscriptions`, including slot creation, rename, deletion, device-code login, polling, cancellation, logout, refresh, and quota posture. Generic API-key provider profiles use the separate Vault-owned `setProviderApiKey` operation and do not enter this subscription-account state machine.

Codex and xAI quota now use their strict direct readers and the provider-neutral `available` / `temporarily_unavailable` projection above. The xAI reader preserves a valid reset time when the upstream omits `creditUsagePercent`; missing percentages remain unknown rather than becoming zero usage. Successful HTTP responses without recognized quota or billing values still do not prove an available observation. The prior Codex account and Gateway artifacts have been physically removed. This spec remains `Partial` until the accepted real-use evidence closes; the fixed run shape is an L3 opt-in test concern and its execution history belongs to the executing release work rather than this specification.

## Accepted Design

The clean target is one cohesive NanoCore Provider-subscription account manager with Provider-specific capability adapters only where the Provider contract genuinely differs. The manager owns slots, authorization, metadata, Vault references, per-slot locking, and public projection. Stock pi-ai owns login and refresh through one `Models` plus one constrained `CredentialStore` view per slot. Codex and xAI each use one small release-coupled quota reader behind the existing route and public quota projection; no Provider-specific account manager, credential format, billing subsystem, or account selector is introduced; the one lazy read-only auto-top-up route shares the existing pair-scoped authorization and credential boundary. Gateway composes multi-account behavior solely from ordinary one-slot Provider profiles and logical-model route members.
...
- Codex and xAI quota each use a bounded read-only provider adapter traced to vendor-maintained client source; public documentation is not a prerequisite when the exact private contract is reviewed and fails closed.
...
- Codex browser login after stock pi-ai exposes a server-compatible callback seam or a separately accepted completion operation; this deferred login flow adds no operation or deployment-mode restriction; the accepted read-only auto-top-up operation above is unrelated.

...
Implementation note: item projection, digest records, and the accepted Turn-owned Task and Goal delivery trace are implemented. Real worker launches persist and verify the exact Coordinator request, generated `context` input, immutable package files and trace, Workspace Input Snapshot, Workspace Materialization Record, backend handoff, selected or excluded Workspace Material revisions, and queue proof before launch. The accepted trace does not yet bind selected Knowledge page identities, content digests, provenance, or worker-visible Knowledge bytes. Current standalone Knowledge preparation traces and materializations are duplicate diagnostic projections scheduled for deletion by G07 and do not close that delivery gap. Historical inspection currently stops at digest-checked materialization readback; broader Knowledge candidate audit detail and citation projection remain unimplemented.
...
- The accepted Task and Goal worker-Turn trace records exact request bytes, ordered Item references, Material selection or exclusion, deterministic byte-based budget decisions, immutable package files, generated `context` input, WIS/WMR and backend-session lineage, queue proof, and stock OpenShell read-only upload verification. Current standalone Knowledge Manager context traces and materializations are duplicate implementation projections scheduled for deletion by G07; they are not part of this contract.

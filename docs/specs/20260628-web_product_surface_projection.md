...
- The live Tier-A board-19 **Repositories** screen currently projects selected-Workspace repository resources, diagnostics, default-repository setup, durable push records, and the existing approval-gated Git push workflow through `client.repositories`. Its consumed-record tuple and authoritative settlement behavior are implemented; navigation places it exactly once in the compact Workspace-access icon row directly above the Settings footer, and TanStack mutation data is the sole server-state owner for the current approval response.

...
A callable included operation whose owning capability is not release-ready MUST NOT be presented as a working feature. Its exact gap remains unpublished. Automations stay unpublished while they only persist in-memory definitions and do not operate recurring work; R092 remains the owner. Knowledge proposal drafting and reversal stay unpublished until ordinary-user reads can recover the exact candidate and accepted application lineage after restart; R070 and R072 remain their owners. Archive transport stays unpublished until the Web portability surface owns export and import progress; R008 remains its capability owner. Workspace deletion and deleted-Workspace recovery stay unpublished until the remaining multi-user Web projection supplies those controls; R049 remains their Web owner. Channels and Generative UI remain unpublished internal review shells because they have no included catalog operations to project. The [Generative UI Interaction](20260908-generative_ui_interaction.md) contract owns presentation and interaction independently of Kernel. Publishing UI over existing records requires that contract, real operation coverage, and tested behavior; only Kernel-backed views additionally require the Kernel data/operation contract. The accepted initial contracts do not themselves add implemented catalog operations or change publication status.

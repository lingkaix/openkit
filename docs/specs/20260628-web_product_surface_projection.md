...
No published ordinary-user route MAY call a server-scoped API or require a deployment-admin token. General Settings MUST NOT call deployment diagnostics or runtime-config routes. The distinct `/settings/configuration` and `/settings/ai-interface` deployment-admin routes MUST first use the current client so local mode remains credential-free; after a typed `401` or `403`, either route MAY accept one explicit `server-admin` bearer token for the lifetime of the mounted page. That token MUST remain only in the Core Client transport held in React component memory and MUST NOT enter TanStack Query keys or data, Zustand, browser storage, routes, logs, rendered values, diagnostics, or error copy.
...
The screen does not clear or revoke generic provider API keys, invent provider or model records outside runtime configuration, persist the admin token or API key, or treat an ordinary Better Auth session as deployment-admin authority. A typed denial opens the same component-memory credential gate as Configuration; other failures remain visible failures with explicit retry.

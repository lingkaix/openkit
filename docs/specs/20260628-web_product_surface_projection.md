...
- Do not fold deployment-admin, bootstrap, Gateway-actor, or Worker-private operations into an ordinary Workspace surface.
- Do not persist a server-admin bearer credential in browser storage or treat a valid Better Auth session by itself as deployment-admin authority; NanoCore may derive deployment-admin authority when the active session User owns a currently usable `server-admin` Token under the remote-auth credential contract.
...
`downloadWorkspaceExportArchive`, `dryRunWorkspaceArchiveImport`, and `importWorkspaceArchive` remain unpublished under R008 because the portability owner explicitly defers Web export and import progress. `deleteWorkspace` and `recoverDeletedWorkspace` remain unpublished under R049 because the multi-user owner defers those controls to its remaining Web projection. These roadmap dispositions add no UI feature and do not reopen the completed non-Web capability slices.

## Contract / Expected Behavior

### Inclusion and exclusions

An ordinary-user Web operation is included when it is a `PUBLIC_OPERATION_ACCESS` key whose `scope` is `user` or `workspace` and whose `authentication` is not `gateway-actor`. The count of that set is 147. Inventory derivation MUST use `PUBLIC_OPERATION_ACCESS` together with generated App API OpenAPI and `@openkit/core-client`; it MUST NOT use screen titles as the source list.

The following are excluded from ordinary-user Web and MUST NOT appear on a published ordinary-user route:

- Server-scoped catalog operations, including deployment-admin and bootstrap-secret authentication, currently 46 keys. Roadmap R048 owns their future separate admin surface.
...
The shell-level Search capability is opened from an accessible icon-only command at the right of the brand row and MUST NOT reserve a persistent strip above every routed main view. The selected Workspace switcher is always visible immediately below the brand row, shows the authoritative selected Workspace name, and owns the UI-only selection that rebinds every selected-Workspace query.

That sidebar control is the only Workspace switcher in published Web. Chat starter, Thread, and every routed main view MUST NOT render a second selector. The canonical Chat Thread route is `/chat/:workspaceId/:threadId`; link producers and route consumers use that exact lineage and MUST NOT encode the Workspace as a query parameter or preserve a compatibility route.

The Agents surface is a selected-Workspace projection. It MUST read the selected Workspace's Agent resource set and bind health operations to that same Workspace; it MUST NOT display the deduplicated cross-Workspace Agent catalog as though every listed Agent belonged to the current Workspace.

The same surface separately presents current Workers through one NanoCore-owned selected-Workspace read projection shared by the composed Core Client and public Skill. Configured Agent supply and observed Worker state MUST have distinct labels. A Worker row represents product-safe availability for the current AgentSession of an authorized visible Thread, not a new durable Worker entity or an AgentSession selection surface. Internal roles, terminal predecessors, hidden Threads and catalog-only supply do not become Worker rows. The row is keyed by Thread, so concurrent Workers using one Agent remain distinct and a current Worker remains visible after its Agent leaves the configured catalog; AgentSession, native conversation, Sandbox and credential identifiers MUST NOT appear in ordinary output. Core retains all lifecycle authority; reading or refreshing this projection creates, resumes, retries, replaces or terminates no work.

Current work affiliation comes only from an exact matching uncleared Worker checkpoint and its existing Turn, Thread and AgentSession lineage. Its Goal/Task pair is either both absent for Task Mode or both present for Goal Mode under the Worker reliability envelope; it MUST NOT be inferred from the latest Goal, a navigation category or a later current Task. A checkpoint with a null recorded worker session is pre-launch and supplies no current assignment; a non-null session must equal the row's current AgentSession. No current checkpoint means no recorded current assignment. Conflicting or incomplete lineage is reported as unavailable, not repaired or guessed. A row may link to its authorized Thread through the existing conversation route; this navigation does not assert that an unsupported runtime can accept input while busy. Runtime status is labeled as the last recorded state with its record update timestamp, not fresh process-liveness proof. Fetch failures retain an explicit unavailable or stale indication and never become an empty successful inventory.

Model and configuration details derive from existing records, not the configured catalog entry. The exact current package snapshot, after matching its Workspace, Thread and AgentSession scope, supplies a separately labeled preferred logical model and package-selected MCP policy. The read requires existing `workspace.read` access, and last-used usage fields additionally retain existing `audit.read` authorization; an unauthorized viewer receives an explicit restricted indication without those values. The most recent `category=llm` usage attributed to the row's current AgentSession, joined by capability-call id to a `family=llm` call and its exact existing Turn with matching Workspace, Thread and current AgentSession lineage, supplies a separately labeled last-used logical model and timestamp even when that Worker is now idle; this historical usage does not imply a current assignment. Usage from a retired predecessor on the same Thread is excluded. A difference between package preference and last-used model is not itself a configuration-drift signal. Mismatched or absent attribution stays unavailable and provider references are not native model identities. Progressive disclosure uses an explicit field allowlist: package-selected Workspace MCP server ids with allowed/denied/approval-required tool names, and filesystem/network/process defaults, enforcement labels and rule counts. Unknown keys and raw values are never copied through. These are the Workspace-owned servers selected by this package, not a new editable Agent policy tier. It MUST NOT expose raw policy rules, local paths, commands, environment values, endpoints, credentials or native runtime metadata. These are package intent and recorded usage, not proof of actual runtime loading or permission to change a running Worker. Replacement, restart or configuration change is reflected only through current authoritative records; this read projection has no retry, cache-repair or durable migration lifecycle of its own.

Acceptance requires configured supply without a current Worker to remain catalog-only; two Workers of one Agent to remain distinct; Workers absent from the latest catalog to remain visible; null-session pre-launch checkpoints to supply no assignment; idle Workers to have no current assignment without historical backfill; last-used usage to retain the audit access boundary; exact current Goal Task attribution despite unrelated or later Goal state; private Thread omission before dependent data reads, producing viewer-relative counts rather than a capacity inventory; distinct package preference and last-used model values; explicit missing/conflicting evidence; no hidden runtime identity or secret-bearing configuration in the payload; and matching Web/Skill results from the same typed read. Model switching and busy Worker input retain their separately owned command and runtime contracts.

Archived threads are a selected-Workspace projection reached from the compact sidebar shortcut grid. The active **Conversations** sidebar list excludes `status=archived`; the Archived threads surface lists only archived rows and restores a row through the existing authoritative Thread update with `status=active` before refreshing both projections. `ThreadSchema` does not own a Chat or Task discriminator and Task Mode deliberately has no separate durable Task record, so Web MUST NOT infer a Thread kind from its route, Turns, or transient browser state. The selected-Workspace **New conversation** action enters Chat Mode; a Task-mode route may enter Task Mode on the same Thread without reclassifying it. Web adds no separate archive store or recovery API.

NanoCore's selected-Workspace `listConversationNavigation` App projection supplies visible active Threads, current/latest activity (`chat`, `task`, `goal`, or `unknown`), viewer-relative state (`needs-you`, `working`, or `idle`), and `lastActivityAt`. It is derived from existing records, not a durable Thread kind. Active Goal context takes precedence, then latest Goal context when newer than the selected Turn; otherwise the current or latest Turn's configured Worker assignment/session or established internal Chat Agent assignment identifies activity. The Chat producer records quick-chat for Assistant commands and knowledge-manager for direct Knowledge Manager commands in the existing Turn.agentId field. Missing or unrecognized assignments remain unknown, including historical unassigned Chat Turns; opaque Item IDs and expiring command receipts do not determine activity. Web consumes this projection without classification heuristics.

`needs-you` takes precedence over `working` and requires an exact viewer-eligible Turn approval/question or existing actionable Goal plan/result review. `working` requires a pending/running Turn or running Goal. A completed Turn is not an unread reply. Working and actionable rows sort before idle rows, then by newest actual Item/Turn/Goal activity and stable Thread id. Thread creation is the baseline activity for an empty conversation; later name or archive metadata changes do not count as conversation activity.

The projection creates, updates, and removes no durable record; each read is recomputed from authorized current owners, including after restart. Missing activity evidence is explicit unknown, and archived or invisible Threads are omitted. A failed refresh must be exposed as stale or unavailable in Web and must not manufacture an idle state. Retry refetches the same read model. Foreground refresh keeps other conversations current without starting work. While this browser is awaiting its own conversation submission response, the existing mutation state may temporarily promote that exact Workspace/Thread row and show a blue Waiting for response indicator; this is explicitly client request progress, ends when the request settles, and neither claims an accepted Turn nor persists a server lifecycle state. Unread receipt persistence and independent Goal child-conversation grouping remain outside this projection until their requested scope is settled; no hierarchy or read status is inferred. Acceptance requires one mixed Workspace to preserve visibility, exact gate authority, unknown unassigned work, and newest-reply ordering on an older Thread.


The Thread title bar prioritizes the Thread name and future compact status. Rename and Archive are accessible icon-only actions immediately beside the name; the accessible icon-only **Side panel** toggle is aligned to the far right. The panel currently mirrors the Thread's Artifact and file-change index and MUST NOT be labeled or described as the global Artifact inventory.
...
A callable included operation whose owning capability is not release-ready MUST NOT be presented as a working feature. Its exact gap remains unpublished. Automations stay unpublished while they only persist in-memory definitions and do not operate recurring work; R092 remains the owner. Knowledge proposal drafting and reversal stay unpublished until ordinary-user reads can recover the exact candidate and accepted application lineage after restart; R070 and R072 remain their owners. Archive transport stays unpublished until the Web portability surface owns export and import progress; R008 remains its capability owner. Workspace deletion and deleted-Workspace recovery stay unpublished until the remaining multi-user Web projection supplies those controls; R049 remains their Web owner. Channels remain unpublished internal review shells because they have no included catalog operations to project. Native Generative UI is published in-thread: Chat renders `generative-ui-reference` Items through the admitted native catalog, while the `/generative` fixture shell stays unpublished Tier C. Kernel create/list/write operations remain agent-first Chat workflow dispositions. The [Generative UI Interaction](20260908-generative_ui_interaction.md) contract owns presentation and interaction independently of Kernel.

No published Workspace route MAY call a server-scoped API or require deployment-admin authority. Workspace General MUST NOT call deployment diagnostics or runtime-config routes. The distinct `/settings/configuration` and `/settings/ai-interface` deployment-admin routes use the current session client; NanoCore alone decides whether the session's active User owns usable deployment-admin authority, and Web MUST NOT request, recover, persist, or replay a `server-admin` bearer credential. A typed authorization denial remains visible with an explicit retry and MUST NOT render a Token-entry fallback.

### Recurring Workflow Projection

R092 uses the [Recurring Trigger contract](20260711-scheduler_recurring_event_triggers.md). The initial surface configures a fixed interval and existing target Thread, supports create/edit/pause/resume/delete with current authority and expected revision, and reads persisted definition status, trigger history, and linked scheduler/Turn status separately after refresh or restart. Accepted-but-queued, admission failure, expiry and later execution failure must remain distinguishable. Admission history shows the typed failure code, including `queue_backlog_full`, so a busy schedule is distinguishable from denied authority or invalid input. Exact expired ranges display their interval, covered time span and count without unbounded expansion. The UI cannot replay a source/imported occurrence or retry execution by changing its trigger state. Automations remain unpublished until the durable path and these observations are implemented; accepting its specification alone changes no publication disposition.
...
### Deployment AI Interface Projection

The published Tier-A **AI interface** screen is the bounded provider slice of R048. It projects the fixed OpenAI Codex and xAI subscription inventory and complete account lifecycle through `client.providerSubscriptions`, redacted provider and model diagnostics through `client.app.getDiagnostics`, authored Provider and server-default JSONC through `client.runtimeConfig`, and API-key store or replacement through `client.app.setProviderApiKey`. Raw API keys remain submit-only form state, and device-code login displays only the provider-owned verification URL and user code returned by the typed client. A typed mutation response is the minimum success proof; when a separate redacted read model exists, Web refetches it before presenting derived state. The API-key route intentionally exposes no configured-secret read model, so its typed redacted mutation response is the sole success proof and Web never attempts credential readback.
...
- Every `roadmap` disposition names an existing Roadmap item. Automations name R092, Knowledge proposal drafting names R070, Knowledge proposal reversal names R072, deferred archive transport names R008, and deferred Workspace deletion and recovery controls name R049. Server-scoped operations are excluded rather than given ordinary-user dispositions, and R048 remains their Web owner.
...
- **Navigation and server state:** After successful Workspace discovery validates the selected Workspace, Repositories appears once in the compact sidebar shortcut grid alongside the Settings gear and never as global primary or Settings navigation; without a validated Workspace, that destination is absent. TanStack mutation data is the sole server-state owner for the current approval response; React state, Zustand, browser storage, and list-row inference must not copy or reconstruct it.
...
- Goal plan approval and revision, steering input, Artifact Review decisions, Knowledge creation, Workspace creation, and Workspace rename use current Core Client operations. The single Settings gear opens the shared Settings shell directly; its Workspace group exposes General at `/workspace` only for a validated selection, and failed mutations preserve the last authoritative server projection rather than creating browser-owned workflow truth.
- General Settings never calls deployment diagnostics or runtime-config routes. The live Tier-A AI interface uses the bounded deployment-admin gate to project provider subscription accounts, provider and model diagnostics, authored provider and default-model configuration, and provider API-key store or replacement. R048 remains open for the broader server administration surface.
- Automations remain unpublished because the current API owns in-memory definition CRUD and does not execute recurring work; publishing that screen as an operating automation feature would be false, and R092 owns the real capability. Channels remain an internal unpublished review screen. Generative UI Chat Items of type `generative-ui-reference` render through the official A2UI v0.9 host; the `/generative` fixture shell remains unpublished Tier C and is absent from Web navigation.
- The live Tier-A board-19 **Repositories** screen currently projects selected-Workspace repository resources, diagnostics, default-repository setup, durable push records, and the existing approval-gated Git push workflow through `client.repositories`. Its consumed-record tuple and authoritative settlement behavior are implemented; navigation places it exactly once in the compact sidebar shortcut grid alongside the Settings gear, and TanStack mutation data is the sole server-state owner for the current approval response.
...
- The live Tier-A Configuration screen projects the existing runtime-config file tree, exact JSONC read and edit, server validation diagnostics, optimistic revision save, and explicit safe reload. Local mode uses its implicit authority; server mode uses the current session client and NanoCore's Token-derived deployment-admin decision without accepting a bearer credential in Web.
- The disposition guard accounts for all 147 included operations. Eleven remain roadmap-only: the four Automation CRUD operations under R092, `draftKnowledgeProposal` under R070, `reverseKnowledgeProposal` under R072, the three deferred Workspace archive operations under R008, and `deleteWorkspace` plus `recoverDeletedWorkspace` under R049. No other included operation remains roadmap-only.
...
- Work Resource Class 2 and Work Resource Class 3 resources, grounded annotation, text-range patching, and generalized locator controls remain deferred under their owning specification and are not implied by the 147-operation catalog. The React app remains a projection and is not the canonical source of Core semantics.
- The shared `Composer` has a bounded auto-growing input region with an approximately two-and-one-half-line minimum, a bottom action row ordered as Artifact/upload, Agent target, logical model, and send, and keyboard plus screen-reader labels. It consumes the Workspace conversation-target catalog, offers internal roles, warm and running Workers, and new Task Worker supply, submits Artifact references and logical model IDs through `client.app.submitConversation`, and preserves the exact draft and request identity after failure. It never exposes Provider, route, Sandbox, Harness, or AgentSession identity.

## Alternatives Considered

A raw API console or operation-id catalog was rejected because it would make Web a second route browser rather than a product projection. One published page per operation was rejected for the same reason. Leaving the previous "exactly these reads" ceilings as accepted scope was rejected because those ceilings contradicted ordinary-user completeness. Publishing Automations from the current in-memory definition store was rejected as a false recurring-workflow product; R092 remains the owner. Treating a Better Auth session alone as deployment-admin authority remains rejected; NanoCore may derive that authority from a currently usable `server-admin` Token owned by the active session User, while R048 retains the broader administration outcome.
...
- Package checks MUST show that no published Workspace route calls a server-scoped API or requires deployment-admin authority, and focused Configuration checks MUST prove its Token-input-free denied state, session-client read, file-tree projection, validation, revision-protected save, and explicit safe reload.
- Automations MUST remain unpublished in routing and navigation while R092 owns the real capability.
- This specification's Current Implementation Projection MUST remain reconcilable with the actual Web diff, including the eleven deliberately unpublished roadmap operations.

## Resolved Decisions

- Web UI work follows stable NanoCore, App API, protocol, and core docs.
- Ordinary-user Web completeness is catalog-derived: 147 included operations, each with exactly one `live`, `workflow`, or `roadmap` disposition.

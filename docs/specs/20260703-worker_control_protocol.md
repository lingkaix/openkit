...
This spec defines `openkit-worker-control-v2`, the minimal end-to-end control protocol between governed worker containers and NanoCore, independent of the transport intermediaries that carry it.
...
V2 serves the one-process, one-configured-NanoHost shared-runtime profile with multiple compatibility-keyed Harnesses, multiple open AgentSessions for distinct Threads, and bounded concurrent Turns across distinct AgentSessions. It preserves exact same-worker continuity across a bounded NanoCore restart or outage through predecessor-fenced NanoHost reconnect; it does not provide AgentSession replacement, transparent failover, or distributed control availability.
...
The command poll cadence is implemented with independent heartbeat and bounded-turn command loops. The command loop measures each interval from request start and waits only the remainder of 1000 ms after a successful response; session-continuity Turns omit it and use the separate private Harness poll.
...
- `packages/worker-shim` is the current Sandbox Integration and owns both the credential-free private Harness pull/result client and the Sandbox-side bearer-authenticated per-Turn control client carried through the same nested HTTP/2 bridge and NanoHost route projection. It instantiates one adapter-selected Harness for each `harnessInstanceId`, keeps operation sequencing independent per Harness, and uses the existing shared process-group supervisor for at most one active Turn in each Harness.
...
- Tests cover token binding, lineage checks, heartbeat recording, artifact notices, interrupt polling and acknowledgement, candidate event append, the 64 KiB typed control-envelope cap, the 256 KiB live event append cap, process-local sequence replay handling, durable sequence-fingerprint replay, complete-lineage collision-resistant command ids, exact replay without replacement, one-interrupt admission, lease-race delivery and acknowledgement, terminal drain rollback, rejected-evidence quarantine, and live lease-backed restart rebuilding with an active-or-terminal command sequence high-water mark.
...
The projection remains Partial because the current combined candidate still lacks the required exact-candidate A2 OpenCode success and restart/reconnect interrupt evidence. Terminal lease release now drains matching queued or delivered command rows to `undeliverable` inside the existing immediate scheduler transaction.
...
The two exact Harness-control paths run before or outside a Turn and therefore MUST NOT accept a worker-control, inference, capability, NanoHost, Gateway, Provider, Vault, User, or server-admin bearer credential. Their authentication is the conjunction of the current exclusive Integration-client nested H2 connection, its NanoHost-owned bridge and Sandbox Integration binding, the current authoritative outer NanoHost connection, and NanoCore verification of the exact current private `sandboxIntegrationBindingRef`. Integration sends no `Authorization` header and cannot supply the binding ref or a Harness selector. NanoHost rejects client-supplied `authorization`, `x-openkit-integration-binding`, or `x-openkit-harness-binding`, then adds exactly one `x-openkit-integration-binding` value from its private bridge context on the outer projected request; NanoCore accepts that header only from the authenticated authoritative NanoHost transport and rejects it on synthetic, public, direct, stale, predecessor, wrong-Integration, or ordinary worker-control requests. The ref is opaque non-secret routing metadata, not a credential, AgentSession identity, independent grant, or fallback token. NanoCore places one exact private `harnessInstanceId` in each command response only after matching it to the admitted Integration descriptor set.

The worker-control token is accepted only by `/worker-control/*`. Two distinct independently generated tokens authenticate `/inference/*` and the exact AEP-enabled selected-MCP `/capabilities/*` routes respectively; none is derived from or equal to the non-secret `sandboxBindingRef` or another route token. Worker control, inference, and capability MUST each reject the other two token families. A package with no selected MCP supply keeps capability disabled and receives no callable route. Shared RelayStream, HTTP/2, NanoHost transport, process, host placement, or restart MUST NOT create token reuse or shared authority.
...
Each active Turn's three raw route tokens remain live-memory only in NanoCore, in NanoHost while carrying that exact admitted Turn operation, and at the AgentSession-local `turn.start` route boundary. Harness bootstrap, Harness-global argv and environment, and Sandbox-wide files contain none. The fixed Turn launcher exposes the worker-control token only to the AgentSession-local Worker Shim control client, the inference token only through its separately authorized sanitized native-Agent binding, and the capability token only through the sanitized native-Agent binding when the exact AEP enables a selected capability route. Raw tokens never enter SQLite, the AEP, Context Package, retained command/result bytes, native configuration text or argv, diagnostics, stdout, stderr, transcript, evidence, ordinary logs, or a sibling AgentSession route. This supplies conversation-context isolation and route-authority separation; it does not claim security and adjudication isolation from a compromised shared Harness.
...
The accepted `schemaVersion` literal is `2` and is the only worker-control contract authorized by this design. This design adds no protocol-version negotiation, accepted-version set, build compatibility registry, legacy adapter, or rollback behavior. Version 1 and every other schema fail closed once this accepted envelope replaces the current implementation.
...
Every private `turn.interrupt` command has the required closed `purpose` value `interrupt` or `human-gate`. Existing product-interrupt callers use `interrupt`; only an exact MCP Approval Gate stop on the delivered Codex `session-continuity` path uses `human-gate`. Missing, unknown, malformed, or extra purpose input fails before a child signal, and purpose is never inferred from free text. Human-gate admission requires the exact Product Turn, Product AgentSession, Gate, and denied `CapabilityCall` to be durable before the Worker Control request, then one Core transaction verifies `agent_session_runtime_bindings.current_turn_id/current_lease_id`, the complete current active scheduler lease lineage, absence of an accepted final status for that lineage, and the free Harness-operation slot while queuing the existing operation. The Product Turn, Product AgentSession, Gate, and `CapabilityCall` remain preconditions outside that Core transaction and are neither verified nor committed by its compare-and-set; no cross-store atomicity, second command, settlement record, or bounded-turn fallback is implied. The caller waits only for durable enqueue and returns its denial; existing asynchronous operation settlement owns dispatch, exact result, non-redelivery, restart uncertainty, and cleanup, and terminal cleanup waits for that settlement before `session.inspect`. Terminal cleanup applies the same settlement ordering to `purpose: interrupt`, including when the accepted terminal status arrives before the admitted interrupt result.
...
- `session.close` revokes the exact native continuation authority and removes only disposable AgentSession control/request slots after collection, preserving retained native and working data;
...
Sandbox Integration may keep at most one Harness-control poll open per declared Harness while no operation for that Harness is unsettled. Each poll has the exact body `{ "schemaVersion": 2 }` and selects no Harness. After NanoCore returns no work, polling start-to-start remains at least 250 ms and at most 1000 ms apart per available Harness; the maximum remains mandatory while a Turn is active so private `turn.interrupt` retains its delivery budget. NanoCore returns empty `204` when no operation is available or one `200` command with exactly these fields:

```text
schemaVersion = 2
harnessInstanceId = exact NanoCore-selected private Harness identity
operationId = lowercase 64-hex identity
sequence = that Harness record's next expected non-negative safe integer
operation = one of the six fixed literals
body = exact operation-specific object
```

`operationId` is the lowercase SHA-256 of the Sandbox Integration binding, Harness Instance identity, per-Harness operation sequence, operation literal, and canonical redacted body identity derived by the existing NanoCore runtime owner. For `turn.start`, the redacted identity contains the three durable route-token hashes rather than any raw token. NanoHost and Integration cannot select the Harness, id, or sequence. NanoCore permits exactly one queued or dispatched operation per Harness, stores that state with the private `HarnessInstanceRecord`, and advances that Harness sequence only after one exact result settles. Different Harnesses may each have one unsettled operation and progress independently. A queued operation stores only admitted non-secret lineage and body references. At `turn.start` dispatch, one checked transaction mints the three mutually distinct raw route tokens, persists their separate hashes on the exact lease, derives `operationId`, and marks the command dispatched before returning the one response containing those raw values; a crash after that commit is uncertain delivery and never regenerates or redelivers them. Other operation identities are fixed no later than dispatch by the same record transition. This is not the worker `interrupt` queue, a NanoHost effect, an App command, a general command table, or an offline backlog.
...
`turn.start` settles as soon as the exact AgentSession child process is running under supervision and its Turn-local route bindings are installed; it does not wait for native exit or `collectTurn`. A first Codex Turn returns `nativeHandleState="pending"` with a null digest, which leaves the next Harness sequence available for `turn.interrupt`. A later Turn may return `ready` only with the already-bound digest it is attempting to resume. After the first Turn reaches its terminal and collection barriers, `collectTurn` must establish the exact handle and a subsequent `session.inspect` must return `ready` with the same digest before NanoCore marks that binding reusable and idle. If the first Turn did not reach `completed`, or terminal collection and inspection cannot prove one exact handle, the binding is non-reusable and proceeds to exact `session.close` or the existing wider cleanup fence.

Integration records the accepted `harnessInstanceId`, `operationId`, sequence, canonical command fingerprint, and then the exact result in bounded per-Harness memory before any retry; it retains only the current or immediately settled operation for each declared Harness and no general queue or history. Exact result replay with the same Harness, identity, sequence, and canonical body is idempotent across outer-session or bridge reconnect. Changed same-identity result, stale or future per-Harness sequence, a Harness outside the admitted descriptor set, or another operation for the same Harness while one is unsettled fails closed without blocking a compatible sibling Harness. NanoCore marks a command dispatched before returning its body and never redelivers a dispatched Harness effect; an incomplete delivery, accepted command without an Integration receipt, missing result after the existing outage bound, or lost per-Harness memory becomes `unknown`, stops admission to the affected Harness, and widens cleanup without replay. A queued command not yet dispatched survives NanoCore restart through the private Harness record because it contains no raw route token; a dispatched command survives only as waiting-for-exact-result truth and never regains raw `turn.start` tokens or execution authority.

The Sandbox Integration supervisor may retry only the same immutable poll or result request under the existing bounded route-outage budget. NanoHost and the Integration transport adapter do not retry it. After a predecessor-fenced bridge successor binds the same current Integration, each surviving Harness resumes at its exact unsettled sequence, first replays a retained result when one exists, and otherwise polls; a new or restarted Integration or Harness cannot adopt the old binding and requires drain, cleanup, and fresh admission.
...
Within each Harness, AgentSession lifecycle is `session.open`, zero or more sequential `turn.start` operations with optional exact `turn.interrupt`, then `session.close`; `session.inspect` is read-only proof and `harness.drain` is a per-Harness admission fence. `session.open` creates one AgentSession-private adapter control binding and selects a separately retained native data root only when the bound Thread has no other current resident AgentSession. When the pinned adapter creates its native conversation only with the first prompt, the open and first start results carry `nativeHandleState=pending`; the first successfully collected Turn plus subsequent exact `session.inspect` must bind and prove the native handle before that AgentSession becomes reusable. A later Turn may launch a new native process that resumes the same handle, but another AgentSession belongs to another Thread and has its own control binding, admitted working/native data slot, handle, child, route credentials and cleanup proof; explicit sequential related-work reuse follows the persistent-volume owner and never inherits live handle authority. A Turn starts only after fresh admission and consumes one active-Turn lease; it ends through existing final-status, output, evidence, route-revocation, child-absence, collection, and cleanup barriers before another Turn may use that AgentSession. Distinct AgentSessions may execute concurrently when their Harness and Sandbox capacity grants permit it.
...
In `bounded-turn` mode, worker `interrupt` asks the shared supervisor to terminate the supervised process group and the worker acknowledges only after it has initiated that generic termination. In `session-continuity` mode, private `turn.interrupt` invokes that same supervisor and returns `succeeded` only after exact process-group absence; it returns `refused` when the terminal owner already won and `unknown` when absence cannot be proved. `purpose="human-gate"` is the sole signal that lets Sandbox Integration report the interrupted child as `blocked/ask_user`; `purpose="interrupt"` retains the ordinary interrupted/aborted mapping. No native graceful-yield hook, free-text inference, double signal, cross-mode retry, or operation-result terminal state is part of either adapter contract.
...
A future supply refresh command may only be used when the resolved AEP declares support proved by the selected image and shared shim. V2 issues no live refresh request: same-snapshot lease renewal continues without refresh acknowledgement, while an incompatible AEP snapshot requires a new plan and lease after the current bounded step.
...
- Token bindings are resolved through the durable session lease records owned by `docs/specs/20260703-durable_scheduler_design.md`. That existing owner stores exactly three nullable lowercase SHA-256 projections, one each for the worker-control, inference, and capability tokens, and never a raw value. A family authenticator hashes the presented token and compares it to the exact current live lease/lineage binding; `stale`, fenced, terminal, or `releasing` lineage outside exact accepted `final_status` replay fails authentication and revokes all three through the existing lease status owner. This discharges restart durability through the scheduler's records instead of a parallel credential store.
...
- Control does not carry large payloads. V2 numeric payload limits are accepted below, and oversized payload rejection is mandatory before broad remote deployment.
...
- Harness delivery is an Integration-initiated two-path private pull protocol under `/worker-control/*`, authenticated by current nested-connection plus authoritative-NanoHost carriage and an injected non-secret `sandboxIntegrationBindingRef`; it accepts no bearer token or client Harness selector, carries at most one sequenced operation per NanoCore-selected Harness, retains no general queue, and is distinct from both the fixed NanoHost effects and per-Turn worker `interrupt` commands.
- Interrupt delivery is mode-exclusive: `bounded-turn` uses only the existing durable worker `interrupt` command row, while `session-continuity` uses only private Harness `turn.interrupt` with required purpose `interrupt` or `human-gate`; the existing Turn and lease terminal compare-and-set and shared process-group supervisor remain the single race and local-effect owners, and neither delivery path mirrors or falls back to the other.
- Worker-control, inference, and capability token-binding state is durable by contract: three separate lowercase SHA-256 projections resolve through scheduler lease records, queued commands and event high-watermarks remain their existing durable rows, and restart rebuilds serving state from those owners while raw tokens stay only with the surviving live path. No raw credential, shared token, or `sandboxBindingRef` derivation is durable.
...
- Previously open questions are resolved by accepted V2 defaults: control envelopes are capped at 64 KiB, individual live event payloads are capped at 256 KiB, and any larger material must move through the data plane as an artifact, evidence bundle, manifest, or collected file reference with typed oversized-payload diagnostics.

Current implementation projection: `apps/nanocore/src/scheduler-records.ts` stores and restores distinct lease-owned lowercase SHA-256 worker-control, inference, and capability token bindings, verifies Workspace/Thread/Turn/AgentSession/package-snapshot lineage, and rejects stale, releasing, terminal, shared-token, wrong-family, or `sandboxBindingRef` authentication. Raw tokens remain only on the live path, and no route family reuses another family's authentication. The lease owner also supports a live-to-`releasing` transition that preserves capacity until evidence collection completes. The scheduler dispatch helper derives the package snapshot id from the selected Turn and reserved AgentSession, keeping lease lineage aligned with AEP resolution. `apps/nanocore/src/runtime/worker-control-gateway.ts` registers the scheduler-owned binding, enforces the durable resolver, records supply refresh acknowledgements and capability summaries, enforces replay and sequence rules across every sequenced stream, and invokes the accepted-terminal hook. Server-scope sequence fingerprints preserve same-sequence conflict rejection across gateway instances. The App routes enforce the typed 64 KiB control-envelope and 256 KiB event limits, map accepted final status into canonical terminal evidence, retain future refresh acknowledgements without making them a V1 renewal requirement, quarantine rejected verification evidence, and hold the matching lease in `releasing` until the existing terminal owners finish. Active-AgentSession command enqueue accepts only `interrupt`, restart rebuild ignores acknowledged interrupts and every retired command kind, and governed launches use scheduler-owned lease lineage. The worker shim reaches these routes through Sandbox Integration over the stock bridge and nested HTTP/2; fixed unary bootstrap/response monitoring and the final-status, process-group-absence, Exit, and clean-response export barrier are implemented. The deleted Knowledge proposal-summary path remains absent. The implemented path now uses one Sandbox Integration binding, compatibility-keyed Harness Instances, and independent per-Harness operation sequences while preserving one active Turn per Harness. Both the public worker-control and private Harness envelope families carry `schemaVersion: 2`. Scheduler authorization for bounded concurrent active Turns across Harnesses, live supply refresh, and backend push remain separately gated future designs.
...
- L1-L2 cover envelope bounds, distinct token and lineage rejection, three hash-only durable bindings, raw-token and process-key secrecy, exact-next adoption without token reissuance or relaunch, final-status ordering, mandatory group absence, one clean monitor Exit, and the rule that Sandbox Integration suggestions are not product truth.
...
- L1-L3 interrupt coverage MUST prove that one product interrupt authorization selects exactly one delivery owner by immutable adapter mode, terminal-first creates no delivery, interrupt-first creates no duplicate or direct product terminal, cross-mode rows and requests fail before signal, `bounded-turn` acknowledgement and shared-Harness result uncertainty never fall back to each other, and both invoke the same process-group supervisor. Session-continuity coverage MUST also reject missing, unknown, malformed, or mismatched purpose before signal; prove that only an exact durable MCP Gate plus denied call can enqueue `human-gate`; prove that the enqueue response does not wait for child absence; and prove that only `human-gate` maps the stopped child to `blocked/ask_user` while ordinary `interrupt` remains interrupted/aborted.
...
- L1-L3 Harness carriage coverage MUST prove the two exact paths, no Authorization header or client Harness selector, rejection of client-supplied Integration or Harness binding, authoritative NanoHost Integration-binding injection, NanoCore-selected admitted `harnessInstanceId`, exact per-Harness sequence and operation identity, at most one unsettled operation per Harness, independent sibling progress, dispatched-command non-redelivery, exact result replay, changed replay rejection, pre-AgentSession admission, and boundary-correct unknown-outcome widening without a Harness-specific NanoHost effect or worker command.
- L1-L3 Harness coverage MUST prove at least two compatibility-distinct Harnesses and two distinct Threads with independent AgentSession-private control roots and handles plus separately addressed Thread-retained native data roots in one Sandbox, rejection of a second current AgentSession binding for one Thread, first `turn.start` settlement with a pending handle while `turn.interrupt` remains deliverable, terminal collection plus `session.inspect` handle establishment before reuse, a later process instance resuming the same handle, sibling-handle rejection, concurrent active Turns only across distinct AgentSessions within declared capacity, and exact close of one AgentSession or drain of one Harness without disturbing compatible siblings.
...
- Risk: A transport observer races the original worker because V2 has no server-fresh application challenge. Mitigation: require the authenticated confidential NanoHost transport plus predecessor fencing, keep the raw process key and all three route tokens out of durable state and logs, retain only three family-specific hashes in the exact lease owner, and fail closed on any connection, key, token family, lineage, sequence, or deadline mismatch.

...
The current implementation satisfies the closed V1 protocol semantics and the initial shared-Harness transport projection in local code and tests; it does not implement the V2 private multi-Harness envelope accepted below, and refreshed real-host and fault-acceptance evidence remains pending:
...
The one current `schemaVersion` literal is `2` and is the only accepted worker-control contract. This design adds no protocol-version negotiation, accepted-version set, build compatibility registry, legacy adapter, or rollback behavior. Version 1 and every other schema fail closed.

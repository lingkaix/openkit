...
`supply_refresh_ack` acknowledges a NanoCore supply refresh notice only when the runtime adapter and shim both declare safe refresh support.
...
A future supply refresh command may only be used when the runtime adapter and shim declare safe refresh support. Otherwise NanoCore should stop at a bounded step and relaunch with a new AEP snapshot.

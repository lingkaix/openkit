...
- The worker-visible control-plane protocol between a governed worker sidecar and NanoCore.
...
- NanoCore-to-worker command delivery semantics for approvals, interrupts, and allowlisted terminal diagnostics.
- The rule that worker-control records become product-visible only after NanoCore verification.
- Live control delivery and local session-file fallback requirements.
...
- Backend-specific relay mechanics beyond the worker-visible protocol contract.
...
- Support live delivery and data-plane fallback.
- Keep the worker sidecar subordinate to NanoCore canonical verification.

## Non-goals

- Do not carry large artifacts, patches, bundles, or raw logs over the control plane.
- Do not expose arbitrary shell execution.
- Do not let the sidecar mutate product state directly.
- Do not duplicate capability calls; use `capability.local` for privileged services.
- Do not define runtime-native transcript parsing in this spec.

## Background

`docs/specs/20260629-worker_runtime_communication_model.md` reserves `https://control.local/v1/worker-control` and requires every real worker container to run the OpenKit worker sidecar or an equivalent shim.
...
- `packages/worker-shim` owns the sandbox-side relay client. It sends bearer-authenticated requests with NanoCore-minted lineage to the configured relay upstream.
...
The endpoint is sandbox-local. The worker must not know the NanoCore host URL, OpenShell gateway URL, tunnel URL, or backend session URL.

The current NanoCore HTTP projection uses `/api/worker-control/*` behind the sandbox-local relay. That route shape is an implementation projection, not the worker-visible authority.
...
`command_ack` acknowledges delivery handling for non-terminal NanoCore commands such as approval results and interrupts.
...
`supply_refresh_ack` acknowledges a NanoCore supply refresh notice only when the runtime adapter and sidecar both declare safe refresh support.
...
`approval-result` resumes a worker after a human or policy decision.
...
A future supply refresh command may only be used when the runtime adapter and sidecar declare safe refresh support. Otherwise NanoCore should stop at a bounded step and relaunch with a new AEP snapshot.
...
The sidecar must send one final status for each bounded worker step.
...
- live append through `control.local`
- fallback collection from `/openkit/session/*.jsonl`
- mixed mode where heartbeat and notices are live but full transcript is collected later

If live append fails, the sidecar must continue writing local session files. NanoCore must collect and verify those files before closing the worker turn.
...
The sidecar can suggest item records, but NanoCore owns:
...
- The canonical worker-visible contract is one `control.local` endpoint with a typed operation envelope. Route-specific `/api/worker-control/*` paths are NanoCore relay projections and do not define a stable worker-visible protocol surface.
...
- Live append plus fallback collection tests.
- Oversized payload rejection tests.
- Command delivery tests for interrupt, approval result, and terminal diagnostics.
- Import tests proving sidecar-suggested items are not canonical until NanoCore writes them.
...
- Risk: Live failures lose work. Mitigation: require session-file fallback.
- Risk: Sidecar becomes a second Core. Mitigation: sidecar emits suggestions and evidence; NanoCore verifies and commits.

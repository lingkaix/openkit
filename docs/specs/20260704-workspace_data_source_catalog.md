...
- `@openkit/core-client` and `@openkit/mcp` can list, read, validate, update, and reload the data source catalog through the existing runtime config surface. MCP resource reads support `openkit://runtime-config/files/workspaces/<workspaceId>/data-sources.jsonc`.

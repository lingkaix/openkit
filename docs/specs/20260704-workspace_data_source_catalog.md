...
- NanoCore repository upserts accept a host `localPath` for the separate Core inspection/apply target and do not create or rewrite the runtime data-source catalog. Existing network locators and source status remain unchanged, including when the host apply target is missing. The former `locator.repositoryResourceId` projection is removed without a compatibility path.
- NanoCore startup does not backfill host-local repository resources into runtime catalogs. Configure remote Worker sources through the existing runtime-config surface with a credential-free network URL and explicit immutable commit; a host repository row alone cannot authorize launch or source-byte ingress.

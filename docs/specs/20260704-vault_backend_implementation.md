...
- `apps/nanocore/src/vault/vault-references.ts` and `apps/nanocore/src/vault/vault-grants.ts` now expose first persistent revocation cascade helpers. Revoking a reference marks the reference revoked, revokes dependent grants, revokes active injection plans, and marks active injection receipts as `stale-session`; revoking one grant applies the same plan and receipt projection for that grant. This is the durable stale-session marking layer. OpenShell backend teardown now runs `sandbox provider detach` for every provider attached to the materialized sandbox, and active materialized OpenShell sessions can detach providers by matching revoked vault grant ids against AEP provider attachments.

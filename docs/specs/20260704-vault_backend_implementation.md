...
### Public Workspace Secret Administration

Deployment-admin actors may create workspace references from request-only material, rotate their material, revoke references, and create or revoke separate workspace host-push and GitHub Worker runtime-env grants. Workspace authorization still applies to the addressed Workspace. Workspace bearer Tokens and ordinary sessions without derived deployment-admin authority cannot mutate this administrative surface. Existing filtered reference and grant lists remain the redacted inventory authority.

`POST /api/app/workspaces/{workspaceId}/vault/secrets` accepts `{ secretKind, material }`, assigns a fresh reference id, stores encrypted backend version 1, then creates matching workspace Core metadata. `secretKind` is a bounded lowercase identifier; it is a classification, not a credential value. `POST .../secrets/{referenceId}/rotate` accepts only replacement `{ material }` and preserves reference and grant identities. `POST .../secrets/{referenceId}/revoke` destroys backend material before applying the existing Core reference/grant/plan/receipt revocation cascade. Revoked ids remain redacted history and are never reused or physically deleted from Core.

`POST .../vault/grants` accepts `{ referenceId, expiresAt?, injectionPath? }` for a matching active workspace reference. Omitted or `gateway-only` injection creates a fresh workspace-lifetime grant limited to `gateway-only` and `workspace.git.push`. Explicit `runtime-env` injection accepts only a `github-token` reference and creates a different workspace-lifetime grant limited to `runtime-env`, with no capability target; it is worker-visible and cannot substitute for host push. Each creation uses a new grant id, preserves the reference and its current material version, and adds no binding automatically. An expiry must be in the future. `POST .../vault/grants/{grantId}/revoke` revokes either workspace grant and dependent injection state using the existing Core lifecycle. Grant revocation needs no material access and remains available while the backend is locked. This slice does not create runtime-file grants, server/user reference management, a generic grant-policy editor, or automatic binding.

Host GitHub push binds only the gateway-only grant through the existing repository `git.vaultGrantRef`. The `git-push:github-token` host path resolves it into host-process `GITHUB_TOKEN` only after current grant and push approval validation. An Agent Manifest `requirementId: github-token` declaration with `visibility: runtime-env` and `targetEnvVarName: GITHUB_TOKEN` uses the separate Worker grant through a Workspace `credentialBindings` entry. Turn admission validates the exact grant, reference, backend scope, material version, and health before worker-visible injection; Vault use and injection evidence remain redacted. Revocation makes later Turns ineligible and follows the existing stale-receipt/session lifecycle for already injected material. A host-push grant does not authorize sandbox environment injection, and a Worker grant does not authorize host push.

Each successful transition emits redacted administrative audit evidence. Secret input is confined to the immediate request and backend store/rotate call; responses expose only the existing redacted reference or grant shape and use `Cache-Control: no-store`. CLI material arrives in stdin JSON, never argv. Web uses a password field cleared on submission, discards mutation responses, and retains no material in query/mutation caches, browser storage, URLs, or error objects.

Missing or wrong-scope records fail without mutation. Rotation and either grant creation require exact active Core/backend identity, owner, locator, and version agreement. A non-GitHub reference requested for runtime-env, unsupported injection path, past expiry, locked or unavailable storage, and any inventory disagreement fail without a grant. Turn injection also fails closed on wrong scope, version disagreement, revoked or missing material, and locked or unavailable backend; it does not expose material to the Worker on failure. Ordered backend and Core writes are not a transaction: a failed fresh metadata insert triggers best-effort destruction of newly stored material; other partial or interrupted effects return a fixed redacted failure requiring inventory inspection before a new request. Neither restart nor retry reconstructs metadata, advances a mismatched version, revives a revoked id, or automatically repairs an unknown result. Grant-only revocation uses one Core transaction. There is no new durable state family or background lifecycle.

Acceptance is observable through synthetic material round trips, version advancement, exact workspace and admin denial, gateway-only grant use through approved host push, separate runtime-env grant creation and successful bound Turn injection, refusal of that public Worker grant as a host-push credential, Turn-injection refusal on backend scope/version/health disagreement, destructive revocation, and absence of the canary from inventory, public push responses, audits, and Web caches.

...
`POST .../vault/grants` accepts `{ referenceId, expiresAt? }` for a matching active workspace reference and creates a fresh workspace-lifetime grant limited to `gateway-only` and `workspace.git.push`. An expiry must be in the future. `POST .../vault/grants/{grantId}/revoke` revokes that workspace grant and dependent injection state using the existing Core lifecycle. Grant revocation needs no material access and remains available while the backend is locked. This slice does not create runtime-env or runtime-file grants, server/user reference management, a generic grant-policy editor, or automatic binding.

Host GitHub push binds the resulting grant through the existing repository `git.vaultGrantRef`. The `git-push:github-token` host path resolves it into host-process `GITHUB_TOKEN` only after current grant and push approval validation. Agent Manifest `requirementId: github-token` and Workspace `credentialBindings` remain the separate worker-declaration contract; a host-push grant does not authorize sandbox environment injection.
...
Missing or wrong-scope records fail without mutation. Rotation and grant creation require exact active Core/backend identity, owner, locator, and version agreement. Locked storage fails closed. Ordered backend and Core writes are not a transaction: a failed fresh metadata insert triggers best-effort destruction of newly stored material; other partial or interrupted effects return a fixed redacted failure requiring inventory inspection before a new request. Neither restart nor retry reconstructs metadata, advances a mismatched version, revives a revoked id, or automatically repairs an unknown result. Grant-only revocation uses one Core transaction. There is no new durable state family or background lifecycle.

Acceptance is observable through synthetic material round trips, version advancement, exact workspace and admin denial, grant use through the existing approved host push, destructive revocation, and absence of the canary from inventory, public push responses, audits, and Web caches.

...
- Scope homing rules for server- and Workspace-scoped secret material stored within one deployment.
...
- One NanoCore deployment is one Vault trust domain. Within it, server-scoped and Workspace-scoped references MAY live in one physical backend (one keychain namespace, one encrypted store), but every entry MUST carry its owner scope in metadata and AEAD-protected entry headers.
...
- The current Workspace vault routes use the accepted multi-user boundary: deployment-level status, lock, unlock, and bootstrap remain administrator-only, while every Workspace route requires normal current membership, fixed-role policy, and token intersection. Workspace reference list/rebind and non-secret grant, injection-plan, and injection-receipt listing are owner-only under `vault.admin`; redacted VaultUse history remains under `audit.read`; editor `vault.use` authority does not imply metadata administration, and viewers receive none of these operations. Stage 8 adds no Workspace grant issue or revoke operation.

...
- The admin Vault API exposes redacted status, unlock, lock, Codex auth JSON bootstrap, VaultUse readback, and Workspace reference discovery or rebind with server-admin and Workspace authorization, rate limiting, secret-input-only request fields, and linked audit evidence.

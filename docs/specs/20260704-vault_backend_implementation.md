...
- Key management for the encrypted-file backend: master key sources, envelope encryption, unlock model, and lock-state behavior.
...
- Give desktop-embedded local mode OS-native keychain integration with the encrypted store as fallback.
- Define the master key source order, the locked state, and the unlock operation so boot never blocks on vault availability.
...
The current implementation resolves provider credentials through `env:` secret references, which the vault spec accepts only as a first-slice projection. `docs/specs/20260703-openshell_mechanism_internalization.md` now needs a real backend: derivation flows credential values from "the vault backend" directly into OpenShell provider operations and projects rotation expiry into gateway credential expiry. Nothing behind that phrase exists yet.
...
2. Backend defaults follow deployment mode: `os-keychain` is the default for desktop-embedded local mode; `encrypted-file` is the default for server mode and the fallback on desktop when no usable keychain service exists.
3. The encrypted-file backend uses envelope encryption: one master key wraps per-entry data keys; each entry is sealed with AEAD binding the vault reference id and version as associated data. The first implementation uses Node stdlib `aes-256-gcm`; libsodium/XChaCha20-Poly1305 remains a later hardening option only if the product needs that primitive. The master key derives from a passphrase via Argon2id or is supplied as a raw key file.
4. Master key sources resolve in a fixed order; when none is available at boot, the vault is `locked`, boot proceeds, vault-dependent operations fail typed, and readiness is degraded. Unlock is an authenticated admin operation.
...
- A NanoCore deployment MAY run both backends concurrently. Each `VaultReference` binds to exactly one backend kind at creation; moving material between backends is an explicit re-store operation that MUST preserve the reference id, reset versions, and emit audit.
...
- When the keychain service is unavailable (headless session, locked login keychain, missing Secret Service), the backend MUST report `locked` or `unavailable` truthfully and MUST NOT silently fall back; fallback to `encrypted-file` is a configuration-level default, not a per-call runtime switch.

### encrypted-file Backend

- The `encrypted-file` backend is the default for server mode and the configured fallback on desktop.
...
- Cryptography uses envelope encryption:
  ...
  - Master key derivation: Argon2id from an operator passphrase, with per-store salt and parameters recorded in the store header; or a raw 32-byte key file supplied directly.
- Per-entry envelopes are the unit of write: rotation, revocation, and export of one entry MUST NOT rewrite or re-encrypt the whole store.
- The store header records format version, KDF parameters, salt, and a master-key verification value that allows unlock verification without decrypting entries. The header MUST NOT allow recovering the master key.
...
The encrypted-file backend resolves its master key from these sources, in order; the first available source wins:

1. An OS keychain entry holding the master key (desktop deployments that want the encrypted store unlocked by the login keychain).
2. A key file with 0600 permissions at a path named in server config. A key file with broader permissions MUST be rejected with a typed diagnostic.
3. An interactive passphrase prompt, when a controlling terminal or admin surface is available.
4. An environment variable. This source is allowed but discouraged: using it MUST emit a warning diagnostic naming it a development-grade key source.

Lock-state rules:

- If no source is available at boot, the vault is `locked`. Boot MUST proceed per `docs/specs/20260704-nanocore_bootstrap_readiness.md`: vault-dependent operations fail typed with `vault-locked`, and readiness reports the vault subsystem degraded.
- Unlock is an authenticated admin operation that supplies a passphrase or key material, verifies it against the store header, and transitions the backend to `available`. Failed unlock attempts MUST be audited and rate-limited.
- Lock is also an admin operation: it discards the in-memory master key and returns the backend to `locked` without touching stored entries.
- The master key and unwrapped data keys live only in process memory and MUST never be written to logs, diagnostics, SQLite, or disk outside the key file the operator configured. Guarded-memory hardening is deferred until a concrete dependency is accepted.
...
- `store`, `rotate`, `revoke`, unlock, lock, and cross-backend re-store are audit events in their own right, homed per `docs/specs/20260703-audit_usage_evidence_records.md`.
...
- An explicit encrypted vault export exists as a separate admin action. It re-encrypts exported entries under a distinct export passphrase (Argon2id-derived, never the store master key), records the export as an audit event, and is never included in workspace or server backups implicitly.
...
The backend layer is a NanoCore-internal module with a small registry: deployment config names the default backend per mode, and each `VaultReference` row records its backend kind. `os-keychain` is implemented over the platform secret service APIs behind one adapter interface per platform. `encrypted-file` is a directory-per-entry store (`server/vault/entries/<referenceId>/<version>.enc` plus a store header file), so per-entry envelopes map directly to per-file writes and atomic renames.

Unlock state is a process-level service that owns the master key in guarded memory and exposes lock-state to readiness. The audit emitter wraps the boundary so no backend implementation can forget `VaultUse` emission.

## Current Implementation Projection

- `VaultReference`, `VaultGrant`, `InjectionPlan`, and `InjectionReceipt` exist as first-slice durable non-secret server metadata in `core.sqlite`, and `VaultUse` exists as first-slice durable non-secret metadata in the scope-owning SQLite database. Workspace-scoped `VaultGrant`, `InjectionPlan`, and `InjectionReceipt` records now have App API, Core Client, and MCP readback through workspace-filtered non-secret surfaces. AEP vault references and grants exist in `packages/config-schema/src/agent-environment.ts`.
- `apps/nanocore/src/vault-backend.ts` defines the NanoCore-internal `VaultBackend` boundary, typed `VaultBackendError` codes, redacted backend health projection, non-secret inventory metadata, owner-scope metadata consistency validation, and a locked backend implementation whose material operations fail with `vault-locked` without carrying secret material in errors.
- `apps/nanocore/src/vault-key-file.ts` implements the encrypted-file backend's raw key-file source validation: only regular files with exact `0600` permissions and exactly 32 bytes are accepted, and invalid key-source failures use typed redacted `VaultBackendError` diagnostics.
- `apps/nanocore/src/vault-store-directory.ts` implements the encrypted-file store directory permission boundary, and `apps/nanocore/src/storage/fs-layout.ts` now routes `server/vault/` creation through it: newly created store directories are forced to `0700`, empty broad-permission directories are tightened before material exists, and existing non-directory or non-empty broad-permission paths fail typed without leaking local paths.
- `apps/nanocore/src/vault-encrypted-file-store.ts` defines and verifies the encrypted-file store header and per-entry envelope format, writes `header.json` and `entries/<referenceId>/<version>.enc` with owner-only files, derives associated data from reference id and version, uses same-directory temp-file rename writes, and now seals/opens caller-supplied material with per-entry data keys wrapped by a raw 32-byte master key using Node stdlib `aes-256-gcm`. It is still a store helper, not an unlocked backend service.
- `apps/nanocore/src/vault-encrypted-file-backend.ts` creates an unlocked encrypted-file `VaultBackend` over a caller-supplied raw 32-byte master key. The current slice implements `health`, `store`, `resolve`, `rotate`, `revoke`, and `listReferences` over encrypted entry files plus per-reference `state.json` records for current version, prior-version expiry, non-secret inventory metadata, version count, and revoked state. Store calls reject metadata whose scope-specific ids do not match the declared owner scope before writing material, and stored state parsing fails closed on the same inconsistency. Rotation writes a new current version, records grace expiry timestamps for prior versions, keeps explicit prior-version resolution available until expiry, returns typed `version-expired` failures after grace, and destroys expired prior-version entry files when an expired resolve is attempted. Revocation now preserves non-secret inventory in state, marks every version as unavailable, and destroys encrypted entry files for every known material version.
- `apps/nanocore/src/vault-os-keychain-backend.ts` creates an `os-keychain` `VaultBackend` over a platform keychain adapter. The default macOS adapter uses the built-in `security` CLI for generic-password reads, upserts, and deletes; the Linux adapter uses the freedesktop Secret Service `secret-tool` CLI for reads, writes, and deletes; the Windows adapter uses PowerShell plus the Windows Credential Manager API for generic credential reads, writes, and deletes while passing secret-bearing payloads through stdin instead of argv. Platforms without a native adapter truthfully report unavailable. The backend stores one secret-bearing keychain item per vault reference and one non-secret index item per deployment namespace, supports `health`, `store`, `resolve`, `rotate`, `revoke`, and `listReferences`, rejects inconsistent owner-scope metadata before writing material, preserves non-secret inventory after revocation, expires prior versions after rotation grace, and keeps secret material out of the inventory index.
- `apps/nanocore/src/vault-references.ts` and `apps/nanocore/src/vault-grants.ts` now expose first persistent revocation cascade helpers. Revoking a reference marks the reference revoked, revokes dependent grants, revokes active injection plans, and marks active injection receipts as `stale-session`; revoking one grant applies the same plan and receipt projection for that grant. This is the durable stale-session marking layer. OpenShell backend teardown now runs `sandbox provider detach` for every provider attached to the materialized sandbox, and active materialized OpenShell sessions can detach providers by matching revoked vault grant ids against AEP provider attachments.
- `apps/nanocore/src/vault-unlock-state.ts` provides the first process-local vault backend state holder. For `encrypted-file`, it starts with a locked backend projection, accepts raw 32-byte key material to create an available encrypted-file backend, and discards the available backend on lock so subsequent operations fail typed with `vault-locked`. For `os-keychain`, it starts directly from the platform keychain adapter, stays available or unavailable according to adapter health, and does not accept encrypted-file unlock keys.
- `packages/app-api-schemas/src/vault-admin.ts` defines the redacted App API contract for vault admin status, unlock, lock, Codex auth JSON bootstrap, and workspace vault-reference discovery/rebind operations, including both `encrypted-file` and `os-keychain` backend kinds in status payloads. `apps/nanocore/src/app.ts` exposes `GET /api/app/vault/status`, `POST /api/app/vault/unlock`, `POST /api/app/vault/lock`, `POST /api/app/vault/bootstrap/codex-auth-json`, `GET /api/app/workspaces/:workspaceId/vault/references`, and `POST /api/app/workspaces/:workspaceId/vault/references/:referenceId/rebind`; these routes inherit server-mode Better Auth protection from the existing `/api/*` middleware, accept raw 32-byte key material, Codex auth JSON, and rebind material only as base64 request input, never echo submitted secret material in responses, rate-limit repeated failed unlock attempts per actor in the process-local window, and write server-scope `vault_admin_audit_events` rows plus linked server `AuditEvent` rows for unlock/lock/bootstrap success, unlock failure, bootstrap failure, and unlock rate-limit denial when Core DB storage is configured. `@openkit/core-client` exposes these routes under `client.app`, and `@openkit/mcp` exposes the same public routes as `openkit.read_vault_admin_status`, `openkit.unlock_vault_admin_backend`, `openkit.lock_vault_admin_backend`, `openkit.bootstrap_codex_auth_json_vault_reference`, `openkit.read_workspace_vault_references`, and `openkit.rebind_workspace_vault_reference`, with submitted base64 secret material redacted from MCP tool responses.
- `apps/nanocore/src/vault-use-audited-backend.ts` wraps any `VaultBackend` so `resolve` success and typed failure create non-secret `VaultUse` rows in the scope-owning database. The wrapper records reference id, resolved version when known, backend kind, resolving path, actor context, outcome, failure code, and timestamp without storing secret material. Server- and workspace-scoped audited resolutions also create linked `AuditEvent` rows with `vault.resolve`, actor lineage, outcome, severity, and typed failure code. Before material leaves the backend, the wrapper rejects implicit cross-scope resolution when the stored reference owner scope differs from the audited caller scope and no explicit grant is attached; explicit grant-backed cross-scope use remains available and audited. Provider `secretRef: vault://<referenceId>` resolution now uses this wrapper with the `provider` resolving path and the vault-aware resolver no longer falls back to `env:` by default. GitHub MCP backend-handle package resolution uses the same wrapper with grant, plan, receipt, and agent-session linkage. Codex auth JSON runtime-file package resolution uses the wrapper with `grant_codex_auth_json`, creates a runtime-file plan and receipt, and passes the resolved file content only through backend-private materialization context for upload to `/sandbox/.codex/auth.json`. Future credential-use paths should use the same audited boundary.
- The server-owned `server/vault/` directory required by the encrypted-file backend now exists in the data-root layout. When an app has a data root and no explicit vault state override, NanoCore defaults local mode to `os-keychain` and server mode to `encrypted-file`; `server.jsonc` can set `vault.localDefaultBackend` to `encrypted-file` as the local-mode encrypted-file fallback, and tests can inject an `os-keychain` adapter or default-platform command runner for deterministic local-mode coverage. OpenShell refresh status evidence collection exists for the current CLI surface; scheduler-cadenced polling and refresh-log import for a stable upstream log surface remain deferred future work.
- `apps/nanocore/src/bootstrap/vault.ts` projects the boot-time locked state into readiness as `vault.locked`, blocking `vault.read`, `vault.use`, and `secret.inject` while leaving unrelated product work admitted. Runtime vault status, unlock, and lock are exposed through the vault admin App API.
- Provider credential resolution supports the first vault-backed shape, `secretRef: vault://<referenceId>`, for server-owned provider credentials. The OpenAI-compatible LLM Gateway routes, including `/v1/chat/completions`, the internal OpenAI-compatible facade, and Quick Chat internal-agent calls now resolve vault-backed provider credentials through the audited provider resolver and create non-secret `VaultUse` rows. Locked or otherwise failed vault-backed provider resolutions preserve the typed vault backend failure instead of collapsing into missing credentials; `/v1/chat/completions` now returns `vault-locked` with HTTP 423 for locked vault provider credentials and records the failed `VaultUse` row without calling the upstream provider. The provider registry no longer resolves `env:` references by default; `resolveEnvSecretRef` remains only as an explicit opt-in test or transition helper. Seeded provider templates and the DATA_ROOT config guide now document `vault://` provider references as the first-class credential shape, and runtime provider config projection now reports vault-resolved credentials as `apiKeySource: "vault"` instead of `env`; unresolved `secretRef` credentials report `apiKeySource: "missing"` rather than `not-required`. Host-side Git push no longer reads GitHub credential variables implicitly from NanoCore's global `process.env`; linked repository Git config can carry a non-secret `git.vaultGrantRef`, and the approved `workspace.git.push` route resolves that grant through the audited vault backend after push preflight and policy checks pass, creating injection plan, injection receipt, workspace-scoped `VaultUse`, and linked audit evidence before passing the token only as gateway-only child-process env. Future credential classes can reuse the audited backend boundary without blocking the V1 backend implementation.
...
- Server mode gets a real vault with no external dependency; desktop gets native keychain behavior with a consistent fallback.
...
2. Ship `encrypted-file` first (it serves both modes), with store creation, unlock, lock, and the master key source chain.
3. Ship `os-keychain` for desktop-embedded mode with the configured fallback.
...
- L0: repository checks that no source outside the backend module owns secret-material crypto helpers; lint rules rejecting secret-shaped field names in schemas that cross the boundary; generated-schema drift checks for backend error types.
- L1: crypto round-trip tests (store then resolve yields identical material across versions); AD-binding tamper tests (ciphertext copied to another reference id or version MUST fail decryption); Argon2id derivation vectors; key file permission rejection; store header verification accepting correct and rejecting wrong passphrases; atomic-write crash simulation leaving no readable partial entry.
- L2: contract tests that both backends satisfy identical `VaultBackend` conformance fixtures: typed errors, scope filtering in `listReferences`, `VaultUse` emission on success and failure, version addressing semantics.
- L3: NanoCore black-box tests: boot with no master key source reaches ready with degraded vault readiness and typed `vault-locked` failures; authenticated unlock transitions to available and failed unlock is audited and rate-limited; rotation grace (old version resolves within grace, fails typed after); revocation cascade (all versions dead, grants invalidated, affected sessions marked stale); cross-scope resolution without explicit context is denied and audited; no-plaintext-at-rest scans that grep the entire data root and SQLite files for planted secret values after a full store/rotate/resolve/export cycle and MUST find none outside the vault directory.
- L4: not applicable beyond existing redacted audit views; unlock admin UI tests arrive with that surface.
- L5: smoke test that a packaged server build creates the vault directory with correct permissions, locks and unlocks, and resolves one secret end to end.
- L6: one opt-in story: an operator configures a provider credential into the vault, a worker uses it through the derivation path, the operator rotates it, and audit shows the full reference-and-version trail with no secret value anywhere in product records.

Acceptance requires: both backends pass the shared conformance suite; tamper and no-plaintext scans pass; a locked vault never blocks boot; every resolution in L3 flows has a matching `VaultUse` row.
...
- Risk: the environment-variable key source becomes de facto production practice. Mitigation: persistent warning diagnostic, readiness annotation, and deployment docs naming it development-grade.
- Risk: keychain platform differences leak into the boundary. Mitigation: the shared L2 conformance suite runs against both backends; platform quirks stay inside the adapter.
...
- Risk: the encrypted store is accidentally captured by backups or sync tooling. Mitigation: the opaque-directory rule, export exclusion by default, and the L3 no-plaintext scan doubling as an export-content check.

## Resolved Decisions

Previously open questions are resolved by accepted V1 defaults: the encrypted-file store supports one active wrapped master key plus explicit encrypted export, while multi-wrapper recovery keys are deferred until operator recovery workflows are designed; `os-keychain` stores one item per vault reference with versions inside the encrypted payload to reduce platform prompt and quota variance.

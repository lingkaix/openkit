...
[Persistent Worker Volumes](20260910-persistent_worker_volumes.md) owns opaque data retention across environment replacement. Adapter close releases active native-handle authority and proves writer absence; it no longer removes retained native data. Per-Turn transport/control/capture cleanup remains separate. This changes filesystem lifetime, not the five-operation continuity contract, bounded-turn result parsing, or normalization/acceptance boundary.
...
Neither adapter mode has an adapter-returned config-artifact field. The shared Harness materializes only runtime-neutral AEP files and rejects any attempt to introduce adapter-authored files through a launch plan. A `bounded-turn` adapter receives fresh disposable Turn control/output slots and may use opaque retained native data without gaining a native continuation contract; a `session-continuity` adapter receives a private AgentSession control binding plus fresh Turn-local output slots, with native handle authority valid only until `closeSession`. Runtime-owned data lives separately in the admitted retained volume and survives either mode's close. OpenCode and Pi express native setup only through adapter-owned argv and safe environment bindings. The Codex adapter alone may write the secret-free Turn-private model capability descriptor owned by `20260716-codex_worker_adapter.md` and bind its path through startup argv; existing Turn-control cleanup removes it. This narrow exception grants no config-artifact field, shared native schema, retained configuration authority, or credential-file generation. Any additional generated native setup requires an amendment rather than an unowned Harness file envelope. Upstream MCP commands, endpoints, credential references, and credentials always remain absent from the worker. When selected MCP supply enables the capability plane, the Codex adapter projects only fixed loopback server URLs and the capability token environment-key name; no selected supply keeps those projections absent.
...
- allocate one private control binding per open AgentSession until exact close, allocate fresh disposable Turn-local output and credential slots there, and address native working data separately in the admitted retained volume
...
- `packages/worker-shim` provides one generic zero-argument `openkit-worker-shim` entrypoint, one static registry, the implemented `bounded-turn` `prepare` and `collect` contract for OpenCode and Pi, and the implemented five-operation `session-continuity` adapter contract for Codex. Its one concrete shared Harness admits independent AgentSessions for distinct Threads and runs at most one active Turn. Bounded-turn bindings admit one Turn, reject inspection and private interruption, close at terminal collection, and use the independent durable command poll; Codex retains only each AgentSession's restricted native handle, resumes the exact thread UUID in a fresh process, and uses private Harness interruption. Runtime-native argv, safe child environment, state-root selection, bounded result parsing, and selected fixed-loopback MCP projection remain adapter-owned. The shared shim has no native config-artifact contract, generic capability client, sidecar binary, or runtime-name fallback.
- Codex `0.153.4` and OpenCode `1.18.1` accept only the trusted NanoCore relay envelope. Pi `0.85.1` accepts only the exact direct Anthropic `claude-sonnet-4-5` envelope. The shared shim selects exactly one preferred route from a Gateway allowed set and preserves the single-route rule outside Gateway mode; every adapter rejects mixed or unsupported selected-route authority before child launch.
...
- The zero-argument Harness keeps one Integration client alive inside the bounded outage budget, may hold multiple mode-constrained AgentSession bindings, and launches one fresh native process for each admitted Turn; the production binary has no package-argument or one-shot compatibility path.
...
- `apps/nanocore/src/runtime/worker-control-gateway.ts`, `worker-control-records.ts`, `worker-control-sequences.ts`, `worker-control-commands.ts`, `worker-control-rejected-evidence.ts`, and `worker-control-rebuild.ts` provide version-2 control envelopes, complete-lineage command compare-and-set, lease-serialized delivery and acknowledgement, terminal drain, rejection evidence, and restart rebuilding for registered AEP snapshots; canonical worker records retain version 1.
...
- Runtime adapters own runtime-native argv, environment, isolated state paths, and output parsing; Codex additionally owns its narrowly specified Turn-private capability descriptor, while the shared harness has no native config-file contract
...
- base-path runtime-native command construction and event parsing occur only in the corresponding adapter, specification, and tests; images and manifests declare binaries and policy but no native argv; `bounded-turn` grants no native session continuity, while `session-continuity` keeps its restricted live handle in its private control binding with authority valid only until `closeSession`; opaque native bytes may persist in either mode's admitted data volume; the only NanoCore native-parser exception is the narrowly isolated version-pinned S33 verifier
...
- e2e smoke proves one configured NanoHost can run a bounded Goal Mode Worker step, produce reviewable evidence, close the exact disposable AgentSession control/request state while preserving retained native and working roots, and preserve the compatible shared Sandbox and healthy Runtime Epoch
...
- The AEP carries one preferred logical model and an exact non-empty allowed route set. The shared shim requires a unique route for the preferred model and passes only that route into the adapter-specific fail-closed native projection; it permits multiple routes only in Gateway mode and never falls back to another model. Codex `0.153.4` and OpenCode `1.18.1` can represent the selected trusted NanoCore relay within the current no-file contract; Pi `0.85.1` cannot and supports only an exact pinned native direct provider/model pair.
...
- The `bounded-turn` adapter contract is only `prepare` and `collect` and exposes no native continuation authority; it may preserve opaque native bytes under the retained-volume owner. The `session-continuity` contract is exactly `openSession`, `prepareTurn`, `collectTurn`, `inspectSession`, and `closeSession`, retains one restricted live handle only in its AgentSession-private control binding, and removes that binding at exact close while preserving the opaque data volume. Shared process-group termination owns interruption, and both modes use the 16 MiB capture and 16 KiB-per-stream diagnostic-prefix bounds.

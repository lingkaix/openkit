...
The AEP snapshot carries lineage, selected runtime, workspace inputs, generated files, Skill refs, inert MCP supply refs, a preferred logical-model ID, an exact allowed logical-model set, non-secret local Integration bindings and token references, the explicit disabled capability declaration, policy summaries, and backend capability requirements. Every worker inference request names one member of that immutable set; NanoCore validates membership at the worker inference boundary before Gateway dispatch. A missing or disallowed logical model returns the typed `worker_logical_model_not_allowed` failure without Provider effects, and neither Sandbox Integration nor the runtime adapter sees or selects a concrete route.

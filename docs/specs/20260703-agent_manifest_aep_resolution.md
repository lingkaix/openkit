...
- `apps/nanocore/src/runtime/agent-environment.ts` preserves the manifest-authored image, runtime binaries, sandbox envelope, adapter id, logical Gateway preference, and static supply through one generic OpenShell AEP resolver. It emits the fixed `openkit-worker-shim --package /openkit/config/package.json` command, and workers receive no private Provider profile, upstream model, route, or credential identity.

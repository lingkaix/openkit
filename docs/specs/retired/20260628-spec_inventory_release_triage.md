...
This one-time release inventory completed its cleanup role and is no longer a maintained source of classification truth. It is retired rather than superseded because the live directory layout and governance rules are operational controls, not a successor specification for this historical triage exercise.

## Retention Reason

This document preserves the first systematic spec inventory, its original classification criteria, and the cleanup backlog at that point in time so future maintainers can audit how the current lifecycle structure emerged.
...
Historical result: the old Web UI slice was first grouped as superseded. The lifecycle governance audit later moved it under `docs/specs/retired/web-ui-pre-rebuild/` because the module was removed and the current Web product surface is a clean-slate rebuild, not a contract-preserving successor.
...
Historical files were moved to `docs/specs/superseded/nanocore-config-identity/` and marked `Status: Superseded`.

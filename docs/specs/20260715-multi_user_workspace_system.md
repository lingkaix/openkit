...
| Removed member, invitee, or unrelated user without a presented usable server-admin bearer | No Workspace operation grant | No implicit content bypass |
| Presented usable server-admin bearer | Cross-Workspace operation eligibility, including Task execution | Current credential, private audience, effect policy, Vault and audit constraints |
...
authenticate credential and canonical actor
  ...
  -> resolve active membership or presented usable administrator bearer and required lifecycle identity
...
| `authorized-workspace-set` | Query active Core memberships for the authenticated user, or active registered Workspaces for a presented usable server-admin bearer, intersect token bindings and mutation posture, evaluate the declared operation for each candidate, and only then load candidate Workspace content. Physical Workspace discovery and load-then-filter are prohibited. |
...
Collection and global search operations must construct their candidate Workspace set from active membership or a presented usable server-admin bearer, token intersection, and policy. They must not list a user directory or scan every physical Workspace and filter after loading private records.
...
A currently usable presented `server-admin` bearer supplies deployment administration and cross-Workspace operation eligibility, including starting and executing Tasks without membership. Its owner must remain active and its current scope, ownership and usability must be rechecked. This does not grant access to another user's private Thread or private-derived content, satisfy human approvals, or bypass Vault grants, effect policy or sandbox constraints. Existing request and effect audit retain the responsible actor, exact credential reference and request lineage.
...
Durable Task execution retains only the non-secret presented server-admin Token reference on its existing admission. It revalidates the same credential through exact admission/Turn/lease lineage before later governed effects; revocation, expiry, changed ownership or scope, disabled User, missing or contradictory lineage denies the effect without falling back to membership. An internal or recurring trigger does not inherit a responsible user's unpresented administrator credential. The scheduler specification owns persistence and recovery details.

This exception does not change user-scoped operation contracts or manufacture membership. Explicit membership recovery remains a separate audited operation.
...
Deletion quiescence distinguishes runtime containment from unavailable historical result evidence. A `failed` scheduler lease may retain `recovery_state = needs-evidence` without blocking deletion only when its existing Worker backend record matches the exact lease, Workspace, Thread, Turn, AgentSession and package snapshot lineage, has `state = cleaned`, and records `physical_cleaned_at`. This does not settle or clear the unknown result. The scheduler lease recovery flag remains deployment-local in Core and is not rewritten or claimed portable; recovery export and audit closure preserve only their existing owned historical Turn, audit and evidence records without converting an unknown outcome to success. Missing or mismatched cleanup evidence, a non-cleaned backend, active AgentSession, unresolved apply, or `needs-evidence` on any other lease status continues to block deletion. Restart and exact-request retry re-evaluate those same authoritative facts; no new recovery record or automatic repair transition is introduced.
...
8. Remove the exact request-owned staging tree and dispose of the exact retained Worker storageRefs bound to this deletion request during fencing, under `docs/specs/20260910-persistent_worker_volumes.md`. The request records each ref/revision and confirmed, retained or unknown cleanup outcome without content or host paths. Its existing non-content deletion-retry authority permits only those exact fenced resources after ordinary Workspace authority closes; it cannot discover and purge unrelated roots. Recheck holds and prove writer absence before each purge. Success of all named cleanup records `cleaned`; failure or an unavailable execution host keeps phase `deleted`, returns the truthful retained-cleanup facts, and permits the same request to retry only those exact targets. Logical `deleted` never certifies full physical cleanup or execution-volume coverage in the recovery export.
...
Every implemented Stage 7 boundary uses one stateless `currentWorkspaceAuthority(workspaceId, actor, productOperation, effectAuthority)` predicate immediately before the NanoCore-owned effect. The predicate derives the responsible user only from `actor` and requires a non-null active canonical User and current Policy eligibility. Ordinary work requires active membership in the exact Workspace. Interactive human work authenticated by a presented usable server-admin bearer may instead use that exact credential; later Worker effects recover its non-secret reference from the exact existing admission/lease/Turn lineage and revalidate current ownership, scope and usability. Recorded invalid provenance denies without falling back to membership. `effectAuthority` is the existing owner-specific Approval, PermissionDecision, VaultGrant, review, or resource tuple when that boundary already requires one; the caller validates that tuple before passing the predicate, and the predicate does not become a second owner-aware policy engine. The tuple must be complete, target-matching, current-deployment authority and cannot replace the user, applicable membership or presented administrator credential, or Policy checks. For Vault use, the durable VaultGrant is the effect-authority owner: it must be target-issued and active, and every non-null user, Workspace, agent, AgentSession, and capability constraint must exactly match the current execution. Its optional Approval and policy-decision identifiers are immutable issuance lineage rather than use-time decision owners; when an Approval id is present it must be target-issued and the policy-decision id must be non-null, but effect execution does not re-run or reconstruct that workflow. A null responsible user, missing or contradictory fact, absent required membership or unusable recorded administrator credential, disabled user, denied Policy, or stale effect authority denies with zero governed effect. The predicate reads current Core and owning-domain authority directly for the small single-writer deployment and adds no cache, durable decision row, state, workflow, or recovery owner.
...
- Search results never include a Workspace outside the caller's current authorized Workspace set, including applicable membership or presented administrator bearer and token intersection.
...
- presented usable server-admin bearer access to active Workspaces without membership, exact private audience isolation, and same-deployment source-authorization checks before portable import collision or content reads
...
- Risk: removed users still act through tokens or workers. Mitigation: intersect current membership or the exact presented administrator credential on every NanoCore request and governed effect, interrupt sessions that have lost current authority at the next governed boundary, reject their publication, and retain the explicit in-flight worker-native compromise above.

...
- L1/L2 tests cover the five operation schemas, exact server-assigned callers, client-override rejection, authenticated actor separation, S61 delegation by both read operations, create-only proposal output, and absence of provider, runner, scheduler, or private lifecycle state.
...
- S18 alone owns the real G07 L6 story; this service spec authorizes no additional story or harness.

Acceptance: routes assign only `assistant`, `task-mode`, or `app-api` and reject client override; every call is request-scoped; both read operations use S61's single governed retrieval owner; public failures are typed and redacted; every proposal draft is create-only and durably fixes its target page id, bytes, digest, and sources through the existing owner; S61 retrieval evidence never counts as S39 delivery proof; and no observable result depends on a generic internal-agent runtime, private lifecycle, second retrieval owner, second context owner, or Goal integration.

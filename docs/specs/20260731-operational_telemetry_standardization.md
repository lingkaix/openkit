...
With no explicit OTLP destination, instrumentation is disabled. The initial operator enablement uses the standard `OTEL_EXPORTER_OTLP_ENDPOINT` for a same-deployment Collector and honors `OTEL_SDK_DISABLED=true`; no secret headers, arbitrary attributes, payload capture or vendor exporter is admitted. Malformed configuration disables export with a bounded safe diagnostic rather than preventing product boot. Nonempty exporter-header environment configuration for an emitted signal is unsupported and disables telemetry; for the initial traces slice this means `OTEL_EXPORTER_OTLP_HEADERS` and `OTEL_EXPORTER_OTLP_TRACES_HEADERS`. The diagnostic names the unsupported variable without its value. Do not temporarily mutate process environment or depend on private SDK evaluation timing to suppress headers. Stock TLS trust and client-certificate settings remain operator transport configuration, with no new credential-management interface or telemetry disclosure of their values or paths. A Collector can retain native OTLP output using its stock file exporter for an internal deployment; the operator owns that file's restricted access, rotation and deletion. Collector output remains outside Data Root product backup and never becomes an OpenKit ledger. No remote fleet collection or central service is required.
...
The accepted target adds to the existing deployment-admin App Diagnostics response a strict `process` projection with `observedAt` (UTC timestamp), `nodeVersion`, nonnegative `uptimeSeconds`, and `memory` containing nonnegative integer `rssBytes`, `heapUsedBytes` and `heapTotalBytes`. These values are sampled from the current NanoCore process after authorization, are not persisted, and are not host-wide capacity or Worker health claims. They reset/change with process lifetime and collection time. The existing boot id correlates the sample with a process instance; exact deployed build identity remains the release/deployment owner's evidence. The process projection includes `process.telemetry` with booleans `enabled` and `exportConfigured`, describing local configuration only, not successful delivery. `exportConfigured` is true exactly when an explicit endpoint parses as an absolute HTTP(S) URL with a host and without embedded credentials, query or fragment; absent or malformed endpoints yield false. `enabled` is true exactly when `exportConfigured` is true and `OTEL_SDK_DISABLED` is not `true` (case-insensitive), and no unsupported exporter-header variable is nonempty. A valid endpoint with the SDK disabled or unsupported headers configured yields `(enabled=false, exportConfigured=true)`; an absent or malformed endpoint yields `(false, false)`. Optional telemetry failure cannot change readiness or a product outcome. This adds no diagnostic store, polling service, public filesystem access, configuration owner or arbitrary runtime inspection.

## Current Implementation Projection

NanoCore implements the HTTP/process subset with a private stock OpenTelemetry provider, bounded OTLP/HTTP trace export, existing boot identity, safe HTTP response-handoff spans and correlated stdout diagnostics. The authorized App Diagnostics projection includes the process sample and nested telemetry configuration. Unsupported exporter-header environment configuration disables telemetry without mutating the process environment. The operator may run a stock local Collector; no analysis backend is bundled. Shutdown starts a concurrent best-effort flush; normal process exit may cut it short, and it cannot delay listener closure or decide the product shutdown result. Live observation verifies owned HTTP-only export, successful ordinary public reads while the Collector is stopped, and resumed export after Collector restart without changing the App boot. A Better Auth session read does not add library spans to this pipeline.

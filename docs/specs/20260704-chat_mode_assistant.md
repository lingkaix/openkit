...
Every Chat Mode App API request MUST include a client-visible `requestId`; `@openkit/core-client` may generate it before sending, but NanoCore MUST reject a missing id. Command identity is the command name, authenticated actor id, Workspace id, Thread id, and request id. The canonical input hash covers only the caller-supplied user input and optional provider and model selection; a default provider, model, Coordinator decision, or current Thread projection resolved during execution is not command input.

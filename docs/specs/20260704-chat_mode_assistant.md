...
- Assistant tools must not mutate Workspace files or Workspace/Server Knowledge, Git state, runtime state, Vault records or Policy configuration. The exact Memory operations are the User Memory exception and never expose filesystem writes; automatic capture is a trusted bounded Core assembly step under the scoped learning owner, not a model-selected tool.

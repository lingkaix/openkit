...
System operations and technical configuration affecting system behavior require the current user's usable administrator authority under Permissions Core, including Workspace-local Worker/profile, runtime/capability configuration and Provider setup. Workspace targets additionally require current content access and their existing Policy checks; administrator status is no content bypass. Workspace creation and ordinary business metadata retain their user/Workspace owners, and personal preferences target only the current user. Selecting an already approved resource for ordinary work is distinct from modifying its technical configuration. The schema/configuration owner, not the selected Thread, model or ordinary member's confirmation, classifies the target. The management entry is for the team's technical administrator; a non-admin request receives an administrator-required result without management discovery or effects.
...
Use existing Item-backed human attention and command records. An immutable candidate Artifact version carries target family/id/scope, operation, expected current revision (or verified absence for create), exact normalized candidate, before/after preview, validation results, impact/reload classification, request lineage, and the existing owner command to invoke. An artifact-reference Item presents that exact version in the private administration Thread; the existing Approval request and decision bind its identity and digest. The enclosing ordinary conversation Turn requires no Worker execution lease. It contains no secret, executable expression or model-selected filesystem path. The command owner derives storage targets and permits only its registered schema fields.

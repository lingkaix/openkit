...
The first supported NanoHost distribution is one reproducible `linux/arm64` archive containing the AArch64 NanoHost binary, the exact pinned stock Gateway, the existing OS unit, the minimal installer, inner checksums, generated bundle metadata, and the required OpenKit and OpenShell license material. It installs no configuration or credential, manages no service lifecycle, and makes no readiness claim before the operator completes the separately owned enrollment, configuration, image, and service-start steps.
...
The archive has one tag-and-target-prefixed root containing exactly `nanohost`, `openshell-gateway`, `openkit-nanohost.service`, `install.sh`, `MANIFEST.json`, `SHA256SUMS`, `licenses/openkit-LICENSE`, `licenses/openshell-LICENSE`, and `licenses/openshell-THIRD-PARTY-NOTICES`. `nanohost` and `openshell-gateway` MUST be little-endian ELF64 AArch64 ET_EXEC or ET_DYN executables with a complete valid ELF header, a bounded complete ELF64 program-header table, and at least one executable file-backed `PT_LOAD` segment whose file range is present, memory size covers file size, alignment is valid, and file-backed virtual range contains the entry point; a header-only ELF is not an executable. The accepted executable segment has entry point and virtual address below `2^63`, each file offset, file size, memory size, and alignment below `2^32`, and its complete memory-backed virtual interval does not cross `2^63`; packaging, shared verification, and installation enforce that same bounded subset. The Gateway bytes MUST match the exact `v0.0.99` archive and extracted-executable identities projected by `apps/nanohost/openshell-pin/manifest.md`, and both OpenShell license files MUST match the same manifest's exact source-commit identities. `MANIFEST.json` is a generated non-authoritative projection of the release tag, target, required file set, fixed live destinations, prerequisite names, the prerequisite identity values owned by `apps/nanohost/deploy/host-manifest.json`, and pin facts; it does not own any of them. Its host identity projection is exactly `prerequisites.identities.docker` with `path` and `version` and `prerequisites.identities.slirp4netns` with `path`, `version`, and `sha256`.
...
The `linux/arm64` distribution, side-effect-free `--version`, deterministic release packager, shared verifier, fixed-path installer, promoted execution-host manifest, isolated live-installer gate, and release-workflow composition defined by this amendment are implemented and pass their focused local regressions. Exact artifact candidate commit `1a5468bce556c14bb3dfc16550a9dc2f1c7adad5` completed the no-lifecycle A1 real-artifact gate with the real pin-bound inputs, successful package and host prerequisites, the expected earlier-installation `destination-conflict`, unchanged live destinations, and byte-identical contained staging; independent verification accepted the retained evidence, so R002 is complete. R004 remains open until a separately authorized tag is published and verified.
...
47. The exact final candidate commit produces one byte-reproducible `openkit-nanohost-<tag>-linux-arm64.tar.gz` with the fixed tree, metadata, promoted host identities, and pin-bound bytes; its AArch64 NanoHost reports the Cargo-owned version without constructing the runtime; the shared verifier accepts the intact bundle and rejects corruption, wrong architecture, missing or substituted checkout inputs, metadata drift, live-path or temporary-path conflict, and staging escape; the isolated fixed-path gate proves every live disposition, exact host identity, cleanup/resume, cooperating serialization, and non-cooperating no-overwrite boundary; and a real A1 run of that exact commit reports successful package and host prerequisites plus the expected nonzero `destination-conflict` for the earlier installed NanoHost bytes, proves all live destinations unchanged, and completes contained staged installation without `systemctl`, service lifecycle, host restart, current-deployment effect, or stale evidence reuse.
...
The manifest records the exact tag and resolved commit, the consumed machine-readable interface definitions themselves with an individual checksum for each one, the checksum identity of every consumed artifact — Gateway executable, published Supervisor OCI index and both supported platform manifests, the CLI retained for installation and diagnostics, and the OpenShell license files redistributed with the NanoHost Distribution — and the observed upstream constants below. Everything else about the boundary is expressed as code: the pinned client source and `Cargo.lock` make a signature change a build failure, and the readiness contract makes an artifact-digest mismatch a non-ready condition. Those two are the primary freeze; the manifest exists for what neither can express. The declared dedicated-`dockerd` deployment preserves published Supervisor index `sha256:ea3632b6e9528e2309103af5b6949606fcdc83ca1f69e8db81482a25bea84bb6` as release evidence and selects only its exact `linux/amd64` child `sha256:4adea8392a81ef34b3cc3284e693ac3cc6c13362fad84a492d95b53b3eb403b9` or `linux/arm64` child `sha256:b548fd939331d830cd9197f20fca9a5d95383c5e67f64929d632a37403115f38` through the private Gateway's existing `supervisor_image` field at Tier 2. The stock Docker driver extracts and caches the selected image's static-musl `/openshell-sandbox`; the multi-platform index is not used as a local image alias, and the release-archive GNU sibling and every later fallback tier are outside the consumed boundary and MUST NOT be selected.

The lightweight `v0.0.99` tag resolves to `8c7dd148a9e6360c9d5b2830e339a0dc4b3f3032`, re-confirmed from the clean complete non-shallow checkout recorded by the pin manifest. The distribution consumes `LICENSE` and `THIRD-PARTY-NOTICES` from that exact commit, and the manifest records each source path, fixed bundle path, and SHA-256 before packaging may use the bytes.

The official non-draft, non-prerelease GitHub Release published on 2026-08-05 carries `openshell-gateway-aarch64-unknown-linux-gnu.tar.gz`, `openshell-gateway-x86_64-unknown-linux-gnu.tar.gz`, `openshell-sandbox-aarch64-unknown-linux-gnu.tar.gz`, `openshell-sandbox-x86_64-unknown-linux-gnu.tar.gz`, `openshell-aarch64-unknown-linux-musl.tar.gz`, and the three corresponding `*-checksums-sha256.txt` files. Only the exact artifacts and platforms recorded as accepted evidence in the pin manifest are consumed; another listed upstream asset remains unusable until the existing pinned-boundary evidence gate admits it.

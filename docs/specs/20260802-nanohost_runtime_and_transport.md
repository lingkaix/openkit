...
- The accepted first-slice shared-Sandbox topology: one Sandbox, one declared Harness, multiple open AgentSessions for distinct Threads, one active Turn across the Harness, and independent AgentSession binding and Turn execution-lease projections.
- `SandboxCompatibilityKey` and `AgentSessionCompatibilityKey`, with `HarnessCompatibilityKey` reserved and non-authorizing until a later multi-Harness phase.
...
- The fixed unary `ExecSandbox` Harness-entry marker, retained Harness-lifetime response-monitor contract, marker-before-bridge ordering, and value-free fail-stop evidence boundary.
...
- Dynamic NanoHost discovery, fleet scheduling, multi-NanoHost placement, multiple active worker slots, session migration, active-active execution, or high availability.
- Multiple Harness Instances, more than one declared runtime family in a Sandbox, or `maxActiveTurns` greater than one. Those are later runtime phases and are non-authorizing here.
...
One Runtime Epoch contains the NanoHost service, one stock OpenShell Gateway, the effect-capable members of exactly one configured container backend, their private roots and authentication state, and zero or more sandboxes. The epoch contains only the configured backend; it never instantiates every backend that stock OpenShell can drive. The target pins the official unmodified OpenShell `0.0.99` release exactly; "latest" is not a version range or runtime resolution rule, and a newer published upstream release does not change the pin until the realization gate is re-run. The first shared-runtime slice authorizes one compatible Sandbox and one declared Harness to retain multiple open AgentSessions while `maxActiveTurns = 1`; it does not authorize concurrent active Turns, multiple Harnesses, or fleet behavior.
...
This specification now deliberately owns the shared-Sandbox Harness behavior that it previously deferred to another accepted owner. Existing transport carriage alone did not authorize multiplicity; this amendment supplies the missing topology, compatibility, lifecycle, failure, authority, and acceptance contract. One Sandbox and its one declared Harness may therefore retain multiple Core AgentSessions in the first slice, but that ownership change creates no concurrent active-Turn authority, second Harness, second runtime family, public runtime identity, transport authority, or scheduler authority beyond the contracts stated here and in the scheduling owner.
...
This specification permits one Sandbox Integration instance to supervise one declared Harness with multiple open Core AgentSessions for distinct Threads. Runtime-native agents, child threads, sub-agents, or provider sessions beneath one governed AgentSession remain private runtime activity unless Core independently schedules another Thread and AgentSession; private multiplicity creates no Core identity, capacity grant, transport authority, or product status by itself.
...
The first shared-runtime slice has this exact private topology:

```text
Runtime Epoch
└── SandboxRuntimeRecord
    └── HarnessInstanceRecord (one declared runtime family)
...
The first slice declares one Harness Instance and `maxActiveTurns = 1`. Multiple open AgentSessions from distinct Threads and sequential execution are current scope; concurrent active Turns, multiple Harness Instances, and runtime Phases 2 through 4 are not authorized by this topology. A Harness does not choose work, grant capacity, extend deadlines, select models, broaden authority, apply Workspace changes, or infer terminal state.
...
The first-slice co-residency envelope is one Workspace and one responsible user. Independent adjudication, adversarial work, incompatible user trust, incompatible credential visibility, authorization-sensitive work, and strict-risk work select a separate Sandbox whenever shared process memory, writable state, credentials, context, or model state could undermine security and adjudication isolation.
...
`HarnessCompatibilityKey` is a reserved Phase 3 name only. The first slice has one declared Harness candidate, so computing or persisting a third key would decide no branch. Runtime family, adapter and native-runtime version, protocol, binaries, tools, and process environment posture are already part of the Sandbox image and declared Harness set, while selected-profile support and required features are rechecked at Turn admission. Its creation, update, termination, retry, recovery, conflict, missing, stale, restart, and dependency-failure classes are explicitly not applicable because no current key instance or placement decision exists. Its current acceptance predicate is absence: no implementation, schema, digest, invalidation rule, fallback, or runtime decision may use `HarnessCompatibilityKey`.
...
Observable conformance requires identical inputs to select one shared Sandbox, any incompatible static input to reject co-residency, an incompatible AgentSession input to reject reuse, and the first-slice implementation to contain no `HarnessCompatibilityKey` artifact.

### Turn Authority Boundary

Every `turn.start` receives one fresh immutable AEP snapshot, Context Package, route credentials, execution lease, deadline, and sequence scope bound to its exact Workspace, Thread, Turn, AgentSession, actor, request, Agent, profile, runtime, provider, policy, Vault grants, resource limits, observability, backend requirements, and package digest. The existing AgentSession supplies continuity only.
...
The private `harnessBindingRef` is opaque non-secret routing metadata allocated by the NanoCore runtime owner and fixed for one Harness lifetime. It is neither a bearer credential nor sufficient authority: it is accepted only when NanoHost supplies it from the exact current bridge context over the current authoritative outer connection and NanoCore matches the current private Harness record. It never enters a native Agent child, AEP, Context Package, product record, ordinary diagnostic, or public request.
...
The unary `ExecSandbox` request is a compiled internal phase of `bridge.open` for the exact current ready Sandbox. Its Start request uses one image-owned fixed Harness entry command, fixed working directory, no TTY, zero columns and rows, `timeout_seconds=0`, no AEP or Context Package, and only non-secret Sandbox and Harness bootstrap lineage. No NanoCore command field, AEP field, configuration, caller, image label, AgentSession, Turn, or worker input may select or alter the executable, argument, working directory, TTY shape, timeout, environment keys, or target.

The unary request carries empty stdin and request EOF before long-lived Harness execution. The request remains within the pinned decoded-message cap, and NanoHost retains the server response stream as the epoch-local Harness monitor. No worker-control, inference, capability, provider, Vault, permission, AgentSession, or Turn credential enters `bridge.open`, Harness argv, Harness environment, disk, static configuration, diagnostics, stdout, stderr, transcript, evidence, or ordinary logs. The static non-secret `harnessBindingRef` remains in the NanoHost bridge context and is never delivered to Integration. Exact per-Turn route credentials arrive only through the fixed authenticated `turn.start` result and are bound to AgentSession-local descriptors or route handles; capability remains disabled unless separately admitted for that Turn.
...
2. NanoHost sends the fixed unary `ExecSandbox` request with empty stdin and request EOF, accepts the exact value-free Harness-entry marker, and retains the still-live response monitor; only then may it attempt the bridge. Sandbox Integration binds at `127.0.0.1:17891` without opening an AgentSession or starting a Turn.
...
8. Integration sends exact `POST /worker-control/harness/poll` with no `Authorization` or Harness-binding header. NanoHost rejects either client-supplied header, injects the current bridge's exact `harnessBindingRef` only on the outer request, and NanoCore returns empty `204` only after matching the authoritative outer connection and current private Harness record.
9. That first exact `204` is the product-safe Harness-ready latch. It proves the private pull path, Sandbox, Harness, adapter, compatibility, and capacity declaration without opening an AgentSession, creating a native conversation, delivering a Turn credential, or releasing a native Agent child; capability remains fail-closed.
10. NanoHost settles `bridge.open` only after the Start, request EOF, bridge, nested H2, Harness-ready latch, and live response monitor are correlated to the same request, Sandbox, Harness, and current static lineage.
...
Both fixed listeners are bound before the Harness-entry marker. Native HTTP/1 admission remains disabled until the nested HTTP/2 client is ready, which occurs before the first private Harness poll; the first credential-free poll `204` then proves the ready outer route and both bound listener roles before any native child may launch. Either bind failure, a non-HTTP/2 Supervisor bridge, unexpected listener or nested-session closure, protocol-role confusion, or inability to preserve the fixed loopback bypass fails the existing bridge/Harness lifecycle; an individual native socket failure cancels only its one request unless it also proves the native listener or nested session failed. Definite Integration close stops both listeners, aborts active native requests, closes the nested session, and follows the existing Harness and Sandbox cleanup owner.
...
Ordinary Sandbox create, proved Sandbox delete, and mid-epoch attempt-image import are the only mutable epoch-membership operations inside a healthy epoch. AgentSession open and close mutate Harness occupancy without changing epoch or Sandbox identity, and the current scheduling owner permits multiple open AgentSessions but at most one active Turn across the first-slice Harness.
...
For a request with no compatible ready Sandbox and Harness, the NanoHost performs stock OpenShell provider setup, Sandbox creation, static materialization, Harness launch, bridge establishment, and the exact first private Harness poll before settling `bridge.open`. For a later compatible AgentSession, NanoCore grants open-session capacity and delivers the fixed `session.open` operation through that existing Harness pull path; no Sandbox, bridge, Harness, or transport authority is recreated. NanoCore has no direct OpenShell, Gateway, container-runtime, filesystem, process, or Sandbox handle.
...
Each resident AgentSession has one private adapter state root and restricted native conversation handle. One admitted Turn may start a new native Agent process for its AgentSession; the first Turn establishes that handle when the adapter requires a prompt to create it, and a later Turn may start another process instance only by resuming the exact retained handle. A sibling AgentSession uses a different root, handle, process, package slots, route credentials, and cleanup proof. The first slice admits at most one active native Turn process across the Harness, so process multiplicity never creates concurrent Turn authority.

The fixed unary `ExecSandbox` response monitor is an epoch-local attempt member for the full Sandbox and Harness lifetime. Ordinary Turn interrupt remains exact worker-control; closing or cancelling the monitor, the bridge, or either H2 connection is not a kill operation and cannot prove AgentSession-local child, Harness, or process-group absence.
...
The Turn export barrier requires NanoCore to accept the exact `final_status` and the Harness to prove the exact AgentSession-local execution absent or quiescent with no further output writer. The Sandbox-wide retained bootstrap monitor remains a Harness-lifetime proof and is not required to exit for one local AgentSession close. A nonzero AgentSession-local exit without proved local absence, response loss, child survival, relay loss, member loss, or ambiguous cleanup blocks that Turn's export and drains the Harness; if the Harness cannot prove its remaining effect domain safe, cleanup widens to Sandbox deletion and then epoch invalidation when deletion is not definite.
...
- A nested bridge reconnect never redelivers a dispatched Harness operation. The same surviving Integration may replay only its retained exact result for the same `harnessBindingRef`, operation id, and sequence; missing command-delivery or result proof becomes `unknown` and widens cleanup, while a restarted Integration or changed Harness binding cannot adopt the predecessor's operation state.
...
The current production path is NanoHost-only. The `apps/nanohost` Rust application owns one configured backend and Runtime Epoch, foreground member supervision, TLS plus dedicated Token admission, exact transport-assigned connection generation, same-connection readiness, fixed effect dispatch, image acquisition and build, and the bounded file-data carriages. NanoCore retains scheduler, package, lease, AgentSession, product-record, and canonical-handoff authority and no longer selects a Cell, SSH lifecycle, Gateway forward, or direct Sandbox endpoint. The epoch-private namespace, fixed slirp helper, resolver boundary, namespace-scoped Gateway connector, service mount isolation, and exact host-manifest identity defined above are implemented and pass their local Rust and host regressions. The retained A1 start, stop, member-failure, and `SIGKILL` noninterference gate proves that the rebuilt NanoHost leaves the system Docker bridge, canonical nftables projection, business-container attachments, and build egress unchanged.

The current worker shim is Sandbox Integration for worker-control and inference over one stock `ForwardTcp`/`RelayStream` pair and nested standard HTTP/2 session. Capability remains disabled. The fixed unary `ExecSandbox` bootstrap supplies no arguments, environment values, or stdin bytes; the Node Harness installs its Supervisor-only `127.0.0.1:17891` listener and separate native HTTP/1 `127.0.0.1:17892` listener, emits exact `OPENKIT_WORKER_SHIM_ENTRY_V1\n`, and NanoHost accepts the marker exactly once across arbitrary stdout event splitting while retaining the same response monitor for the Harness lifetime. The injected `harnessBindingRef`, two credential-free private Harness paths, first-poll empty-`204` readiness latch, fixed six-operation sequencer, multiple independent AgentSessions, one active Turn, Codex exact-handle resume in fresh processes, private runtime-record decomposition, two compatibility keys, separate open-session occupancy, per-Turn route-token binding, package-config before generated Context imports, required present exports, the one exact optional Workspace-manifest absence result, fixed `image.build/input` carriage, and authenticated native inference projection through the existing H2 session are implemented and locally tested. Retained A1 real-provider and reconnect evidence covers the accepted fault scenarios.
...
Boot Phase 8 is effect-free and registers only deterministic cleanup result expectations; the existing post-listener single-flight service owns effectful cleanup and fail-closed accepted-final-status handling. A retained exact result settles the original owner without replay, a successor poll-first observation marks the old request unknown and fences that connection, and NanoHost fail-stops a successor that loses the first poll response before completion while retaining reconnect behavior for the initial connection, a delivered retained result, and later polls. Every authoritative readiness report still projects the same coordinator-established `freshEmpty` proof, while Sandbox cleanup state and retained scheduler capacity prevent a healthy reconnect from clearing an uncertainty fence. The retained A1 Unit F run passed F1 through F4, the ordinary lifecycle, and Aggregate with no failure record.
...
5. Completed: implement Sandbox Integration, the multi-AgentSession Harness, fixed operation carriage, continuity adapter, route separation, and compatibility and capacity bounds.
...
8. Completed: run the four required A1 fault scenarios and Aggregate against stock OpenShell.
9. Completed: delete the retired Cell, SSH lifecycle, Gateway-forward, direct-endpoint, runner, test, and current-support paths.
10. The deprecated Cell specification is terminal and remains at the specification root only until the documentation-governance auditor archives it.
...
- One Sandbox and one declared Harness retain at least two exact AgentSession bindings for distinct Threads while fixed `maxOpenSessions` and Sandbox aggregate bounds hold and `maxActiveTurns = 1`; a second current binding for one Thread and a second active Turn are refused without closing valid idle AgentSessions.
- `SandboxCompatibilityKey` and `AgentSessionCompatibilityKey` accept only exact current inputs, a changed static or continuity input selects replacement rather than approximate reuse, and `HarnessCompatibilityKey` has no first-slice runtime or schema artifact.
...
- The fixed `bridge.open` disposition carries only its static non-secret Harness descriptor, starts exactly one retained Sandbox-Harness monitor with the fixed Start fields, empty stdin and EOF, accepts exactly one `OPENKIT_WORKER_SHIM_ENTRY_V1\n` byte-stream marker before any bridge attempt, opens the one bridge, receives exact `204` from the first header-free private Harness poll, and only then permits fixed `session.open`. Missing, incomplete, duplicate, or adjacent non-empty stdout, any non-empty stderr, a credential-bearing or client-bound poll, or a non-`204` response fails stop; reconnect performs no relaunch. A Turn export requires accepted `final_status` and exact AgentSession-local quiescence; correlated monitor Exit and clean completion are required only when the Harness ends.
...
30. `bridge.open` carries only its static non-secret Harness descriptor and no AgentSession, Turn, package, Context, or route credential; NanoHost runs one fixed timeout-zero Harness Start with empty stdin and EOF, accepts exactly one fixed value-free `OPENKIT_WORKER_SHIM_ENTRY_V1\n` marker as an arbitrarily split stdout byte stream before any bridge attempt, keeps the response monitor live for the Harness lifetime, establishes the one bridge, receives exact empty `204` from the first private Harness poll, and permits fixed `session.open`. Missing, incomplete, duplicate, or adjacent non-empty stdout, any non-empty stderr, premature Exit, response loss, malformed or credential-bearing first poll, non-`204`, or stream completion fails stop; reconnect causes zero extra Start, marker, Sandbox, Harness, or already-current bridge effects.
...
36. One compatible Sandbox and one declared Harness retain at least two open AgentSessions for distinct Threads, each with an exact Core binding and either an adapter-authorized pending handle before its first Turn or one restricted exact native conversation handle afterward, while each Thread has at most one current resident AgentSession, `maxActiveTurns = 1` across the Harness, and each AgentSession has at most one active Turn.
...
38. `SandboxCompatibilityKey` rejects any incompatible Workspace, responsible-user trust, static filesystem or mount, network, provider, Vault, credential visibility, resource, sensitivity, or containment input; `AgentSessionCompatibilityKey` rejects incompatible continuity; no first-slice artifact computes or persists `HarnessCompatibilityKey`.
...
| Fixed Harness bootstrap | One unary-request/server-streaming `ExecSandbox` use over the same client and Sandbox carries empty stdin plus request EOF before execution and retains the response as the Harness-lifetime monitor |
...
For the current dogfood profile of one NanoHost, multiple open AgentSessions, and one active Turn across the first-slice Harness, the shared blast radius is an intentional simplification. Future smaller blast radius is obtained by multiple independent NanoHosts, not by reintroducing one Cell or Runtime Epoch per AgentSession.
...
- Runtime Phase 2 concurrent active Turns only after the scheduling-scale owner accepts `maxActiveTurns > 1` and the Harness proves independent multiplexing, routing, interruption, context, output, and cleanup.
- Runtime Phase 3 multiple Harness Instances and runtime families only after a separate accepted placement and compatibility contract activates the reserved `HarnessCompatibilityKey` seam.

...
10. Implement the existing `HarnessCompatibilityKey` seam, multiple Harness records per Sandbox, Integration supervision of the declared Harness set, and bounded concurrent Turns across distinct AgentSessions without changing the six-operation protocol.
11. Run focused scheduler, worker-shim, NanoHost transport, restart, cleanup-width, and real two-runtime acceptance before reporting the new target implemented.
12. The deprecated Cell specification is terminal and remains at the specification root only until the documentation-governance auditor archives it.

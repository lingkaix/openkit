...
[Persistent Worker Volumes](20260910-persistent_worker_volumes.md) owns the storageRef and complete inherited volume-layout contract. NanoHost realizes its current admitted attachment through stock OpenShell `SandboxTemplate.driver_config` and the private Docker driver's explicit mount support. The Gateway enables binds only for NanoHost-generated, root-validated mounts; no public caller supplies backend paths or arbitrary driver config. `/var/lib/openkit/nanohost-work` is a durable execution-host root outside both epoch roots, the Image Store, credentials and forensic evidence. It is never included in recursive epoch cleanup, image eviction or normal Sandbox deletion.

Fresh-empty readiness still proves absence of prior processes, containers, backend/Gateway state and authority, not absence of retained data. A fresh epoch cannot mount a storageRef until the prior attachment's complete effect domain is fenced and Core freshly admits the same scope and compatible image layout. Existing cleanup and reconnect proofs remain strict. An unknown create, detach or replacement result never authorizes another writer or empty reinitialization; the data remains retained and the existing runtime outcome remains unknown/cleanup-required.

The existing `sandbox.create` effect carries the Core-resolved storageRef, exact scoped attachment identity and admitted image storage-layout expectation alongside its existing image, policy and lease. NanoHost derives local paths, verifies identity/ownership, initializes only explicitly new volumes from the exact image, and supplies fixed validated mounts to the official SDK. Root metadata is host-owned and not worker-writable. Definite Sandbox deletion releases execution attachment; it does not purge any volume. A private local root record identifies bytes and initialization only, never authorizes work or settles a prior operation. Image preparation continues through the existing image acquisition/build/store owners and must not mount retained work into a build.

The deployed implementation has no retained-volume mount yet. Image/SDK presence and healthy reconnect do not establish this amendment's implementation; its persistence and replacement predicates require their own exact-build tests.
...
- The per-Turn AEP-before-worker-supply-before-Context-before-`turn.start` import order through the existing fixed file-data mechanism, with AgentSession-private destinations and no Sandbox-wide authorization package.
...
- The first NanoHost distribution boundary: the exact bundle contents, binary identities, fixed live destinations, staging containment, installer effect and retry semantics, and acceptance evidence.
...
- The supported OpenShell release identity, its ordinary external-artifact metadata, and the integration qualification required before a newer release becomes selectable.
...
- Operator installation, diagnostics, update, rollback, uninstall, or incident-response procedure beyond the minimal distribution and installer contract stated here.
- Cross-product deployment host profiles, source-build versus installed dependency classification, resource recommendations, and the side-effect-free host-requirement verdict, which are owned by `docs/specs/20260909-deployment_host_requirements.md`.
...
One Runtime Epoch contains the NanoHost service, one stock OpenShell Gateway, the effect-capable members of exactly one configured container backend, their private roots and authentication state, and zero or more sandboxes. The epoch contains only the configured backend; it never instantiates every backend that stock OpenShell can drive. The supported OpenShell dependency set is one exact official release: its SDK source, Gateway, Supervisor, and any retained diagnostic CLI resolve to that release, and a newer upstream publication does not change the supported set until OpenKit qualifies its own integration against the candidate. One compatible Sandbox may contain multiple declared Harnesses for different runtime families or different static configurations, each may retain multiple open AgentSessions, and bounded active Turns may run concurrently only across distinct AgentSessions and Threads under the scheduling owner.
...
The NanoHost distribution set is one reproducible target-matched archive for each supported target, `linux/amd64` and `linux/arm64`, containing the matching NanoHost binary, exact supported stock Gateway, existing OS unit, minimal installer, inner checksums, generated bundle metadata, and required OpenKit and OpenShell license material. An archive installs no configuration or credential, manages no service lifecycle, and makes no readiness claim before the operator completes the separately owned enrollment, configuration, image, and service-start steps.
...
The durable requirement is one authenticated bidirectional byte stream per active sandbox carrying one standard HTTP/2 session with Integration as client, stated under Sandbox Transport Requirement. The stock `ForwardTcp`/`RelayOpen`/`ConnectSupervisor`/`RelayStream` bridge is its selected realization, enumerated in the Stock Integration Qualification Annex. OpenKit trusts an official matched release to make its SDK, Gateway, and Supervisor mutually compatible; OpenKit qualifies the behavior and limits of its own adapter, process, transport, and packaging integration. A candidate that fails those predicates remains unselectable, and the failure MUST NOT be hidden by weakening a bound or substituting an OpenKit protocol, OpenShell fork, arbitrary tunnel, direct Gateway exposure, or weaker transport.
...
Use one authoritative NanoHost-initiated authenticated NanoCore transport session for every declared control, inference, capability, and fixed NanoHost-effect route, with predecessor fencing before replacement. Reserve one distinct fixed file-data stream on that same physical connection for the exact V1 single-file effects. Use one stock `ForwardTcp`/`RelayStream` pair containing one standard HTTP/2 session per active sandbox for `/worker-control/*`, `/inference/*`, and `/capabilities/*`, without an OpenKit multiplexer and without merging route credentials or semantics. Carry the six fixed Harness operations only through Integration-initiated private `/worker-control/harness/poll` and `/worker-control/harness/result` requests bound to that existing nested connection and one NanoHost-injected non-secret Harness binding; add no Harness-specific effect, fourth namespace, reverse request direction, second connection, or `bridge.open` credential.

Implement the NanoHost service as one Rust application under `apps/nanohost`. Compile the official OpenShell Rust SDK from the exact supported release commit recorded by Cargo and its lockfile, run the matching checksum-verified stock Gateway as a separate foreground process in the OS-supervised failure group, select the matching exact Supervisor image, and use the SDK directly rather than the CLI for normal operation. Do not vendor protobuf definitions or maintain a generated-client fallback while the official SDK supplies the consumed client surface.

Distribute that application as target-matched `linux/amd64` and `linux/arm64` archives through the exact archive and installer contract below. Another operating system, architecture, package manager, updater, service manager, deployment record, compatibility path, or cross-build framework requires a later accepted owner and is not implied by upstream support.
...
### NanoHost Distribution

The NanoHost Distribution is the controlled execution-host archive owned by this specification and composed into an OpenKit release by `docs/specs/20260829-release_management.md`. The supported target set is exactly `linux/amd64` and `linux/arm64`; each filename is `openkit-nanohost-<tag>-linux-<architecture>.tar.gz`. Another operating system, architecture, package registry, or source archive is not a supported distribution.

Each archive has one tag-and-target-prefixed root containing exactly `nanohost`, `openshell-gateway`, `openkit-nanohost.service`, `install.sh`, `host-manifest.json`, `MANIFEST.json`, `SHA256SUMS`, `licenses/openkit-LICENSE`, `licenses/openshell-LICENSE`, and `licenses/openshell-THIRD-PARTY-NOTICES`. `nanohost` and `openshell-gateway` MUST be little-endian ELF64 x86-64 for `linux/amd64` or AArch64 for `linux/arm64`, using ET_EXEC or ET_DYN with a complete valid ELF header, a bounded complete ELF64 program-header table, and at least one executable file-backed `PT_LOAD` segment whose file range is present, memory size covers file size, alignment is valid, and file-backed virtual range contains the entry point; a header-only ELF is not an executable. The accepted executable segment has entry point and virtual address below `2^63`, each file offset, file size, memory size, and alignment below `2^32`, and its complete memory-backed virtual interval does not cross `2^63`; packaging, shared verification, and installation enforce that same bounded subset. The Gateway bytes MUST match the target-specific supported release archive and extracted-executable identities in `apps/nanohost/openshell/release.json`, and both OpenShell license files MUST match that release metadata's exact source identities. `host-manifest.json` MUST equal the exact checkout bytes at `apps/nanohost/deploy/host-manifest.json`. `MANIFEST.json` is a generated non-authoritative projection of the release tag, exact OpenKit product commit, target, required file set, fixed live destinations, exact target architecture, and exact host-profile id and digest, plus OpenShell release facts; it does not own any of them and does not duplicate the bundled host profile's requirement fields.

The archive is GNU tar in the declared fixed member order. Both directories have mode `0755`; `nanohost`, `openshell-gateway`, and `install.sh` have mode `0755`; the service unit, host and release manifests, checksums, and all licenses have mode `0644`; every member has numeric uid/gid `0/0` and mtime Unix epoch second `0`. The tar stream has two zero end blocks and only zero bytes in any remaining record padding. Gzip uses the exact header bytes `1f 8b 08 00 00 00 00 00 02 03`, which encode deflate, no optional fields, zero mtime, maximum-compression extra flags, and Unix OS. These values plus fixed path and content make identical inputs byte-reproducible. The bundled `install.sh`, service unit, host manifest, and OpenKit license must equal the exact checkout bytes at `apps/nanohost/deploy/install.sh`, `apps/nanohost/deploy/openkit-nanohost.service`, `apps/nanohost/deploy/host-manifest.json`, and `LICENSE`. The archive contains no configuration, credential, Token hash, deployment identity, environment file, OpenShell CLI, Supervisor image, Image Store content, runtime evidence, or source tree.

The installer writes only `nanohost` to `/usr/lib/openkit/nanohost` with mode `0755`, `openshell-gateway` to `/usr/lib/openkit/openshell-gateway` with mode `0755`, and `openkit-nanohost.service` to `/etc/systemd/system/openkit-nanohost.service` with mode `0644`. Before any target write, every mode verifies all inner checksums, both ELF identities, and that the bundled host-profile bytes hash to the profile digest in `MANIFEST.json`. Once the schema-version-2 requirement profile and checker are implemented, target live installation additionally requires `requirements-met` under `docs/specs/20260909-deployment_host_requirements.md`; it reads only the verified bundled profile and never uses a repository or network fallback. Compatibility is decided by that profile's bounded platform, path, version-predicate, and static prerequisite probes rather than equality with one promoted machine's Docker output, kernel release, or `slirp4netns` digest. Until then, the exact current preconditions described under Current Implementation Projection remain mandatory.

The target bundled `install.sh --check-host` mode owns the deployment requirement specification's single JSON result and performs no destination mutation or lifecycle operation. The `--check-host` mode, existing `--check` mode, and default live installation share one internal profile-probe implementation. Both `--check` and default live installation consume that same result to decide installation eligibility, print no second host-check JSON object, and retain their separately owned installation output defined below; the real-use host assertion consumes the same `--check-host` result through its existing harness. This mode is not present in the current installer.

Live preflight requires every existing destination ancestor to be a real non-symlink directory and every existing destination to be a regular non-symlink file. `--check` performs package, host, destination, and reserved-temporary-path inspection without writing or invoking `systemctl`, reports `host-prerequisites=pass` only after every prerequisite succeeds, and reports exactly one destination disposition: `installable` when all destinations and reserved temporary paths are absent; `already-installed` when every destination has the current payload's exact bytes and mode and no reserved temporary path exists; `resumable` when every destination and reserved temporary path is absent or is a regular non-symlink exact byte-and-mode match for its current payload, and the state satisfies neither `installable` nor `already-installed`; or `destination-conflict` otherwise. `destination-conflict` returns nonzero while preserving the truthful successful host-prerequisite observation; the other three dispositions return zero without claiming runtime readiness.

Staging accepts only an absolute `DESTDIR` whose canonical normalized form is textually identical and not `/`, which does not exist, and whose existing ancestors are real directories rather than symbolic links. The installer creates the staging root with private permissions and creates every descendant itself before writing; traversal, repeated-root, existing-root, symlink-root, or symlink-ancestor input fails before any write. Staging verifies package integrity and target-matched ELF payload identity, skips host-only requirement checks, writes the three fixed destinations only beneath that root, and reports `staged-only` without making a live-install or runtime-readiness claim.

Every live `--check` or installation opens and holds one exclusive advisory lock on the fixed existing `/etc/systemd/system` directory before observing any destination or reserved temporary path and through its complete observation or mutation. This serializes cooperating invocations without creating a lock file or other durable installer state; no-overwrite publication and exact revalidation remain the backstop for a non-cooperating writer.

Live installation uses exactly `/usr/lib/openkit/.nanohost.openkit-install`, `/usr/lib/openkit/.openshell-gateway.openkit-install`, and `/etc/systemd/system/.openkit-nanohost.service.openkit-install` as its reserved adjacent temporary paths. It accepts each destination or reserved path only when absent or when it is a regular non-symlink file with the current payload's exact bytes and required mode; any conflict fails before the first destination mutation. It prepares every missing payload at its reserved path before publishing any destination, then atomically publishes each missing destination without overwriting an appearing path and revalidates its exact bytes and mode. A caught failure or signal removes only regular temporary files and empty parent directories created by that invocation; a matching regular temporary file left by an uncatchable interruption is reusable, while a changed, non-regular, or symlink temporary path is never removed or adopted. A publication-set failure reports incomplete; retry derives authority only from the current three payloads and the observed destination and temporary bytes and modes, preserves exact installed files, and completes missing files. Successful completion removes any remaining exact reserved temporary file. V1 owns no bundle-level installer identity, update, downgrade, rollback, uninstall, automatic repair, configuration, enrollment, image pull, `systemctl` call, service start, service stop, or service restart.

A successful new live installation reports `installation=complete`, and an exact already-installed live invocation reports `installation=already-installed`; both then report `remaining=configuration,enrollment,deployment-images,service-start`. Neither output claims runtime readiness, performs those remaining steps, or changes the `--check` and staging output contracts.

The NanoHost binary provides `nanohost --version` before configuration parsing, storage, networking, enrollment, Runtime Epoch creation, or child-process startup. It prints the Cargo-owned version and exits successfully without another effect; an unknown argument does not select a lifecycle or installer operation.
...
Resource installation is a projection of the exact AEP supply under `docs/specs/20260907-agent_plugin_packaging_and_worker_supply.md`. Integration and the selected runtime adapter consume verified Skill inputs through the existing `reference.import` file effect and import-only `worker-supply` identity owned by `docs/specs/20260801-nanohost_workspace_data_boundary.md`; AEP admission precedes resource imports, Context imports, and native launch. The adapter produces a session-private read-only native package or thin component projection with only selected Skills and the fixed Gateway MCP binding. It cannot forward original MCP configuration, activate vendor extensions, fetch packages, choose versions, or create a new route. Native loading is advertised only with proof on the exact adapter, image, and launch flags; missing required support blocks readiness. This resource path is an accepted target, not a claim about the current static Skill metadata implementation.
...
Exact selected static Skill/MCP supply, its verified inventories, and the applicable native resource projection participate in `AgentSessionCompatibilityKey`. A changed selection rejects continuity reuse and follows the existing session replacement path; it never rewrites active session files. Per-AgentSession resource files do not become Sandbox-global discovery roots or shared mutable Harness configuration. Fixed image/adapter plugin digests retain their separate Harness meaning below.
...
- An explicit `server-admin`-authorized installation or maintenance acquisition action outside the readiness path, for a deployment image or the exact immutable environment-preparation candidate owned by `20260910-persistent_worker_volumes.md`. Candidate preparation travels on the authenticated control session and requires no Worker lease; its image remains non-published until a separately authorized workload consumes its resolved digest. A deployment image absent from the store keeps the NanoHost non-ready until such an action succeeds; readiness never repairs it.
...
- A definite build failure fails the exact attempt through its existing lease and worker owners, or the exact preparation through its candidate/command owner when no Worker attempt exists. It does not invalidate a healthy epoch, because it completes no sandbox effect. An uncertain runtime effect retains the existing transport recovery rules; result settlement does not authorize candidate application.
...
| NanoHost distribution | This specification owns target, contents, prerequisites, destinations, installer effects, and readiness exclusions; Release Management owns product-release composition and publication. | `apps/nanohost/openshell/release.json` records the supported upstream release and external-artifact identities, and generated bundle metadata projects the exact release instance without becoming authority. |
...
| OpenShell client boundary | Official `openshell-sdk` source fixed to the supported release commit by Cargo and `Cargo.lock` | Typed lifecycle and forwarding RPC access over one authenticated loopback gRPC/HTTP/2 channel. | Vendored protobuf copies, generated-client fallback, Gateway server code, CLI output parsing, filesystem discovery, floating `main`, and OpenShell protocol extensions. |
...
The Rust implementation uses the smallest async stack that satisfies the accepted transports: Tokio for asynchronous process and I/O ownership, the official exact-release OpenShell SDK for Gateway gRPC, Hyper/H2 for the server side of the nested sandbox HTTP/2 session, and Rustls for the non-loopback NanoCore TLS boundary. Exact dependency versions and the Rust toolchain are reproducibly fixed through the app-local Cargo lock and toolchain setup owned by `docs/toolchain.md`; no application Web framework is required merely to match three fixed route namespaces.
...
- **Upstream client reuse.** The supported OpenShell release publishes its client surface in Rust. Consuming that exact-release client keeps the NanoHost on the upstream contract without a second binding layer or a vendored interface copy.
...
### OpenShell Release Selection And Qualification

NanoHost consumes one official matched OpenShell release. `apps/nanohost/Cargo.toml` and `Cargo.lock` fix the SDK source to that release's resolved commit. `apps/nanohost/openshell/release.json` records only the release version and commit, the exact external Gateway and Supervisor artifact identities required by packaging and readiness, and redistributed license-file checksums. `apps/nanohost/openshell/transport-assumptions.json` separately owns the source identity and evidence required by the hard-memory proof below. Production builds MUST NOT depend on OpenShell `main`, a version range, a locally modified checkout, a vendored protobuf copy, or a second hand-authored description of the SDK surface.

OpenKit accepts upstream responsibility for mutual compatibility among the official SDK, Gateway, and Supervisor of that release. OpenKit remains responsible for proving that NanoHost uses those components correctly: the compiled calls and types remain valid, external artifacts match the selected release, lifecycle and security failures remain fail closed, the byte adapter preserves exact data and backpressure, control traffic meets its accepted liveness bounds under saturation, cancellation and reconnect preserve the owning semantics, and packaging selects the recorded bytes.

The stock Gateway executable, exact Supervisor OCI image, and any CLI retained for installation or diagnostics MUST resolve to the same exact release as the SDK. A mixed release set, unresolved artifact, or digest mismatch keeps the candidate and any deployment that selects it non-ready. A candidate is adopted only after the fast foundation checks and the opt-in real-stock qualification in the annex pass; ordinary development does not contact upstream or a live host.

NanoHost's private Gateway configuration fixes the existing `supervisor_image` field to the exact platform manifest recorded in release metadata for its supported compile target: `ghcr.io/nvidia/openshell/supervisor:0.0.99@sha256:4adea8392a81ef34b3cc3284e693ac3cc6c13362fad84a492d95b53b3eb403b9` on `linux/amd64`, or `ghcr.io/nvidia/openshell/supervisor:0.0.99@sha256:b548fd939331d830cd9197f20fca9a5d95383c5e67f64929d632a37403115f38` on `linux/arm64`. The stock Docker driver therefore selects explicit-image Tier 2 before the Tier-3 sibling lookup and uses its existing image extraction and cache path for the selected image's static-musl `/openshell-sandbox`. NanoHost does not consume the GNU sibling executable, infer another image, accept an unsupported compile target, or expose this fixed private value through deployment schema, configuration input, command, package, or worker state; a missing, changed, unresolvable, or unextractable exact platform image keeps the epoch non-ready.
...
Every active Turn uses independently generated worker-control, inference, and capability raw tokens, not a shared value and not a value derived from or equal to the non-secret `sandboxBindingRef`. The existing scheduler lease/token-binding owner persists exactly three nullable lowercase SHA-256 projections per live lease, one per family, and never a raw token. Each family hashes a presented raw value and compares it to the exact current AgentSession, Turn, lease, package, and route lineage; worker control, inference, and capability each reject the other two tokens and every sibling AgentSession token. Raw tokens remain live-memory only in NanoCore, NanoHost while carrying the exact admitted Turn operation, and that Turn's AgentSession-local route boundary. They never enter Harness-global process environment, Sandbox-wide files, native argv, or another AgentSession.

NanoCore restart rebuilds each active Turn's three family bindings from its lease-owned hashes and current AEP and lineage while the surviving NanoHost and AgentSession-local route boundary retain the same raw values. Reconnect rebinds carriage without token reissuance, Start, Sandbox, Harness, or AgentSession recreation, or another bridge open. Terminal, stale, fenced, and releasing lineage revokes all three bindings through the existing lease status owner, except that exact accepted `final_status` replay retains only its already-owned idempotency semantics. No credential service, token table, configuration, or durable raw-secret owner is added.
...
**The stock interior needs a hard bound as well as a live observation.** A finite load test can expose growth, stalls, and broken backpressure, but it cannot prove an absolute memory bound for the opaque Supervisor and Gateway interior. `apps/nanohost/openshell/transport-assumptions.json` therefore retains only the exact-release source evidence needed to establish the finite stock queue and pairing-buffer contributions to this proof. Each retained entry names the release commit, source location, source checksum, queue multiplicity where applicable, per-frame or per-direction bound, and why the complete contribution is load-bearing. A mechanical source-hash check proves only identity and staleness; acceptance of the memory argument requires review of the cited source slices together with the real-stock observations. The evidence MUST NOT retain protobuf copies, an RPC inventory, compile probes, authentication summaries, or a whole-upstream review.

The integration qualification proves the composition bounded by the following argument, and MUST state that argument rather than assert that one live sample is sufficient:
...
- every opaque stock holding point relevant to this path has a finite contribution admitted by the retained hard-bound evidence;
- the real-stock slow-consumer and saturation scenarios confirm that backpressure reaches the producer and that outstanding bytes settle instead of growing for the duration of the observation.

Every term is therefore either ceiling-enforced and observed at an OpenKit boundary or a finite source-evidenced stock contribution. If a candidate release changes the retained source or makes a stock holding point unbounded or unprovable, the hard-memory proof fails and the candidate remains unselectable. A passing finite load test does not override that failure.

The composed ceiling for a family is derived, not chosen: it is the sum of that family's in-flight ceiling on every session the observation spans plus the admitted finite stock contributions for that path. Changing an input or a retained stock contribution recomputes it.
...
**Qualification result.** A missing, invalid, incomplete, unbound, or out-of-bound measurement fails the real-stock qualification. A measurement that is present, complete, attempt-bound, and within its composed ceiling satisfies the bounded-intermediate-buffering requirement stated above. A measured value that exceeds a calibratable target while staying inside its admissibility ceiling is a calibration under `### Calibration Disposition` and MUST NOT be recorded as a qualification failure; a value beyond the ceiling, or an inability to bound the composition at any admissible configuration, is a mechanism failure.
...
**Calibratable targets** are the values chosen ahead of measurement: the per-stream window, the family stream ceilings, the connection-level receive window, the maximum forwarded DATA per inference write, the interrupt-delivery bound, the heartbeat-delay bound, the bridge re-establishment target and hard bound, and the epoch rebuild budgets. They were selected for internal consistency, not from observation, and real-stock qualification is where a candidate release meets them in a real system.
...
**Mechanism failure — blocking.** The mechanism cannot satisfy the owning requirement at any admissible configuration. Concretely: worker-control liveness cannot be maintained under saturating load at any permitted stream, window, or chunk setting; intermediate buffering across the nested layers cannot be bounded; predecessor closure cannot be made definite; or a measured value exceeds the admissibility ceiling below. This blocks candidate adoption and returns the failed behavior to the Stock Integration Qualification Annex.
...
The subsections below are the supported stock realization of the requirement above.
...
8. Integration sends exact `POST /worker-control/harness/poll` with the poll body owned by `docs/specs/20260703-worker_control_protocol.md` and no `Authorization` or Integration-binding header. NanoHost rejects either client-supplied header, injects the current bridge's exact `sandboxIntegrationBindingRef` only on the outer request, and NanoCore returns empty `204` only after matching the authoritative outer connection, current private Sandbox Integration record, and complete declared Harness set. A later operation response carries one NanoCore-selected `harnessInstanceId`; Integration cannot request a Harness through the poll.
...
Sandbox Integration also binds a distinct fixed loopback-only `127.0.0.1:17892` HTTP/1 listener for the native Agent's inference and selected MCP projections. That listener is not another `ForwardTcp`, `RelayStream`, nested HTTP/2 session, NanoHost connection, route family, or egress path; `127.0.0.1:17891` remains exclusive to every initial or successor stock Supervisor bridge. The native listener accepts only `POST /inference/*` with the exact active Turn inference bearer or `POST /capabilities/*` with the exact active Turn capability bearer when that route is enabled, enforces each semantic owner's encoded request aggregate before carriage, and forwards the unchanged origin-form path, bytes, end-to-end headers, and content encoding through the already-ready nested HTTP/2 client session. Inference retains its 2 MiB family in-flight ceiling, capability retains its separate 512 KiB family ceiling, and both retain the 64 KiB write bound. Responses, including SSE and content encoding, stream back with bounded writes and backpressure; native cancellation cancels the one outer stream, and the local projection never retries or replays. Worker-control, cross-family bearer, disabled-capability, absolute-form, unauthenticated, and non-`POST` traffic is rejected before an outer stream opens. The adapter-owned fixed inference URL is `http://127.0.0.1:17892/inference/v1`, and the selected adapter owns any fixed capability URL; no AEP field or caller selects either. Both `NO_PROXY` and `no_proxy` in the native child must include exact `127.0.0.1` so inherited HTTP, HTTPS, or all-protocol proxy settings cannot capture these requests.
...
Step 5 is deadline-bound by NanoHost's accepted bridge-establishment and recovery budgets. NanoHost MUST treat an unclaimed, rejected, closed, or locally timed-out relay slot as a definite bridge-establishment failure for that sandbox, MUST NOT open a second overlapping `ForwardTcp` for the same sandbox while a slot is pending, and MUST expose the stage and elapsed interval in the private qualification result. OpenKit does not depend on the Gateway's internal reaper cadence or maintain that cadence as release evidence.
...
Sandbox Integration MUST NOT launch a native Agent child until the NanoHost proves the accepted worker-entry marker, current fixed monitor, Gateway client, `ConnectSupervisor`, `ForwardTcp`/`RelayStream` pair, nested HTTP/2 handshake, exact route table, client-header rejection, injected Harness binding, and the exact first Harness poll `204`. A native child launches only after one admitted `turn.start` supplies the exact AgentSession-local Turn package and three mutually distinct route tokens. A package with no selected MCP supply keeps the capability plane disabled even though the namespace is present; an enabled package binds its capability token only to the named Turn.

This entire sequence is subject to the mandatory integration qualification owned by the Stock Integration Qualification Annex, including its evidence-quality requirements and blocking failure disposition. The supported release is the immutable official OpenShell [`v0.0.99`](https://github.com/NVIDIA/OpenShell/releases/tag/v0.0.99); a development source snapshot is not a target input.

## Sandbox-To-Gateway Communication Boundary

This section realizes the Sandbox Transport Requirement through the supported stock mechanism.
...
This bridge projection realizes the Sandbox Transport Requirement and is accepted subject to the mandatory qualification in the Stock Integration Qualification Annex. A failed or indeterminate candidate result blocks adoption and returns the failed behavior to that annex and its owning requirement.
...
The deployment configures one non-secret NanoHost identity reference, one deployment identity, one NanoCore rendezvous endpoint, two NanoHost Token slots with separate non-secret secret-file and companion-file references, an optional non-secret TLS CA reference, one container-backend selection, one NanoHost Image Store location, one evidence location, and the required deployment image digest set. The V1 anonymous public acquisition registry set is the fixed runtime constant `{docker.io, ghcr.io}` and has no configuration input. The execution target requires the NanoHost service, stock OpenShell Gateway, pinned container-runtime components, Sandbox Integration image content, OS-supervision unit, and the host capabilities owned by `docs/specs/20260909-deployment_host_requirements.md`. Operator provisioning remains separate from host checking and installation and may not treat one exact OS-package identity as the portable support boundary.

Distribution installation establishes only the NanoHost binary, stock Gateway, and OS unit through the fixed contract above. It does not establish a configured execution target. Enrollment, the non-secret environment file, secret slots, deployment images, Image Store content, and an explicit operator service start remain required after live installation before runtime readiness is possible.

Installing the same exact distribution is idempotent when every existing destination matches its expected bytes and mode. Replacing a different installed distribution, terminating it, or recovering its configuration or durable runtime content is outside V1 distribution ownership and requires an explicit later operation rather than installer inference.
...
- The installed `apps/nanohost` Rust binary, its Cargo-selected OpenShell SDK source, the stock Gateway executable, and the selected Supervisor image resolve to the accepted build and exact supported release boundary.
...
When an AgentSession reaches its existing terminal and output barriers, Sandbox Integration invokes the fixed `session.close` operation, revokes that AgentSession's route bindings and native conversation handle, removes only its disposable control/request slots after required collection while preserving retained native and working roots, and releases open-session capacity only after exact local cleanup proof. A proved ordinary close leaves the shared Harness, bridge, Sandbox, and compatible sibling AgentSessions running.
...
The temporary fence-to-ready timestamp is create-once for one unresolved interval. Every later failure and evidence-export path MUST preserve the first timestamp. After authoritative admission, NanoHost MUST derive one absolute monotonic deadline from the remaining hard bound, recheck it immediately before sending readiness, and apply it to request construction, send, and the complete exact empty `204` response; only an acknowledgement completed within that deadline may consume the marker.
...
| One AgentSession ends with exact local cleanup proof | The Harness revokes only that native continuation binding, removes its disposable AgentSession control/request slots, preserves retained native and working bytes, releases open-session capacity, and preserves compatible siblings and the shared Sandbox. |
...
| Worker-control liveness bound is missed under saturating inference load | Qualification fails; implementation repairs the required mechanisms or records the measured evidence that authorizes the bounded second worker-control byte stream. No route is dropped, reordered, or reinterpreted to recover latency. |
...
- A missing, extra, corrupt, checksum-mismatched, architecture-mismatched, wrong-target, release-inconsistent, or host-profile-inconsistent distribution input fails before packaging or installation. Once the requirement-profile projection is implemented, live installation without a current `requirements-met` result for the exact profile, product commit, archive, and machine identity fails before destination mutation; until then, the exact current architecture, Docker, slirp, and systemd checks remain the implementation gate. A non-regular, symlink, byte-mismatched, or mode-mismatched destination, ancestor, or reserved temporary path fails before destination mutation; a non-canonical, root-resolving, existing, traversal-bearing, or symlink-bearing staging root fails before its creation. Caught pre-publication failure cleans only invocation-owned residue; interruption residue is reusable only when each current payload exactly matches the observed regular non-symlink destination or reserved temporary bytes and mode. A partial publication or a destination appearing after observation reports incomplete without overwrite and may resume only from exact post-publication bytes, modes, and per-file observations; stale real-artifact evidence or an artifact-affecting change cannot close distribution acceptance.
...
- External OpenShell artifacts MUST remain exact-release, checksum-verified, stock, and replaceable through the application-local release and image owners.
- The SDK, Gateway, exact Tier-2 Supervisor OCI image, and any CLI retained for diagnostics MUST resolve to the exact supported `0.0.99` release; the private Gateway configuration selects only the digest-fixed image and its stock static-musl extraction/cache path, while a mixed, floating, relabeled, fallback-tier, or version-range deployment remains non-ready. OpenKit-owned provider and policy projections remain with their existing owners rather than the OpenShell release metadata.

## Current Implementation Projection

The current production path is NanoHost-only. The `apps/nanohost` Rust application owns one configured backend and Runtime Epoch, foreground member supervision, TLS plus dedicated Token admission, exact transport-assigned connection generation, same-connection readiness, fixed effect dispatch, image acquisition and build, and the bounded file-data carriages. NanoCore retains scheduler, package, lease, AgentSession, product-record, and canonical-handoff authority and no longer selects a Cell, SSH lifecycle, Gateway forward, or direct Sandbox endpoint. The epoch-private namespace, fixed slirp helper, resolver boundary, namespace-scoped Gateway connector, service mount isolation, and schema-version-1 exact host-manifest identity are implemented and pass their local Rust and host regressions. The capability-profile target is not implemented, and retained exact-identity results do not satisfy it. Retained A1 noninterference evidence applies only to the earlier product commit it names; the current candidate still requires fresh exact-product no-host-reboot evidence before R001 may close.

The current worker shim is Sandbox Integration for worker-control, inference, and enabled selected-MCP capability traffic over one stock `ForwardTcp`/`RelayStream` pair and nested standard HTTP/2 session. Capability remains disabled for packages with no selected MCP supply. The fixed unary `ExecSandbox` bootstrap supplies no arguments, environment values, or stdin bytes; the Node supervisor installs its Supervisor-only `127.0.0.1:17891` listener and separate native HTTP/1 `127.0.0.1:17892` listener, emits exact `OPENKIT_WORKER_SHIM_ENTRY_V1\n`, and NanoHost accepts the marker exactly once across arbitrary stdout event splitting while retaining the same response monitor for the Sandbox Integration lifetime. The current `sandboxIntegrationBindingRef`, two credential-free private Harness paths, first-poll empty-`204` readiness latch, fixed six-operation protocol with an independent sequence per NanoCore-selected `harnessInstanceId`, multiple adapter-selected Harness Instances, multiple independent AgentSessions, one active Turn per Harness, private runtime-record decomposition, three compatibility keys, separate open-session occupancy, per-Turn three-family route-token binding, package-config before generated Context imports, required present exports, the one exact optional Workspace-manifest absence result, fixed `image.build/input` carriage, authenticated native inference and selected-MCP projections through the existing H2 session, and exact Codex native-handle resume across per-Turn child processes are implemented and locally tested. Retained A1 real-provider and reconnect evidence covers the accepted fault scenarios. Multiple Harness records per Sandbox, compatibility-keyed placement, and one Integration supervisor routing the declared private Harness identities are implemented; bounded concurrent active Turns and the real two-runtime acceptance remain divergent from the accepted target.
...
Boot Phase 8 is effect-free and registers only deterministic cleanup result expectations; the existing post-listener single-flight service owns effectful cleanup and fail-closed accepted-final-status handling. A retained exact result settles the original owner without replay, a successor poll-first observation marks the old request unknown and fences that connection, and NanoHost fail-stops a successor that loses the first poll response before completion while retaining reconnect behavior for the initial connection, a delivered retained result, and later polls. Every authoritative readiness report still projects the same coordinator-established `freshEmpty` proof, while Sandbox cleanup state and retained scheduler capacity prevent a healthy reconnect from clearing an uncertainty fence. The earlier A1 Unit F run remains historical evidence for its recorded commit and does not settle the current candidate's R001 gate.

Cargo and the application-local OpenShell release metadata select stock OpenShell `0.0.99`, its official SDK source, Gateway artifact, Supervisor image, and redistributed license bytes. The byte adapter and lifecycle paths have fast local regression coverage; the exact real-stock slow-consumer, saturation, cancellation, and reconnect qualification remains an opt-in live-host check and MUST NOT be inferred from those regressions. The retained A1 noninterference blocker remains independent and is not waived by an OpenShell integration result.

The upstream project has published releases beyond the supported release, and publication alone does not move the target. A newer candidate becomes selectable only after its matched official components pass the upgrade qualification in the Stock Integration Qualification Annex.
...
The `linux/arm64` distribution, side-effect-free `--version`, deterministic release packager, shared verifier, fixed-path installer, promoted execution-host manifest, isolated live-installer gate, and release-workflow composition defined by this amendment are implemented and pass their focused local regressions. Exact artifact candidate commit `1a5468bce556c14bb3dfc16550a9dc2f1c7adad5` completed the no-lifecycle A1 real-artifact gate with the real release-bound inputs, successful package and host prerequisites, the expected earlier-installation `destination-conflict`, unchanged live destinations, and byte-identical contained staging; independent verification accepted that arm64 evidence, and this architecture correction does not reopen that historical R002 result. The `linux/amd64` archive, target-aware installer and verifier, release manifest projection, native build job, and fresh real-host qualification remain unimplemented, so the two-target distribution contract is partial. R004 remains open until a separately authorized complete tag is published and verified.
...
1. Completed: establish the exact-release stock OpenShell client, selected bridge, transport mechanisms, and complete OS failure group.
...
8. Pending: obtain fresh exact-product no-host-reboot A1 evidence for the current R001 noninterference gate; execution-server restart truth remains assigned to the separate Worker outcome work rather than this release slice.
...
14. Before adopting any later OpenShell release, complete the fast foundation and full real-stock qualification against that candidate; an unavailable host or observation leaves the upgrade incomplete.

There is no compatibility mode after cutover. A failed stock-integration qualification or failed fail-stop proof blocks migration and returns the design to its owners.
...
- An ordinary definite local-owner failure uses only exact `{"requestId":"<64-lowercase-hex>","failureCode":"effect_failed"}` on its fixed result path; NanoCore matches the current operation and pending request, acknowledges and rejects it exactly once without success validation, and the same session continues only where the existing operation lifecycle permits. Physical or result-delivery uncertainty, partial or uncertain `storage.purge`, uncertain `bridge.open`, create, or delete, `reference.import`, and present-file `file.export` retain their special rules and gain no fallback; the sole non-failure JSON export result is exact optional absence on the same fixed path.
...
- The distinct fixed `127.0.0.1:17892` native listener and Supervisor-only `127.0.0.1:17891` bridge listener bind before the marker; native admission stays disabled until the nested H2 client is ready, and the first Harness poll `204` proves both listener roles before child launch. Initial and successor bridges continue to use only `17891`. Native requests use only exact authenticated `POST /inference/*` or enabled `POST /capabilities/*`, preserve end-to-end bytes, headers, content encoding, streaming, backpressure, and cancellation under the semantic family aggregate plus the separate 2 MiB inference or 512 KiB capability in-flight ceiling and common 64 KiB write bound, and create no outer stream for a wrong or cross-family token, disabled family, method, absolute-form target, or proxy capture. Closing Integration rejects both listeners, cancels active local requests, closes the one nested session, and uses existing Harness and Sandbox cleanup without local retry or replay.
...
- Worker-control, inference, and capability use distinct independently generated 32-byte tokens whose exact current scheduler lease owns three separate lowercase SHA-256 binding projections; no raw value is durable or derived from `sandboxBindingRef`, cross-family use is rejected, and capability remains non-callable unless the exact AEP enables selected MCP routes.
...
- The NanoHost compiles the official OpenShell Rust SDK fixed to the supported release commit by Cargo and `Cargo.lock`; no vendored protobuf copy, generated-client fallback, Gateway server crate, floating source, OpenShell source fork, or normal-operation CLI adapter enters the NanoHost build or runtime path.
...
- `nanohost --version` exits before configuration or runtime effects, and each target-native amd64 and arm64 release build reports the Cargo-owned version.
- Identical inputs produce byte-identical target-specific NanoHost archives with the exact file tree, modes, owner, group, timestamp, gzip metadata, checkout-owned installer/unit/host-profile/license bytes, release-bound target-matched Gateway and OpenShell license bytes, complete target-matched executable ELF identities, inner checksums, and exact host-profile id and digest. A table over all-absent, all-installed, partial-or-temp-residue, and conflicting destination states proves exactly one mutually exclusive disposition; and `--check-host` exact JSON/verdict/identity framing, live `--check`, profile-digest mismatch, required prerequisite, destination/ancestor/temp regular-file and symlink, caught-partial-residue cleanup, interrupted-temp resume, cooperating serialization, non-cooperating path appearance without overwrite, newly created contained `DESTDIR`, corrupt-input, wrong-host, conflicting-destination, traversal, repeated-root, existing-root, symlink-root, and symlink-ancestor checks all settle before an unauthorized write or readiness claim. Successful new and already-installed live paths emit their exact installation state and remaining operator steps without claiming readiness. Until the requirement-profile projection is implemented, the current exact Docker and slirp identity regressions remain required current-path checks rather than target qualification criteria.

### Fast OpenShell Integration Foundation

The ordinary NanoHost test path MUST remain local and deterministic. It compiles the Cargo-selected SDK, validates that Cargo, `Cargo.lock`, release metadata, packaging inputs, and readiness artifact identities describe one release, and exercises the real NanoHost byte adapter with controlled byte streams. The adapter checks MUST cover arbitrary input lengths and frame boundaries, exact byte preservation without loss or duplication, a full send queue that waits until consumption releases capacity, recovery after consumption resumes, and EOF, error, cancellation, and closure paths that release accounting without replay. A reversible negative case MUST show that the check detects broken byte preservation or backpressure rather than only exercising the happy path.

These checks are foundation evidence. They do not run a stock Gateway or Supervisor, prove upstream component compatibility, qualify a release for adoption, satisfy the saturation predicates, or settle the A1 noninterference blocker. Their failure blocks the candidate because OpenKit's own adapter or release projection is inconsistent.

### Real-Stock OpenShell Upgrade Qualification

Changing the supported OpenShell release requires an opt-in live-host qualification using the candidate's matched official SDK source, Gateway, and Supervisor. The qualification MUST exercise the real NanoHost path and stock processes; a fake Gateway, mocked Supervisor, source inspection, compile probe, or saved prior result cannot produce a PASS.

The required candidate scenarios are:

- slow consumption followed by recovery, proving that backpressure reaches the producer, outstanding bytes remain inside the composed bound, and the exact byte sequence settles without loss or duplication;
- saturating inference at both the stream and in-flight DATA ceilings for at least 60 continuous seconds while accepted heartbeats and interrupts continue to meet the unchanged liveness predicates;
- cancellation during active transfer, proving prompt capacity release, no logical request replay, and the owning interrupted or unknown outcome;
- bridge loss and successor establishment, proving predecessor fencing, no late-byte acceptance, no replay, and the existing bridge recovery bound; and
- the fixed bootstrap monitor and single-file Interactive helper behaviors on which NanoHost currently depends, including authentication, request/response completion, terminal-event handling, and fail-closed ambiguity.

Each scenario result MUST identify the candidate tag and commit, Gateway and Supervisor digests, effective NanoHost transport values, host and load conditions, scenario and attempt identity, expected predicate, observed value, and the stage at which a failure or delay occurred. Reports MUST distinguish prerequisite or collection failure, foundation failure, stock-integration incompatibility, an admissible calibration candidate, and a hard-ceiling or safety failure. A missing prerequisite, incomplete observation, invalid correlation, unavailable saturation proof, or unavailable hard-memory evidence is not a PASS.

Run the previously accepted release under the same conditions only when needed to distinguish an environment or instrument failure from a candidate regression; OpenKit does not maintain a multi-version support matrix. Parameter investigation changes one input at a time. A value may change only through the calibration rule and independent acceptance already defined here; the test runner MUST NOT loosen a target, hard ceiling, safety predicate, or sample requirement. The exact operational sequence and commands live in `docs/cookbooks/openshell-upgrade.md`.
...
4. Normal AgentSession end revokes its exact native continuation binding and removes only disposable control/request state while preserving retained native and working data, releases open-session capacity after proof, and preserves compatible siblings, the shared Sandbox, and the healthy epoch.
...
15. The NanoHost links only the official SDK source fixed to the supported release commit by Cargo, while the matching official checksum-verified Gateway runs as a separate foreground process, the matching exact Supervisor image is selected, and no ordinary lifecycle operation invokes the CLI.
...
34. Sandbox Integration keeps `127.0.0.1:17891` exclusive to every initial and successor stock Supervisor bridge and binds one separate native HTTP/1 listener at `127.0.0.1:17892` before Harness readiness. After the nested H2 client becomes ready, exact bearer requests may cross only from authenticated native `POST /inference/*` or enabled `POST /capabilities/*` to the one existing H2 session with each semantic request aggregate, the separate 2 MiB inference or 512 KiB capability in-flight ceiling, the common 64 KiB write bound, byte- and header-preserving streaming, backpressure, cancellation, and no local retry; exact loopback proxy bypass, cross-family and disabled-route rejection before outer stream creation, both-listener closure, and existing cleanup are observed.
...
47. The exact final candidate commit produces byte-reproducible `openkit-nanohost-<tag>-linux-amd64.tar.gz` and `openkit-nanohost-<tag>-linux-arm64.tar.gz` archives with the fixed tree, metadata, target, host-profile id and digest, and release-bound bytes; each target-matched NanoHost reports the Cargo-owned version without constructing the runtime; the shared verifier accepts each intact bundle and rejects corruption, target mismatch, missing or substituted checkout inputs, metadata drift, live-path or temporary-path conflict, and staging escape; the isolated fixed-path gate proves `--check-host` JSON/verdict/identity framing and every live disposition, requirement verdict, cleanup/resume, cooperating serialization, and non-cooperating no-overwrite boundary; and a fresh real-host run of each exact candidate, target, and profile reports requirements met plus the expected installation disposition, proves all live destinations unchanged when checking, and completes contained staging without `systemctl`, service lifecycle, host restart, current-deployment effect, or stale evidence reuse.

## Stock Integration Qualification Annex

This annex owns how NanoHost follows an official OpenShell release while trusting the upstream project's matched SDK, Gateway, and Supervisor as one tested product. It qualifies OpenKit's integration and retained product guarantees; it does not duplicate upstream protocol ownership or certify the OpenShell project as a whole.

### Supported Release Set

| Consumed surface | Supported realization |
| --- | --- |
| Release boundary | Official unmodified OpenShell `v0.0.99`, resolved to commit `8c7dd148a9e6360c9d5b2830e339a0dc4b3f3032` |
| SDK | Official Rust SDK source fixed to that commit by `apps/nanohost/Cargo.toml` and `Cargo.lock` |
| Gateway | Official checksum-verified target-matched x86-64 or AArch64 Gateway release artifact, run separately as a foreground Runtime Epoch process |
| Supervisor | Exact platform manifest `sha256:4adea8392a81ef34b3cc3284e693ac3cc6c13362fad84a492d95b53b3eb403b9` on `linux/amd64` or `sha256:b548fd939331d830cd9197f20fca9a5d95383c5e67f64929d632a37403115f38` on `linux/arm64` |
| Integration path | The SDK calls, stock relay, bootstrap monitor, fixed single-file helper, and container-backend process shape already specified above |

The official release identity is the compatibility unit. OpenKit does not maintain a supported cross-product of SDK, Gateway, and Supervisor versions, and a deployment that resolves those components to different releases remains non-ready.

### Release And Assumption Metadata

`apps/nanohost/openshell/release.json` contains the minimum ordinary dependency and supply-chain facts that Cargo and the container registry cannot carry together: release version and commit, external Gateway archive and binary checksums, selected Supervisor platform-manifest digests, and redistributed license-file checksums and bundle paths. Cargo and `Cargo.lock` remain the SDK source authority. Packaging, readiness, and focused consistency checks consume these owners directly.

`apps/nanohost/openshell/transport-assumptions.json` contains only the exact source evidence still needed to prove a finite bound for opaque stock buffering under `### Envelope Measurement Boundary`: the forward queue multiplicity and frame bound, both relay queue multiplicities and frame bounds, and the Gateway pairing-buffer bound per direction. It is not an interface snapshot. Its mechanical integrity check cannot certify the semantic memory proof. No checked-in OpenShell protobuf copy, compile probe, RPC inventory, historical release comparison, authentication annotation catalog, or whole-upstream source certification is required.

Every release candidate rebinds and reviews the three retained assumptions against that candidate's immutable source commit, even when the resulting values are unchanged. Only that bounded source slice is retained. SDK type or method changes are exposed by the Cargo build; byte semantics, backpressure, lifecycle, cancellation, reconnect, and failure behavior are exposed by the foundation and real-stock qualification. If OpenKit later enforces or observes the remaining opaque contribution without source evidence, the assumption entry is deleted in the same change.

### Upgrade Adoption

A newer official release is a candidate, not a supported version, until all of the following hold:

1. Cargo, the lockfile, release metadata, external artifacts, and any retained transport assumption identify the same immutable release.
2. The Fast OpenShell Integration Foundation passes against the candidate SDK source.
3. The Real-Stock OpenShell Upgrade Qualification completes with real candidate Gateway and Supervisor processes and satisfies every required observation.
4. Packaging and readiness consume the exact recorded external artifacts.
5. The actual diff and evidence receive independent acceptance under repository governance.

A prerequisite failure, unavailable live host, unavailable saturation observation, invalid report, or missing hard-memory proof leaves the qualification incomplete and cannot be narrated as PASS. A failed candidate does not create an automatic downgrade, compatibility mode, updater, retry loop, or alternate component mix; the currently supported release remains selected and any later attempt is a new reviewed change. The A1 exact-product noninterference blocker remains a separate release condition and is neither repeated nor waived here.
...
| A candidate SDK changes or removes the required client surface | Let the Cargo build and focused integration checks fail; adapt NanoHost within the accepted contract or reject the candidate. Do not add a generated-client fallback. |
| Gateway or runtime daemonizes outside the epoch cgroup | Require foreground execution and observed cgroup membership; fail real-stock qualification if any effect-capable process can escape group termination. |
...
| A candidate stock `ForwardTcp`/`RelayStream` bridge cannot carry the target | Reject the candidate at mandatory real-stock qualification; do not fork, patch, tunnel, or add a custom mux. |
...
| Nested flow control stalls or buffers without bound | Enforce explicit per-stream windows, per-family in-flight ceilings, and bounded chunking; measure throughput, added latency, and peak buffering during real-stock qualification and reject the candidate if buffering is unbounded. |

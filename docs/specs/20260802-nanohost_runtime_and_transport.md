...
The archive has one tag-and-target-prefixed root containing exactly `nanohost`, `openshell-gateway`, `openkit-nanohost.service`, `install.sh`, `MANIFEST.json`, `SHA256SUMS`, `licenses/openkit-LICENSE`, `licenses/openshell-LICENSE`, and `licenses/openshell-THIRD-PARTY-NOTICES`. `nanohost` and `openshell-gateway` MUST be little-endian 64-bit AArch64 ELF executables, the Gateway bytes MUST match the exact `v0.0.99` archive and extracted-executable identities projected by `apps/nanohost/openshell-pin/manifest.md`, and both OpenShell license files MUST match the same manifest's exact source-commit identities. `MANIFEST.json` is a generated non-authoritative projection of the release tag, target, required file set, fixed live destinations, prerequisite names, and pin facts; it does not own any of them.

The archive is byte-reproducible for identical inputs through fixed member order, path, content, mode, modification time, owner, group, and gzip metadata. It contains no configuration, credential, Token hash, deployment identity, environment file, OpenShell CLI, Supervisor image, Image Store content, runtime evidence, or source tree.

The installer writes only `nanohost` to `/usr/lib/openkit/nanohost` with mode `0755`, `openshell-gateway` to `/usr/lib/openkit/openshell-gateway` with mode `0755`, and `openkit-nanohost.service` to `/etc/systemd/system/openkit-nanohost.service` with mode `0644`. Before any target write, every mode verifies all inner checksums and both ELF identities. Default live installation additionally requires an `aarch64` host, fixed `/usr/bin/containerd` and `/usr/bin/dockerd`, the execution-host-manifest-owned `/usr/bin/docker` and `/usr/bin/slirp4netns` identities, and a systemd host.
...
- A missing, extra, corrupt, checksum-mismatched, non-AArch64, wrong-target, or pin-inconsistent distribution input fails before packaging or installation. Live installation on a non-`aarch64` host, without fixed `/usr/bin/docker` or another fixed prerequisite, or with a non-regular, symlink, byte-mismatched, or mode-mismatched destination, ancestor, or reserved temporary path fails before destination mutation; a non-canonical, root-resolving, existing, traversal-bearing, or symlink-bearing staging root fails before its creation. Caught pre-rename failure cleans only invocation-owned residue; interruption residue is reusable only when each current payload exactly matches the observed regular non-symlink destination or reserved temporary bytes and mode. A partial rename reports incomplete and may resume only from those per-file observations; stale real-artifact evidence or an artifact-affecting change cannot close distribution acceptance.
...
- Identical inputs produce byte-identical NanoHost archives with the exact file tree, pin-bound Gateway and OpenShell license bytes, AArch64 ELF identities, and inner checksums; a table over all-absent, all-installed, partial-or-temp-residue, and conflicting destination states proves exactly one mutually exclusive disposition; and live `--check`, `/usr/bin/docker` and other missing-prerequisite, destination/ancestor/temp regular-file and symlink, caught-residue cleanup, interrupted-temp resume, newly created contained `DESTDIR`, corrupt-input, wrong-host, conflicting-destination, traversal, repeated-root, existing-root, symlink-root, and symlink-ancestor checks all settle before an unauthorized write or readiness claim.
...
47. The exact final candidate commit produces one byte-reproducible `openkit-nanohost-<tag>-linux-arm64.tar.gz` with the fixed tree and pin-bound bytes; its AArch64 NanoHost reports the Cargo-owned version without side effects; the shared verifier accepts the intact bundle and rejects corruption, wrong architecture, missing inputs, live-path or temporary-path conflict, and staging escape; and a real A1 run of that exact commit reports successful package and host prerequisites plus the expected nonzero `destination-conflict` for the earlier installed NanoHost bytes, proves all live destinations unchanged, and completes contained staged installation without `systemctl`, service lifecycle, host restart, current-deployment effect, or stale evidence reuse.

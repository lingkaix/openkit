...
8. Partial: the current R001 runner admits F1, F2, and F4 with exact raw and NanoCore-restart-invariant baseline ownership, and an independently audited retained Aggregate accepts its recorded `38a83058f900f3cfdc01ade13fd42fbcf11cbf91` product commit. The rebased product still requires a fresh no-reboot Aggregate before R001 closes; Scenario 3 execution-server restart evidence remains separately deferred to R005 under the explicit no-reboot boundary.

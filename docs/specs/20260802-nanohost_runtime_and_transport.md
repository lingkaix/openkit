...
- The first NanoHost distribution boundary: the supported platform, exact bundle contents, binary identities, fixed live destinations and prerequisites, staging containment, installer effect and retry semantics, and acceptance evidence.
...
The first supported NanoHost distribution is one reproducible `linux/arm64` archive containing the AArch64 NanoHost binary, the exact supported stock Gateway, the existing OS unit, the minimal installer, inner checksums, generated bundle metadata, and the required OpenKit and OpenShell license material. It installs no configuration or credential, manages no service lifecycle, and makes no readiness claim before the operator completes the separately owned enrollment, configuration, image, and service-start steps.
...
Distribute that application first and only as `linux/arm64` through the exact archive and installer contract below. A second target, package manager, updater, service manager, deployment record, compatibility path, or cross-build framework requires a later accepted owner and is not implied by the runtime's source-level platform branches.
...
The NanoHost Distribution is the controlled execution-host archive owned by this specification and composed into an OpenKit release by `docs/specs/20260829-release_management.md`. The current and only supported distribution target is `linux/arm64`, and its filename is `openkit-nanohost-<tag>-linux-arm64.tar.gz`; `linux/amd64`, another operating system, a package registry, and a source archive are not supported distributions.

The archive has one tag-and-target-prefixed root containing exactly `nanohost`, `openshell-gateway`, `openkit-nanohost.service`, `install.sh`, `MANIFEST.json`, `SHA256SUMS`, `licenses/openkit-LICENSE`, `licenses/openshell-LICENSE`, and `licenses/openshell-THIRD-PARTY-NOTICES`. `nanohost` and `openshell-gateway` MUST be little-endian ELF64 AArch64 ET_EXEC or ET_DYN executables with a complete valid ELF header, a bounded complete ELF64 program-header table, and at least one executable file-backed `PT_LOAD` segment whose file range is present, memory size covers file size, alignment is valid, and file-backed virtual range contains the entry point; a header-only ELF is not an executable. The accepted executable segment has entry point and virtual address below `2^63`, each file offset, file size, memory size, and alignment below `2^32`, and its complete memory-backed virtual interval does not cross `2^63`; packaging, shared verification, and installation enforce that same bounded subset. The Gateway bytes MUST match the supported release's exact archive and extracted-executable identities in `apps/nanohost/openshell/release.json`, and both OpenShell license files MUST match that release metadata's exact source identities. `MANIFEST.json` is a generated non-authoritative projection of the release tag, target, required file set, fixed live destinations, prerequisite names, the prerequisite identity values owned by `apps/nanohost/deploy/host-manifest.json`, and OpenShell release facts; it does not own any of them. Its host identity projection is exactly `prerequisites.identities.docker` with `path` and `version` and `prerequisites.identities.slirp4netns` with `path`, `version`, and `sha256`.

The archive is GNU tar in the declared fixed member order. Both directories have mode `0755`; `nanohost`, `openshell-gateway`, and `install.sh` have mode `0755`; the service unit, manifests, checksums, and all licenses have mode `0644`; every member has numeric uid/gid `0/0` and mtime Unix epoch second `0`. The tar stream has two zero end blocks and only zero bytes in any remaining record padding. Gzip uses the exact header bytes `1f 8b 08 00 00 00 00 00 02 03`, which encode deflate, no optional fields, zero mtime, maximum-compression extra flags, and Unix OS. These values plus fixed path and content make identical inputs byte-reproducible. The bundled `install.sh`, service unit, and OpenKit license must equal the exact checkout bytes at `apps/nanohost/deploy/install.sh`, `apps/nanohost/deploy/openkit-nanohost.service`, and `LICENSE`. The archive contains no configuration, credential, Token hash, deployment identity, environment file, OpenShell CLI, Supervisor image, Image Store content, runtime evidence, or source tree.

The installer writes only `nanohost` to `/usr/lib/openkit/nanohost` with mode `0755`, `openshell-gateway` to `/usr/lib/openkit/openshell-gateway` with mode `0755`, and `openkit-nanohost.service` to `/etc/systemd/system/openkit-nanohost.service` with mode `0644`. Before any target write, every mode verifies all inner checksums and both ELF identities. Default live installation additionally requires an `aarch64` host, fixed `/usr/bin/containerd` and `/usr/bin/dockerd`, the exact `/usr/bin/docker` version plus `/usr/bin/slirp4netns` version and SHA-256 owned by `apps/nanohost/deploy/host-manifest.json`, and a systemd host.
...
Staging accepts only an absolute `DESTDIR` whose canonical normalized form is textually identical and not `/`, which does not exist, and whose existing ancestors are real directories rather than symbolic links. The installer creates the staging root with private permissions and creates every descendant itself before writing; traversal, repeated-root, existing-root, symlink-root, or symlink-ancestor input fails before any write. Staging verifies package integrity and AArch64 payload identity, skips host-only readiness checks, writes the three fixed destinations only beneath that root, and reports `staged-only` without making a live-install or runtime-readiness claim.
...
- A missing, extra, corrupt, checksum-mismatched, non-AArch64, wrong-target, release-inconsistent, or host-manifest-inconsistent distribution input fails before packaging or installation. Live installation on a non-`aarch64` host, without the exact promoted-manifest Docker or slirp identity or another fixed prerequisite, or with a non-regular, symlink, byte-mismatched, or mode-mismatched destination, ancestor, or reserved temporary path fails before destination mutation; a non-canonical, root-resolving, existing, traversal-bearing, or symlink-bearing staging root fails before its creation. Caught pre-publication failure cleans only invocation-owned residue; interruption residue is reusable only when each current payload exactly matches the observed regular non-symlink destination or reserved temporary bytes and mode. A partial publication or a destination appearing after observation reports incomplete without overwrite and may resume only from exact post-publication bytes, modes, and per-file observations; stale real-artifact evidence or an artifact-affecting change cannot close distribution acceptance.
...
- `nanohost --version` exits before configuration or runtime effects, and the native arm64 release build reports the Cargo-owned version.
- Identical inputs produce byte-identical NanoHost archives with the exact file tree, modes, owner, group, timestamp, gzip metadata, checkout-owned installer/unit/license bytes, release-bound Gateway and OpenShell license bytes, complete AArch64 executable ELF identities, inner checksums, and promoted host-manifest identity projection. A table over all-absent, all-installed, partial-or-temp-residue, and conflicting destination states proves exactly one mutually exclusive disposition; and live `--check`, exact Docker/slirp identity, other missing-prerequisite, destination/ancestor/temp regular-file and symlink, caught-partial-residue cleanup, interrupted-temp resume, cooperating serialization, non-cooperating path appearance without overwrite, newly created contained `DESTDIR`, corrupt-input, wrong-host, conflicting-destination, traversal, repeated-root, existing-root, symlink-root, and symlink-ancestor checks all settle before an unauthorized write or readiness claim. Successful new and already-installed live paths emit their exact installation state and remaining operator steps without claiming readiness.
...
47. The exact final candidate commit produces one byte-reproducible `openkit-nanohost-<tag>-linux-arm64.tar.gz` with the fixed tree, metadata, promoted host identities, and release-bound bytes; its AArch64 NanoHost reports the Cargo-owned version without constructing the runtime; the shared verifier accepts the intact bundle and rejects corruption, wrong architecture, missing or substituted checkout inputs, metadata drift, live-path or temporary-path conflict, and staging escape; the isolated fixed-path gate proves every live disposition, exact host identity, cleanup/resume, cooperating serialization, and non-cooperating no-overwrite boundary; and a real A1 run of that exact commit reports successful package and host prerequisites plus the expected nonzero `destination-conflict` for the earlier installed NanoHost bytes, proves all live destinations unchanged, and completes contained staged installation without `systemctl`, service lifecycle, host restart, current-deployment effect, or stale evidence reuse.
...
| Gateway | Official checksum-verified AArch64 Gateway release artifact, run separately as a foreground Runtime Epoch process |

...
The `linux/arm64` distribution, side-effect-free `--version`, deterministic release packager, shared verifier, fixed-path installer, promoted execution-host manifest, isolated live-installer gate, and release-workflow composition defined by this amendment are implemented and pass their focused local regressions. R002 remains open until the exact final candidate commit completes the no-lifecycle A1 real-artifact gate, and R004 remains open until a separately authorized tag is published and verified.

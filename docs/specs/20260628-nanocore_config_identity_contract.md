...
`workspace.jsonc` owns the editable Workspace `name`, Workspace-shared `defaultAgentId`, Agent Manifest bindings, internal-role bindings, logical-model visibility or preference, Skill and MCP composition references, credential-requirement bindings, Workspace roots, and other Workspace-owned resources accepted by their owners. `workspace-record.json` retains only system-owned identity, ownership, lifecycle, revision, and timestamp facts. A binding may add Workspace-owned resources and override or extend the referenced Server supply using stable field identities; it does not create another resolver or an unrelated full Workspace-local Agent Manifest.

The strict Workspace schema admits `assistant.resources` only as the existing Skill-ID selection defined by `20260909-internal_agent_resource_integration.md`. It is Workspace-authored composition, never a per-user Tool preference, copied server configuration or new catalog. The first slice admits no MCP selection field; a later capability-keyed binding requires its accepted adapter consumer and strict schema extension together. The runtime-enabled internal profile's three `limits` values/defaults are owned by `20260813-internal_agent_runtime.md`. Both additions remain Not Started and require their strict config-schema/runtime consumer together before enablement. Reload changes the next bounded internal run; no AEP is created for internal roles and no mid-run prompt or Tool mutation is permitted.

Workspace MCP server catalogs are not authored runtime-config files. Effective Gateway entries are projected from `DATA_ROOT/workspaces/<workspaceId>/catalog/catalog.json` under `docs/specs/20260907-mcp_catalog_management.md`. The runtime-config editor does not list, read, or write `mcp-servers.jsonc`.

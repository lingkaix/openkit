...
Two forms satisfy this and both already exist in the repository. A check may exclude a platform through a pre-run predicate, as `it.skipIf(process.platform === 'win32')` does, which is a declared opt-out and not a runtime-error skip. A check may instead make the divergence its subject and assert it, as the check bounding macOS `EPERM` to the first post-`TERM` liveness probe does; that is the stronger form, because it converts a platform difference from an obstacle into a contract. A check that does neither, and simply happens to pass on the author's machine, is the form this rule forbids.
...
- The manifest and its two halves live under `tests/support/host/`. If a deployment consumer later needs the facts a manifest declares, that second demonstrated consumer is what promotes it out of `tests/`; a prediction made in advance of one is not. `tests/` owns repository-level tests and shared cross-package test support, and `tests/support/` owns setup support with demonstrated consumers, which is what a host provisioner is and what a real-use gate is. The drift check that keeps the manifest honest against the rest of the repository is a root test rather than support, on the same terms as the image's.
- The manifest is addressed by a digest of its own content, computed the way `scripts/docker/test-image-tag.mjs` computes the image's build-input digest. On success the assertion half emits exactly `manifestDigest=<64-lowercase-hex>` followed by one newline, where the value is the SHA-256 of the exact asserted `manifest.json` bytes; a retained result consumes that observed value rather than recomputing or copying it. Without a computed identity emitted by the assertion that produced the observation, the environment identity that `docs/change-execution.md` requires of every real-use result is a phrase rather than a fact.

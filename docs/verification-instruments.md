...
Four independent conditions govern a predicate: obtain its deciding evidence, establish an instrument capable of a negative result, use a sound observation-to-verdict function, and run in an environment where the verdict means what it claims. Engineering judgments about value, materiality, or a reasonable method may remain evaluator-dependent; their delegation and resolution belong to `docs/change-execution.md`, not a claim of deterministic proof.
...
**Declare.** A gate claiming deterministic proof states its oracle's class from running each property's falsifier, not an unexamined claim. The falsifier column is the instrument rather than an illustration. A reasoned engineering decision instead states its question, authority, evidence, alternatives examined, and uncertainty; evaluator agreement is not a reproducibility proof.

**Demote.** A weak oracle informs and does not establish a deterministic predicate. Its observations may support a delegated Consultant, Reviewer, or Auditor decision under `docs/change-execution.md`, including pausing affected work. Such a decision is accountable judgment, not mechanical certification; it cannot waive strict controls or override accepted authority.
...
Some judgments, such as whether a change is the smallest coherent one, have no strong oracle. Agents may decide within delegated authority using reasons and evidence. Ask the engineer when the remaining choice belongs to the engineer or no credible route can be found, not merely because judgment is fallible.
...
A check that requires an effect domain its subject does not own is an architectural finding recorded under `docs/change-execution.md`, not repaired by granting the check access to that domain. `docs/engineering-doctrine.md` explains the rationale; `docs/specs/20260529-test_strategy.md` decides the lowest sufficient layer.

Use an existing owned observation channel first. A temporary probe may establish a named fact from actual records, interfaces, or execution when it preserves the subject's semantics and stays within authorized effect domains; retain its method, result, and limits. A one-off probe does not by itself require a permanent channel. If deciding evidence depends on a stable missing observation, or repeated reconstruction creates a parallel source of truth, return the gap to the subject's owner before relying on that reconstruction as a standing gate. Any durable channel belongs with the subject; when no product surface may expose it, its owning specification names a verification-only channel and its semantics before gate admission. A proxy cannot establish an unobserved effect, and convenience cannot expand access or weaken the oracle.
...
- For the reproducible real-use fixture, a manifest is applied by a command and never by a role. A manifest a role must interpret and apply is a runbook, and two correct applications of one runbook produce two different hosts with nothing in the evidence to tell them apart. Deployment provisioning may follow an authorized cookbook; the independent executable requirement check observes its resulting state and never infers a pass from the recipe or role report. The role's work is to run the command and adjudicate its result, which reduces the question of whether the host was prepared correctly to whether the command exited zero and the assertion half passed. This needs no new registered role: a builder owns the manifest and the command, and an independent reviewer adjudicates the outcome.
...
- A manifest describes the machine and never the product's state. The machine is prepared once, asserted, and long-lived. The product starts cold on each fixture-qualification attempt whose subject requires that state. Ordinary functional L6 and Agent workload/benchmark acceptance uses the persistent-deployment specialization in `docs/specs/20260909-persistent_deployment_acceptance.md`; it observes the current product build and creates scenario state without restarting a healthy deployment. Merging the two returns this tier to reconstructing the world per attempt, which is the cost the manifest exists to remove.
...
- For deployment qualification, [Deployment Host Requirements](specs/20260909-deployment_host_requirements.md) owns the machine-readable capability profile, supported role/artifact combinations, and typed requirement-check result. `apps/nanohost/deploy/host-manifest.json` is the NanoHost profile projection; observed machine identity and exact tool versions are separately retained evidence, never requirements copied from one qualified host. This specialization replaces identity-equality admission, not Harness Admission, current-host observation, cleanup, or real-subject proof.
- The provision and assertion halves and real-use harness programs remain under `tests/support/host/`; they consume the owning profile without copying its requirements. `tests/` owns repository-level verification and `tests/support/` owns demonstrated shared setup support. Profile drift is checked against its owning projections. A placement change requires owner reconciliation rather than a compatibility copy. Provisioning may use an authorized cookbook; its commands and result remain distinct from the executable assertion, and a role's interpretation never supplies a qualification verdict.
- The assertion emits the exact structured result defined by the profile owner, including observed profile/artifact, product-commit and machine identity. The profile is addressed by the SHA-256 of its exact raw bytes; the structured result is one JSON object followed by exactly one newline. Retained gate evidence consumes those emitted identities rather than recomputing, copying, or inferring them from a hostname. The current exact-identity comparator and single `manifestDigest` output remain implementation facts only; replacing them requires fresh Harness Admission and real bring-up on a host admitted by the capability profile. Prior exact-host passes cannot qualify the new instrument.
...
- Provisioning and bring-up are two commands and never one. Provisioning takes a machine to the manifest's declared state and runs once for as long as that state holds; bring-up starts the real subject on a provisioned machine, reaches the readiness a predicate requires, and runs on each fixture-qualification attempt. Ordinary persistent-deployment acceptance observes the already running product instead. Collapsing them makes every attempt pay for provisioning and hides which of the two failed, which is the distinction the manifest exists to make legible.
- For NanoHost real-use qualification, the assertion half runs through the existing `pnpm host:assert <alias>` command before real use rather than from a parallel preflight or wrapper. This remote Linux harness does not implement local or macOS NanoCore prerequisite inspection; those dependencies remain in the deployment guidance and existing toolchain/image owners. Bring-up runs the same assertion immediately before starting the subject.
- Credentials created by reproducible qualification fixtures are generated per attempt, are attempt-local, and are removed on every terminal path. Ordinary acceptance against an authorized persistent deployment may use its current securely stored access credentials and Provider subscriptions; these are deployment resources, not fixture inputs or cleanup targets. This exception does not allow retained evidence to carry credential values, revive revoked grants, or waive credential isolation in qualification tests. No manifest, closure, sealed artifact, or committed file carries credential material of any kind: a manifest declares that a credential satisfying a stated requirement must exist and never its value, and a fixture attempt offered credential material from a sealed or static evidence input rejects it rather than consuming it. This is a `[SCOPE-004]` boundary and it does not relax inside any workspace however bounded, nor inside the agent iteration workspace that `docs/engineering-doctrine.md` defines. The rule is stated here because the first program to need a real-use credential solved it correctly inside its own fixture with no owner to carry the solution forward, and because a manifest is precisely the artifact that would otherwise attract the value, being the place a reader looks for what the host must have.

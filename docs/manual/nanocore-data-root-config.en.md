...
status: Accepted
---
# NanoCore DATA_ROOT Config

This manual explains the user-editable files under `DATA_ROOT/config` for NanoCore.
...
When configured, NanoHost uses a dedicated native HTTP/2 listener that is separate from the App listener under `server.bind`.

| Option | Type | Description |
| --- | --- | --- |
| `identityId` | non-empty string | Configured NanoHost integration identity. |
| `deploymentId` | non-empty string | Deployment identity bound to the NanoHost transport token. |
| `bind.host` | non-empty string | Local host for the dedicated NanoHost HTTP/2 listener. This field has no environment override. |
| `bind.port` | integer `1..65535` | Local port for the dedicated NanoHost HTTP/2 listener. This field has no environment override. |
| `rendezvousUrl` | exact HTTP or HTTPS origin | Endpoint the NanoHost connects to. Plain HTTP is accepted only when both this host and `bind.host` are loopback. |
| `credentialRef` | non-empty string | Non-secret reference naming the NanoHost transport credential. |
| `credentialSlots.A` / `credentialSlots.B` | distinct absolute path pairs | Fixed `secretPath` and `companionPath` safe-sink files for rotation. |

The NanoHost and App listeners must use different local TCP ports; sharing one port on different interface addresses is not supported. A non-loopback NanoHost bind or rendezvous requires HTTPS and readable `OPENKIT_TLS_CERT_FILE` plus `OPENKIT_TLS_KEY_FILE` material. Missing TLS material, a port collision, or a public request on the private listener fails closed; NanoCore does not fall back to the App listener or a proxy.
...
Local and server modes both use the encrypted-file Vault under `DATA_ROOT/server/vault/`; there is no backend selector.
...
Use `oauth` for the supported OpenAI Codex and xAI subscription families whose credentials are obtained through NanoCore's provider-subscription account manager rather than a static API key. Do not use it for plain API-key services or unsupported OAuth families.
...
| `oauth` | Supported subscription providers | No | No | Provider-specific model IDs |
...
`secretRef` is the authored credential path for non-subscription providers. Those credentials should use `vault://<referenceId>`, and the matching `VaultReference` must exist before the provider is usable.
...
Non-subscription provider config has no authored credential path other than `secretRef`. Inline fields such as `apiKey`, `token`, `secret`, and `clientSecret` and username or password userinfo in `baseUrl` are rejected. The consumer-free `extraHeaders` and `extraBody` fields are removed, rejected as unknown fields, and have no compatibility path. Keep raw credential values in the vault.

### Provider Subscription Accounts

OpenAI Codex and xAI subscription profiles use `kind: "oauth"` and bind explicitly to one existing provider-scoped account slot through the strict extension `extensions.openkit.subscriptionAccount: { accountSlotId }`. The supported family is resolved from normalized `vendor` or, when `vendor` is not recognized, normalized `id`; normalization trims, lowercases, and replaces hyphens with underscores. The recognized values are `openai_codex` and `xai`, and conflicting recognized values are rejected.

Subscription profiles must omit `secretRef` and `baseUrl`. NanoCore never guesses a default slot, and `accountSlotId` must match `^[a-z0-9][a-z0-9_-]{0,63}$`. The same slot text may be used under both subscription providers because the durable identity is `(subscriptionProviderId, accountSlotId)`.
...
The configured model list does not prove subscription entitlement; the provider may still reject a model after login.

Each slot's non-secret authority lives at:

```text
DATA_ROOT/server/files/provider-subscriptions/<subscription-provider-id>/accounts/<account-slot-id>/account.json
```

`account.json` stores slot metadata and an internal Vault reference, never credential material. Credentials live only in the encrypted-file Vault under `DATA_ROOT/server/vault/`.

Existing Codex credentials are not imported, so create the provider-scoped slot and login again after cutover. Stock pi-ai performs Codex and xAI device-code login, credential refresh, logout, and authenticated inference through the slot-scoped credential store. Codex quota reports `available` or `temporarily_unavailable`; xAI quota reports `unsupported`. Provider-subscription account management and Gateway dispatch do not depend on Codex app-server, `CODEX_HOME`, or Codex-managed `auth.json`.

The authoritative account contract is [Provider Subscription Accounts](../specs/20260721-provider_subscription_accounts.md), and encrypted credential storage is defined by [Vault Backend Implementation](../specs/20260704-vault_backend_implementation.md).

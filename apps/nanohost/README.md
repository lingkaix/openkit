...
Private backend and Gateway processes receive the validated epoch resolver set through a read-only `/etc/resolv.conf` projection in their own recursively private mount namespaces. Docker `--dns` supplies the same set to containers; it does not configure daemon registry lookups. The inherited system Docker socket mask remains in place, and the host resolver is unchanged. An unavailable upstream or failed resolver projection fails startup without a fallback.

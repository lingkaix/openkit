...
The story suite uses `apps/web/playwright.stories.config.ts` to execute Markdown story artifacts from `tests/stories/` through deterministic Playwright adapters in `tests/story-runner/`. The deterministic story runs the local self-check flow without real Codex, real provider credentials, or external network access. The real Codex Goal Mode story is agentic-only and has a separate opt-in preflight command, `pnpm -w test:stories:real-codex`.

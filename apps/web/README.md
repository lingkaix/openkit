...
The live Tier-A **Catalog** screen projects selected-Workspace Skill, MCP, and Agent Plugin catalog management through `client.catalog`. It imports SKILL.md trees, pins current Skill versions, creates inactive MCP configurations, toggles enablement, and imports plugin.json packages. Fresh Skill and MCP fields keep actual values empty and show examples only as native placeholders; Skill file import stays disabled until a display name is entered. Native Codex plugin loading is not advertised. Stdio MCP enablement remains a deployment-admin authority on the server. Writes stay disabled while disconnected.

...
The published Tier-A **AI interface** screen is a separately authorized deployment-admin workflow inside the Settings shell. It reuses the Configuration security posture: the current browser session is tried first, and a `server-admin` token is requested only after a typed `401` or `403` and kept only in mounted component memory. General Settings reads only the selected Workspace, exposes its ordinary-user default model, Agent, and Skill controls, and never requests deployment diagnostics or runtime config. The separate Tier-A **Configuration** screen uses the existing `client.runtimeConfig` file, validation, revision, and safe-reload contract; local mode opens directly, while server mode asks for a `server-admin` token only after the current browser session is denied and keeps that token only in mounted component memory.
...
The live Tier-A board-19 **Repositories** screen projects selected-Workspace repository resources, diagnostics, default-repository setup, durable push records, and the existing approval-gated Git push workflow through `client.repositories`. It requests approval for one exact target, executes only a matching granted approval, and re-reads the authoritative repository projection without adding an API or external-effect owner. Its approval response remains TanStack mutation data, and its route appears once beneath the successfully discovered, authorized Workspace name beside direct new-conversation and Workspace settings entries instead of in global primary navigation. Workspace Sync review-to-apply is a separate live **Workspace changes** surface. Chat exposes the same selected-Workspace control on its starter and active-thread views, and changing it from a thread returns to the starter before issuing commands in the new Workspace. The complete collapsible Workspace/thread tree remains outside this surface. Focused tests cover this Web projection; no real external push is claimed here.
...
src/
  ...
  screens/
    ...
    portability/      Tier-A user import plus project-Workspace export and Vault rebind
    ...
    settings/         Tier-A General, Configuration, AI interface, read-only Workspace Vault, Usage & audit, and Debug

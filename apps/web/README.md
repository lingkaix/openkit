...
The live Tier-A board-19 **Repositories** screen projects selected-Workspace repository resources, diagnostics, default-repository setup, durable push records, and the existing approval-gated Git push workflow through `client.repositories`. It requests approval for one exact target, executes only a matching granted approval, and re-reads the authoritative repository projection without adding an API or external-effect owner. Its approval response remains TanStack mutation data, and its route appears once beneath the successfully discovered, authorized Workspace name beside a direct Workspace settings entry instead of in global primary navigation. Workspace Sync review-to-apply is a separate live **Workspace changes** surface. The complete collapsible Workspace/thread tree remains outside this surface. Focused tests cover this Web projection; no real external push is claimed here.

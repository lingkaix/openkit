...
For deployment-owned registration beyond this inventory, use the separate `config/model-catalog.jsonc` file described in the [model-not-in-models.dev recipe](../../../../../skills/openkit-ops/references/nanocore-data-root-config.en.md#model-not-in-modelsdev). The opt-in profile examples remain examples; template edits are not needed to add catalog metadata.

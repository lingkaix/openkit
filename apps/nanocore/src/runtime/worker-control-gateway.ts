...
export interface WorkerControlRegistrationOptions {
  /** Scheduler-owned non-secret sandbox binding associated with both route-token families. */
...
export type WorkerRouteTokenFamily = 'worker-control' | 'inference';
...
export class WorkerControlGateway {
  ...
  /**
   * Registers one package snapshot with two distinct live-memory route tokens.
   *
   * @param environmentPackage Resolved Agent Environment Package.
   * @returns Registration containing the raw bootstrap pair and hash-only durable projections.
   */
  public registerSession(
    ...
    if (
      (options.workerControlToken === undefined) !==
      (options.workerInferenceToken === undefined)
    ) {
      throw new Error('Worker-control and worker-inference tokens must be supplied together.');
    }
    ...
    if (token === workerInferenceToken) {
      throw new Error('Worker-control and worker-inference tokens must be distinct.');
    }
  ...
  /**
   * Revokes one live package session and both route-token families.
   *
  ...
  public restoreSession(input: WorkerControlSessionRestoreInput): void {
    ...
    if (input.workerInferenceTokenHash === input.workerControlTokenHash) {
      throw new WorkerControlGatewayError(

...
function workspaceMaterializationRefusalExplanation(
  ...
  if (startup.reason === 'git_fetch_transport_failed') {
    return ' The worker could not complete the Git fetch transport. This is a sandbox network or HTTP failure, not proof that the remote lacks the commit. Host repository diagnostics only confirm the local checkout, and the incomplete slot stays in place.';
  }
  return '';
}

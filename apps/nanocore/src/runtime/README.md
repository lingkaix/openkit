...
- `agent-environment.ts` owns the metadata-only pre-lease AEP and SessionCompatibilityKey projection and the full effect-owning AEP construction; scheduler and runtime owners use that projection for compatibility and one-Sandbox capacity admission, then revalidate before launch. Incompatible idle retirement drains existing bindings before exact private closure and physical deletion; cached reuse cannot bypass the durable drain state.

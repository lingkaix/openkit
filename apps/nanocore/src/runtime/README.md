...
Environment preparation binds private authored-candidate lineage to image effects before dispatch. `worker-image-settlements.ts` owns the immutable Server SQLite outcome written before acknowledgment, independent of candidate visibility and current user credentials. Local persistence failure returns exact empty `503` and `recovery_required` to the caller while retaining result correlation; later explicit recovery reads the durable outcome without rebuilding. These records grant no activation or readiness authority. Package planning and attachment reservation share the binding owner's work-slot resolver so sequential handoff retains the selected work directory.

...
- Publish logical model IDs only. Private Provider profile and native model route members remain Gateway-owned. The pinned models.dev inventory supplies optional metadata, not model-ID admission. Unknown family is `null` and permits one authored route only; multi-route models require a shared known family across authored members, including unavailable Providers. An admitted logical model stays available while at least one member is dispatchable.

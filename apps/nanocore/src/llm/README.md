...
- Internal worker inference accepts only its distinct inference-family live token plus the narrow transport contract; the authenticated AEP still owns provider/model selection and durable lineage, privileged request fields remain server-owned, Worker Responses local tools normalize to the exact message-anchored prefix after closed-vocabulary validation, and worker-control or sandbox-binding values cannot authenticate inference.

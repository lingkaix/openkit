...
describe('capability usage ledger', () => {
  ...
  it('does not duplicate usage rows when the same idempotent call is retried', () => {
    const dataRoot = mkdtempSync(join(tmpdir(), 'openkit-capability-ledger-usage-retry-'));
    const workspaceDb = openWorkspaceDb(dataRoot, 'local-user', 'ws_demo');
    ...
    try {
      ...
      expect(usageRows(workspaceDb, first.id)).toHaveLength(2);
    } finally {
      workspaceDb.sqlite.close();
    }
  });

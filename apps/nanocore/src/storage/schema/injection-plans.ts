...
export type InjectionPlanStatus = 'active' | 'revoked' | 'expired';
...
export const injectionPlans = sqliteTable(
  'injection_plans',
  {
    ...
    status: text('status').$type<InjectionPlanStatus>().notNull(),
    /** ISO timestamp for plan creation. */
    createdAt: text('created_at').notNull(),
  },
  (table) => [
    index('injection_plans_grant_idx').on(table.grantId, table.status),
    index('injection_plans_capability_idx').on(table.capabilityId, table.status),
  ]
);

...
- `nanohost-transport-token.ts` / `nanohost-transport-token-store.ts` / `nanohost-transport-routes.ts` own the dedicated NanoHost transport Token class, hash-only Core store, server-admin enrollment/lifecycle routes, and the configured RuntimeTarget readiness observation. Issuance and verification require the Token's exact active identity/deployment owner, so orphan or decommissioned material cannot authenticate, update last-use metadata, rotate, or produce another Token. Enrollment inserts only a lineage-free configured identity or reactivates the exact retained decommissioned identity/deployment pair with one fresh Token; retained Token history without its identity row and every active, missing, duplicate, or cross-bound lineage fail closed.

...
- Keep `gateway.jsonc`, `internal-role-profiles.jsonc`, `providers/*.provider.jsonc`, `agents/*.agent.jsonc`, `workspaces/*/config/workspace.jsonc`, `workspaces/*/config/data-sources.jsonc`, `workspaces/*/config/mcp-servers.jsonc`, and `users/*/config/user.jsonc` distinct in snapshots and diagnostics.
...
- MCP catalog changes are session-scoped and deployment-admin-only; field-level authority changes emit Workspace audit events, while admitted Turns retain the captured catalog snapshot.

...
/**
 * Rejects fenced code blocks in a story body.
 *
 * Owned by `docs/specs/20260529-l6_story_acceptance.md` Admission: L6 is
 * agent-first by definition, so committed executable detail in story prose is
 * the actor's trajectory fed back through Setup. A story that appears to need
 * it has failed admission, and its proof belongs to an L3 or L4 test.
 *
 * @param {string} body Markdown story body after the front matter block.
 * @param {string} sourceName Human-readable source name for error messages.
 * @returns {void}
 * @throws {Error} When the story body contains a fenced code block.
 */
export function validateStoryHasNoFencedCode(body, sourceName = 'story') {
  for (const [index, line] of body.split('\n').entries()) {
    if (line.trimStart().startsWith('```')) {
      throw new Error(
        `${sourceName} must not contain a fenced code block; line ${index + 1}. Executable detail means the proof is mechanical and belongs to a lower test layer.`
      );
    }
  }
}
...
/**
 * Validates one parsed Story document and its repository-owned contracts.
 *
 * @param {ParsedStoryDocument} story Parsed Story document.
 * @param {string} repoRoot Absolute repository root used to resolve contract paths.
 * @param {string} sourceName Human-readable source name for error messages.
 * @returns {void}
 * @throws {Error} When metadata, sections, or an owning contract path or file is invalid.
 */
export function validateStoryDocument(story, repoRoot, sourceName) {
  validateStoryMetadata(story.metadata, sourceName);
  validateStoryBodySections(story.body, sourceName);
  validateStoryHasNoFencedCode(story.body, sourceName);
  const realRepoRoot = realpathSync(repoRoot);

  for (const contractPath of parseStoryContracts(story.metadata, sourceName)) {
    const absolutePath = resolve(repoRoot, contractPath);
    const repositoryPath = relative(repoRoot, absolutePath).split(sep).join('/');
    ...
    if (
      ...
      !/^docs\/(?:core|specs)\/(?:[^/]+\/)*[^/]+\.md$/u.test(contractPath)
    ) {
      throw new Error(
        `${sourceName} contract path must be a canonical repository-relative owning document: ${contractPath}`
      );
    }

    if (!existsSync(absolutePath)) {
      throw new Error(`${sourceName} references missing contract document: ${contractPath}`);
    }

    if (
      realpathSync(absolutePath) !== resolve(realRepoRoot, contractPath) ||
      !lstatSync(absolutePath).isFile()
    ) {
      throw new Error(
        `${sourceName} contract document must be a repository-owned regular file: ${contractPath}`
      );
    }
  }
}

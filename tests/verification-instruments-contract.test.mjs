...
test('keeps the real-use host manifest invariants', () => {
  ...
  assertRetains(
    ...
    [
      ...
      'Ordinary functional L6 and Agent workload/benchmark acceptance uses the persistent-deployment specialization in `docs/specs/20260909-persistent_deployment_acceptance.md`',
      'it observes the current product build and creates scenario state without restarting a healthy deployment',
      'Provisioning and bring-up are two commands and never one.',
      'Ordinary persistent-deployment acceptance observes the already running product instead',
      ...
      'Credentials created by reproducible qualification fixtures are generated per attempt, are attempt-local, and are removed on every terminal path.',
      'Ordinary acceptance against an authorized persistent deployment may use its current securely stored access credentials and Provider subscriptions',
      'these are deployment resources, not fixture inputs or cleanup targets',

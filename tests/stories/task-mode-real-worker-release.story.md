...
Verify that a release owner can use the OpenKit MCP facade to start one bounded Task Mode request whose real OpenShell/Codex worker spawns at least two runtime-internal sub-agents while NanoCore preserves one outer result, a complete runtime forest, trusted relay attribution, cache lineage, cached-token telemetry, audit linkage, and redacted product evidence.
...
4. Start Task Mode through `openkit.start_task` with one bounded repository change request that explicitly delegates two independent inspections to at least two Codex sub-agents before producing one outer result.
5. Read the Task Mode thread after the attempt returns.
6. Read Action Center for pending review, approval, or question rows.
7. Read capability usage plus the public EvidenceBundle, RuntimeEvidence, and audit ledgers.
8. Record the Task Mode state, worker selection, provenance assertion summary, evidence ids, workspace review ids, and final git status.
...
- RuntimeEvidence identifies the actual backend as OpenShell 0.0.80 and reports at least one root, two children, four retained streams, and complete reconciliation of at least three authenticated worker Gateway calls.
...
- The run contains at least three distinct runtime origins and at least two cache lineages; positive cached-input token rows remain queryable when the provider reports them, while absence means zero or unreported and is recorded instead of treated as a cache failure.
...
- Public RuntimeEvidence reports backend type `openshell`, backend version `0.0.80`, at least four streams, one root, two children, and complete reconciliation for at least three Gateway calls.

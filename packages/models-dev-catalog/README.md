...
- `snapshots/YYYY-MM-DD/metadata.json` records source URL, source project, fetch evidence, raw and formatted checksums, observed source revision, maintenance note, and provider-template mappings.
...
4. Record `sourceUrl`, `sourceProject`, `version`, `refreshedAt`, exact fetch time, response ETag, raw and formatted checksums, the observed source revision, provider mappings, and a maintenance note in `metadata.json`.

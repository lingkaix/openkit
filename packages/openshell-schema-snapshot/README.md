...
- `snapshots/2026-07-11/metadata.json` records the pinned OpenShell release provenance, mapping version, gateway compatibility range, source paths, and artifact checksums.
- `snapshots/2026-07-11/provider-profile-surface.json` separates the upstream provider categories, built-in profiles, auth, and refresh surface from OpenKit's emitted profile mapping.
- `snapshots/2026-07-11/policy-surface.json` separates the upstream protocol, enforcement, and access surface from OpenKit's narrower emitted sandbox policy mapping.
- `snapshots/2026-07-11/cli-surface.json` records the OpenShell CLI subcommands invoked by NanoCore.
...
4. Update `metadata.json` release tag, resolved commit, reviewed source paths, checksums, mapping version, and compatibility range.

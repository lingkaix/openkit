...
Relay-required Agent Environment Packages use the `trusted-worker-inference-relay` capability. The canonical schema requires one OpenAI-compatible NanoCore gateway route, placeholder credential visibility, a matching secret-free gateway provider/model, and no `workerBaseUrl`, native URL, direct credential, vault, provider attachment, or provider-backed MCP supply. The selected worker adapter owns its fixed Sandbox Integration URL, while unrelated manifest-authored development grants may coexist with the mandatory deny-by-default OpenShell policy.

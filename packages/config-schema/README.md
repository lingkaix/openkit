...
Authored configuration is split by owner: Server resource and fallback files (`server.jsonc`, `gateway.jsonc`, internal-role profiles, Providers, and Agent Manifests), shared Workspace composition (`workspace.jsonc` and data-source catalogs), and lightweight User preferences (`user.jsonc`). Explicit request or Orchestrator selection is most specific, followed by User, Workspace, and Server defaults. Workspace configuration owns its name, default Agent, Agent and internal-role bindings, roots, Assistant repository inspection, and extensions; `workspace-record.json` remains the machine-owned record.
